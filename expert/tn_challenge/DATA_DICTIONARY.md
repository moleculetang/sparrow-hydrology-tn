...
`data/*.npz` 均只有 `values` 一个数值数组，可用 `np.load(path, allow_pickle=False)['values']` 读取。浮点为float64；日期、索引为整数；enabled为bool。无已拟合TN状态或历史残差缓存。每天覆盖1961-01-01至2024-12-31，共23,376天；月份768。当前输入产品为原冻结`sensitivity`水文配S1氮源，和最新F23/F24参数路径相同的产品身份。
