...
from text_arrays import read_arrays
...
def load_data(directory=HERE/'data'):
    ...
    for name,h in hashes.items():
        if name.startswith('arrays/') or name=='topology.json':
    ...
    arrays=read_arrays(directory/'arrays')
    if set(arrays)!=set(meta['array_files']):raise ValueError('Array field mismatch')
    for key in meta['array_files']:setattr(d,key,arrays[key])
