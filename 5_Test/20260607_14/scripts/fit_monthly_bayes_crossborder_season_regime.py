...
def add_hydrologic_features(
    ...
    crossborder_class = crossborder_strength_by_reach(
        external_fraction=external_fraction, decay=crossborder_decay, max_order=8
    )
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_14 cross-border inflow + season-regime Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_12 season x flow-regime SAS hierarchy and adds a cross-border inflow proxy on the reach 112 Zuojiang/Longzhou downstream chain.",
        "",
        ...
        "",
        "The cross-border chain starts at reach 112 and propagates downstream through topology with a fitted decay. No 2019-2022 observed Q is used to form the proxy. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_14.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_crossborder_season_regime_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_crossborder_season_regime_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
