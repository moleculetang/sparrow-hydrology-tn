...
def main() -> None:
    ...
    manual_reservoir_tokens = ("水库", "坝上", "坝下", "渠道", "大坝")
    manual_reservoir_name = evidence["q_site"].astype(str).apply(lambda name: any(token in name for token in manual_reservoir_tokens))
    evidence.loc[manual_reservoir_name & evidence["reservoir_relation"].eq("not_reservoir_related"), "reservoir_relation"] = "manual_station_name_reservoir"
    evidence["reservoir_deferred"] = (~evidence["reservoir_relation"].eq("not_reservoir_related")) | manual_reservoir_name
    ...
    evidence["candidate_priority"] = (
        200 * evidence["persistent_model_failure"].astype(int)
