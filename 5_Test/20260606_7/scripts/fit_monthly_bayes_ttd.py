...
def add_hydrologic_features(df: pd.DataFrame, rho: float, wm: float, et_gamma: float, ttd_lag: int, ttd_tau: float) -> pd.DataFrame:
    out = df.copy()
    ...
    out["legacy_effective_mm"] = np.maximum(ppt - aet - et_gamma * et_deficit, 0.0)
...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    # Keep the successful 20260606_4 ET-aware state near its best values, and test
    # only the explicit travel-time kernel in this generation.
    for rho in [0.70]:
        for wm in [480.0]:
            for et_gamma in [0.50, 0.75]:
                for ttd_lag in [3, 6, 12, 24]:
                    for ttd_tau in [1.5, 3.0, 6.0, 12.0]:
                        featured = prepare_design(add_hydrologic_features(df, rho=rho, wm=wm, et_gamma=et_gamma, ttd_lag=ttd_lag, ttd_tau=ttd_tau))
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260606_7 TTD-kernel strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_ttd_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_ttd_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_ttd_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_ttd_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_ttd_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_ttd_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(param_rows).to_csv(REPORT_DIR / "monthly_bayes_ttd_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_ttd_station_random_effects.csv", index=False, encoding="utf-8-sig")
    ...
    kernel_df.to_csv(REPORT_DIR / "monthly_bayes_ttd_kernel_weights.csv", index=False, encoding="utf-8-sig")
    fig, ax = plt.subplots(figsize=(6.2, 3.8))
    ax.bar(kernel_df["lag_month"], kernel_df["weight"], color="#477998")
    ax.set_xlabel("Lag month")
    ax.set_ylabel("Kernel weight")
    ax.set_title(f"Best TTD exponential kernel: L={int(best['ttd_lag'])}, tau={float(best['ttd_tau']):.1f}")
    ax.grid(axis="y", alpha=0.22)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_kernel_weights.png", dpi=300)
    plt.close(fig)

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260606_7 monthly TTD-kernel empirical-Bayes model",
        "",
        "This numeric generation returns to the 20260606_4 ET-aware baseline and adds an explicit exponential travel-time-distribution kernel over historical effective water input.",
        "",
        "## Equation",
        "",
        ...
        "",
        "Hydrologic terms include Q_calc, Q_ma, cumulative area, monthly basin water surplus, threshold surplus, antecedent wetness state, explicit TTD-convolved net water input, TTD-threshold flow, TTD-wetness interactions, explicit AET/PET/ET-deficit terms, ET ratios, aridity, seasonal sine/cosine, and reservoir lag indicators.",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_ttd_metric_summary.csv",
        "- reports/monthly_bayes_ttd_metrics_by_station.csv",
        "- reports/monthly_bayes_ttd_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_ttd_fixed_parameters.csv",
        "- reports/monthly_bayes_ttd_station_random_effects.csv",
        "- reports/monthly_bayes_ttd_kernel_weights.csv",
        "- figure/monthly_bayes_ttd_kernel_weights.png",
        "",
    ]
    (RUN_DIR / "README_20260606_7.md").write_text("\n".join(lines), encoding="utf-8")
