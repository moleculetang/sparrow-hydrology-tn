                                        ...
                                        for base_rho in [0.85]:
                                            for base_release in [0.10]:
                                                ...
                                                for fixed_sigma in [3.0]:
                                                    for production_sigma in [1.5]:
                                                        for station_sigma in [1.00]:
                                                            for slope_sigma in [0.15]:
                                                                for regime_slope_sigma in [0.25]:
                                                                    for anomaly_weight in [0.5, 1.0, 2.0, 4.0]:
                                                                        for flow_contrast_weight in [2.0, 4.0]:
                                                                            beta = fit_map_ridge(
                                                                                tr,
                                                                                stations,
                                                                                mean,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_20 production-routing strong anomaly-objective Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_19 nonlinear monthly production-store and quick/base routing mechanism, but changes the fitting objective substantially. It forces the model to learn both log-flow level and station-standardized monthly anomaly/flow-duration contrast, targeting the persistent KGE alpha bottleneck.",
        "",
        ...
        "",
        "`logQ = fixed monthly hydrologic/SAS terms + nonlinear production-routing terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors + strong calibration anomaly/contrast objective rows`",
        "",
        "The production states are identical in kind to 20260607_19. The change is the calibration objective: station-centered standardized log-flow anomaly rows and high-low/peak-mid flow contrast rows are appended to the MAP system using fitting-period observations only. No 2019-2022 observed Q is used for states, gates, hyperparameters, contrasts, or corrections.",
        "",
    ...
    (RUN_DIR / "README_20260607_20.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_production_anomaly_objective_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_production_anomaly_objective_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
