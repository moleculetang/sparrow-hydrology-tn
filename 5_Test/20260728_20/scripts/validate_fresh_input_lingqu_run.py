...
def main() -> None:
    ...
    assert status["status"] == "passed" and status["completed_steps"] == 8
    assert gate["passed"]
    ...
    formal = json.loads((OUT / "formal_baseline_gate.json").read_text(encoding="utf-8"))
    assert formal["passed"]
    print(json.dumps({"status": "PASS", "strict_station_count": diagnostic["all_station_strict_summary"]["fresh"]["station_count"], "common_station_count": diagnostic["common_station_strict_summary"]["common_station_count"], "formal_baseline_gate": "PASS"}, ensure_ascii=False))
