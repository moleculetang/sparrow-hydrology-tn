...
MASS_BALANCE_SCRIPT = ROOT / "5_Test" / "20260607_25" / "scripts" / "fit_monthly_constrained_mass_balance.py"
...
def load_mass_balance_module():
    spec = importlib.util.spec_from_file_location("mass_balance_20260607_25", MASS_BALANCE_SCRIPT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load mass-balance script: {MASS_BALANCE_SCRIPT}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def mass_balance_prior_by_fit_end(fit_end_year: int) -> pd.DataFrame:
    mb = load_mass_balance_module()
    cols = mb.design_columns()
    df = mb.prepare_mass_balance_design(0.98)
    train = df[df["year"] <= fit_end_year].copy()
    beta, scale = mb.fit_constrained_map(train, cols, "sqrt", 8.0)
    out = df[["comid", "year", "month", "q_site"]].copy()
    out["mass_balance_predict"] = mb.predict(df, cols, beta, scale)
    out["mass_balance_fit_end_year"] = fit_end_year
    return out


def attach_mass_balance_prior(df: pd.DataFrame, fit_end_year: int) -> pd.DataFrame:
    prior = mass_balance_prior_by_fit_end(fit_end_year)
    out = df.merge(prior, on=["comid", "year", "month", "q_site"], how="left")
    fallback = out["Q_calc_cfs"].fillna(out["Q_ma_cfs"]).fillna(out["Q_obsv_cfs"]).clip(lower=EPS)
    out["mass_balance_predict"] = out["mass_balance_predict"].fillna(fallback).clip(lower=EPS)
    out["log_mass_balance_prior"] = np.log(out["mass_balance_predict"].clip(lower=EPS))
    return out
...
def fit_map_ridge(
    ...
    x_parts = [x]
    y_parts = [y]
    if conservation_weight > 0 and "log_mass_balance_prior" in train.columns:
        sqrt_w = float(np.sqrt(conservation_weight))
        x_parts.append(x * sqrt_w)
        y_parts.append(train["log_mass_balance_prior"].to_numpy(dtype=float) * sqrt_w)
    if anomaly_weight > 0:
...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    df = attach_mass_balance_prior(df, INNER_TRAIN_END_YEAR)
    ...
    rows = []
    for conservation_weight in [0.10, 0.50, 1.50, 4.00]:
        beta = fit_map_ridge(
            tr,
            stations,
            mean,
        ...
        rows.append(
            {
                **base,
                "conservation_weight": conservation_weight,
                "mass_balance_prior_fit_end_year": INNER_TRAIN_END_YEAR,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_27 production-routing Bayesian model with conservation prior",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_19 nonlinear monthly production-store plus quick/base routing Bayesian equation, then adds a routed mass-balance pseudo-observation prior inside the MAP objective.",
        "",
        ...
        "",
        "The conservation prior is generated by a constrained routed mass-balance model fitted only within the allowed calibration period. It enters as extra MAP objective rows that pull fitted logQ toward log(Q_mass), not as a post-hoc correction.",
        "",
    ...
    (RUN_DIR / "README_20260607_27.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    raw = attach_mass_balance_prior(raw, CAL_END_YEAR)
    ...
    beta = fit_map_ridge(
        ...
        conservation_weight=float(best["conservation_weight"]),
    )
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_production_conservation_prior_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_production_conservation_prior_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
