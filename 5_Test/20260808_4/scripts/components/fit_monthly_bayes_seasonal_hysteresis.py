...
def simulate_production_variant(
    ...
    month_end_quick_store_mm = np.zeros(len(out), dtype=float)
    month_end_base_store_mm = np.zeros(len(out), dtype=float)
    for _, idx in out.groupby("comid", sort=False).groups.items():
        ...
        for pos in idx:
            ...
            month_end_quick_store_mm[pos] = quick_store
            month_end_base_store_mm[pos] = base_store
    return {
        "storage_mm": storage,
        ...
        "month_end_quick_store_mm": month_end_quick_store_mm,
        "month_end_base_store_mm": month_end_base_store_mm,
...
def add_hydrologic_features(
    ...
    routed_quick_cfs = np.zeros(len(out), dtype=float)
    routed_base_cfs = np.zeros(len(out), dtype=float)
    production_month_end_quick_store_mm = np.zeros(len(out), dtype=float)
    production_month_end_base_store_mm = np.zeros(len(out), dtype=float)
    for _, idx in out.groupby("comid", sort=False).groups.items():
        soil_store = 0.50 * prod_capacity
        quick_store = 0.0
        base_store = 0.0
        for pos in idx:
            ...
            routed_quick_cfs[pos] = quick_release_mm * factor
            routed_base_cfs[pos] = base_release_mm * factor
            production_month_end_quick_store_mm[pos] = quick_store
            production_month_end_base_store_mm[pos] = base_store
    ...
    out["production_month_end_quick_store_mm"] = production_month_end_quick_store_mm
    out["production_month_end_base_store_mm"] = production_month_end_base_store_mm
...
def main() -> None:
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_seasonal_hysteresis_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
