...
def station_log_centers(train: pd.DataFrame, stations: list[str]) -> pd.Series:
    centers = train.groupby("q_site")["log_obs"].mean()
    global_center = float(train["log_obs"].mean())
    return centers.reindex(stations).fillna(global_center)
...
def build_matrix(
    ...
    center_values = df["q_site"].astype(str).map(centers).to_numpy(dtype=float)
    y = df["log_obs"].to_numpy(dtype=float) - center_values
    return x, y
                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            ...
                            centers = station_log_centers(tr, stations)
                            for fixed_sigma in [3.0]:
                                for station_sigma in [0.20, 0.60, 1.00]:
                                    for slope_sigma in [0.10, 0.15]:
                                        for regime_slope_sigma in [0.10, 0.25]:
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_22 station-normalized response Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the SAS season x flow-regime hierarchy, but changes the modeled target from absolute logQ to station-normalized logQ anomaly. The station mean logQ is computed from the fitting period only and added back for prediction.",
        "",
        ...
        "",
        "`logQ - station_calibration_mean_logQ = fixed monthly hydrologic terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The station centers are computed only from the fitting period. No 2019-2022 observed Q is used to form centers, gates, hyperparameters, or corrections. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_22.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    centers = station_log_centers(train, stations)
    beta = fit_map_ridge(
        train,
        stations,
        mean,
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_metric_summary.csv", index=False, encoding="utf-8-sig")
    centers.rename("station_calibration_mean_logQ").reset_index().rename(columns={"q_site": "q_site"}).to_csv(
        REPORT_DIR / "monthly_bayes_station_normalized_response_station_centers.csv", index=False, encoding="utf-8-sig"
    )
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_station_normalized_response_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_station_normalized_response_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
