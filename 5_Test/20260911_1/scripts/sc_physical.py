...
def independent_decode(data, artifact):
    ...
    hazard = np.where(positive, np.exp(loghazard), 0.)
...
def replay_stores(hazard, survival, fraction, release, source, demand, month_index, check=lambda: None, closure=None):
    ...
    for t in range(nd):
        ...
        if closure is not None:
            ref, wet, delta, coeff = closure
            z = (available-ref)/(available+ref)
            q = (3*z*z-1)/2
            terms = np.stack([z,q,z*delta[t],q*delta[t],z*wet[t]*delta[t],q*wet[t]*delta[t]])
            raw_hazard = raw_hazard*np.exp(np.log(4.)*np.tanh(coeff@terms/np.log(4.)))
        probability = -np.expm1(-np.minimum(raw_hazard, 700.))
...
def replay_artifact(data, artifact, check=lambda: None):
    ...
    closure = None
    if 'shared_closure' in artifact['design']:
        w = data.soil_wetness
        lagged = np.zeros_like(w)
        for lag in range(1,31):
            lagged[:lag] += w[0]
            lagged[lag:] += w[:-lag]
        closure = (np.asarray(artifact['design']['inventory_scale_kg']), w, w-lagged/30, np.asarray(artifact['theta'][-6:]))
    daily, relative = replay_stores(h, s, f, data.lower_release, data.source, data.crop, data.mid, check, closure)
