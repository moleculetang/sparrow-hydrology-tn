...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260606_12 TTD + reservoir-proxy strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_reservoir_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_reservoir_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_reservoir_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(param_rows).to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_station_random_effects.csv", index=False, encoding="utf-8-sig")
    ...
    kernel_df.to_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_kernel_weights.csv", index=False, encoding="utf-8-sig")
    fig, ax = plt.subplots(figsize=(6.2, 3.8))
    ax.bar(kernel_df["lag_month"], kernel_df["weight"], color="#477998")
    ax.set_xlabel("Lag month")
    ax.set_ylabel("Kernel weight")
    ax.set_title(f"Best TTD kernel in combined model: L={int(best['ttd_lag'])}, tau={float(best['ttd_tau']):.1f}")
    ax.grid(axis="y", alpha=0.22)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_ttd_reservoir_kernel_weights.png", dpi=300)
    plt.close(fig)

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260606_12 TTD + explicit reservoir release/attenuation proxy model",
        "",
        "This numeric generation combines two previously supported mechanisms: 20260606_7's short travel-time-distribution response for general peak/amplitude behavior, and 20260606_11's explicit reservoir storage/release/attenuation proxy split by reservoir influence order.",
        "",
        "## Equation",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_ttd_reservoir_metric_summary.csv",
        "- reports/monthly_bayes_ttd_reservoir_metrics_by_station.csv",
        "- reports/monthly_bayes_ttd_reservoir_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_ttd_reservoir_fixed_parameters.csv",
        "- reports/monthly_bayes_ttd_reservoir_station_random_effects.csv",
        "- reports/monthly_bayes_ttd_reservoir_kernel_weights.csv",
        "",
    ]
