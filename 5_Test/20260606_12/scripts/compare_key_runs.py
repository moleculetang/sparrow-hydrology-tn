...
def parameter_group_summary() -> pd.DataFrame:
    params = pd.read_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_fixed_parameters.csv")
...
def plot_comparison(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    ordered = summary.set_index("run_id").loc[["20260606_4", "20260606_7", "20260606_11", "20260606_12"]].reset_index()
    ...
    colors = ["#64748B", "#477998", "#8B5E34", "#C44E24"]
    ...
    plot_delta = delta_summary.set_index("reference").loc[["20260606_4", "20260606_7", "20260606_11"]].reset_index()
    ...
    ax.set_title("20260606_12 good-station transitions")
    ax.grid(axis="y", alpha=0.22)
...
def write_markdown(summary: pd.DataFrame, delta_summary: pd.DataFrame, param_groups: pd.DataFrame) -> None:
    best = pd.read_csv(REPORT_DIR / "monthly_bayes_ttd_reservoir_hyperparameter_grid.csv").iloc[0]
    val = summary[summary["run_id"] == "20260606_12"].iloc[0]
    lines = [
        "# 20260606_12 numeric generation",
        "",
        "## Model Tested",
        "",
        "`20260606_12` combines the two strongest previous mechanisms without using 2019-2022 observations in fitting:",
        "",
        "- General short TTD terms from `20260606_7` for travel-time weighted effective water input.",
        "- Explicit order-specific reservoir storage/release/attenuation proxy from `20260606_11`.",
        ...
        "## Interpretation",
        "",
        f"`20260606_12` has validation median_NSE_log={val['median_NSE_log']:.6f}, median_KGE={val['median_KGE']:.6f}, median_abs_PBIAS={val['median_abs_PBIAS_pct']:.6f}%, and good stations={int(val['good_validation_station_count'])}.",
        "",
        "The selected hyperparameters exactly match the previously supported short TTD and reservoir proxy settings, so the mechanism is not being rejected by inner validation. However, strict validation shows that simply stacking TTD and reservoir proxy terms is not a full promotion: NSElog improves relative to the ET baseline, but KGE and PBIAS are worse than the strongest individual modules. The most likely issue is feature competition/double smoothing between basin_net, TTD_net, reservoir_storage, and reservoir_release terms.",
        "",
        "Decision: keep as a negative/diagnostic combination experiment, not promoted as current best. The next useful generation should separate the ordinary-reach TTD response from reservoir-reach storage/release response more explicitly, or use grouped/shrunk slopes so TTD cannot overwrite the reservoir proxy where the reservoir module already explains the signal.",
        "",
        "## Main Files",
        "",
    ...
    (RUN_DIR / "README_20260606_12.md").write_text("\n".join(lines), encoding="utf-8")
...
def append_daily_log(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    line = (
        "[2026-06-06] Created and ran 20260606_12 as a combined mechanism experiment. "
        "Model change: kept the ET-aware monthly empirical-Bayes/MAP base, added 20260606_7-style short exponential TTD terms and 20260606_11-style explicit order-specific reservoir storage/release/attenuation proxy in the same parameter equation. "
        "Best hyperparameters selected without 2019-2022 observations: rho=0.70, wm=480.0, et_gamma=0.50, ttd_lag=3 months, ttd_tau=1.5 months, res_rho=0.85, release_alpha=0.35, fixed_sigma=3.0, station_sigma=0.6. "
        ...
        "Decision: diagnostic_negative_combination_not_promoted. The selected hyperparameters support both mechanisms, but strict validation suggests simple stacking creates feature competition/double smoothing; next generation should gate or group TTD away from reservoir-proxy reaches rather than stack both everywhere. "
        "Main files: 20260606_12/reports/key_run_validation_comparison.csv, ttd_reservoir_delta_summary.csv, ttd_reservoir_group_delta_summary.csv, ttd_reservoir_parameter_group_strength.csv, README_20260606_12.md.\n"
    )
...
def main() -> None:
    ...
    candidate = metrics["20260606_12"]
    failure = pd.read_csv(REPORT_DIR / "failure_taxonomy.csv")

    delta_rows = []
    group_rows = []
    for reference_id in ["20260606_4", "20260606_7", "20260606_11"]:
