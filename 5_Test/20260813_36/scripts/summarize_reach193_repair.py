...
NEW_PATH = ROOT / "outputs" / "A1" / "q72_three_fold_oof_predictions.parquet"
OLD_PATH = Path(r"E:\SPARROW\5_Test\20260813_30\outputs\P1\q72_three_fold_oof_predictions.parquet")
...
def main() -> None:
    ...
    result = []
    result += metrics(old, "parent_full_8738", "parent_P1")
    result += metrics(new, "repaired_full_8822", "reach193_A1")
    result += metrics(old_common, "common_reach_month_fold", "parent_P1")
    result += metrics(new_common, "common_reach_month_fold", "reach193_A1")
    metric_frame = pd.DataFrame(result)
    metric_frame.to_csv(ROOT / "fold_metrics.csv", index=False, encoding="utf-8-sig")

    station_rows = []
    for scenario, frame in [("parent_P1", old), ("reach193_A1", new)]:
