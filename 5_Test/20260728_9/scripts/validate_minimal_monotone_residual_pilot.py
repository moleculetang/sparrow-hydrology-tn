...
def main() -> int:
    ...
    predecessor_predictions = pd.read_csv(
        SERIES
        / "20260728_8"
        / "reports"
        / "minimal_residual_pilot"
        / "outer_oof_corrected_predictions.csv",
        encoding="utf-8-sig",
    )
    predecessor_time_stats = pd.read_csv(
        SERIES
        / "20260728_8"
        / "reports"
        / "minimal_residual_pilot"
        / "outer_fold_time_gate_statistics.csv",
        encoding="utf-8-sig",
    )

    source_hashes = {
        row["role"]: row["sha256"] for row in source_manifest["records"]
    }
    predecessor_source_hashes = {
        row["role"]: row["sha256"]
        for row in predecessor_manifest["records"]
    }
    frozen_source_hashes_exact = source_hashes == predecessor_source_hashes

    frozen_keys = ["outer_fold", "q_site", "reach_id", "year", "month"]
    frozen_columns = frozen_keys + ["Q0_pred_cfs", "station_gate"]
    frozen_current = predictions[frozen_columns].sort_values(
        frozen_keys
    ).reset_index(drop=True)
    frozen_previous = predecessor_predictions[frozen_columns].sort_values(
        frozen_keys
    ).reset_index(drop=True)
    baseline_and_station_gate_frozen = bool(
        frozen_current[frozen_keys].equals(frozen_previous[frozen_keys])
        and np.allclose(
            frozen_current[["Q0_pred_cfs", "station_gate"]].to_numpy(
                dtype=float
            ),
            frozen_previous[["Q0_pred_cfs", "station_gate"]].to_numpy(
                dtype=float
            ),
            rtol=0,
            atol=1.0e-12,
    ...
    time_sort = ["outer_fold", "q_site"]
    current_time_sorted = time_stats.sort_values(time_sort).reset_index(
        drop=True
    )
    previous_time_sorted = predecessor_time_stats.sort_values(
        time_sort
    ).reset_index(drop=True)
    time_statistics_frozen = bool(
        list(current_time_sorted.columns)
        == list(previous_time_sorted.columns)
        and current_time_sorted[time_sort].equals(
            previous_time_sorted[time_sort]
        )
        and np.allclose(
            current_time_sorted.select_dtypes(include=[np.number]).to_numpy(
                dtype=float
            ),
            previous_time_sorted.select_dtypes(include=[np.number]).to_numpy(
                dtype=float
            ),
            rtol=0,
            atol=1.0e-12,
        )
        and current_time_sorted.select_dtypes(exclude=[np.number]).equals(
            previous_time_sorted.select_dtypes(exclude=[np.number])
        )
    )
    ...
    gates = {
        ...
        "source_hashes_match_20260728_8": bool(
        ...
        "time_statistics_match_20260728_8": bool(
            time_statistics_frozen
        ),
        "fixed_sharpness_time_gate_formula_exact": bool(
            TIME_GATE_SHARPNESS == 3.0 and time_gate_exact
        ),
        "baseline_and_station_gates_match_20260728_8": bool(
            baseline_and_station_gate_frozen
        ),
        ...
        "bounded_iteration_terminal_decision_reproduced": bool(
            float(scientific["time_gate_sharpness"]) == TIME_GATE_SHARPNESS
            and bool(scientific["bounded_iteration_consumed"])
            and bool(scientific["candidate_evidence_stop"])
            == (not bool(scientific["minimal_pilot_passed"]))
            and (
                scientific["decision"]
                == "bounded_time_gate_sharpening_failed"
            )
            == (not bool(scientific["minimal_pilot_passed"]))
        ),
    ...
    payload = {
        ...
        "decision": (
            ...
            else "evidence_stop_bounded_time_gate_iteration_exhausted"
            if passed
