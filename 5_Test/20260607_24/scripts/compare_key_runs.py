...
def main() -> None:
    ...
    candidate = load_metrics("20260607_24")
    failure = pd.read_csv(REPORT_DIR / "failure_taxonomy.csv")
    delta_rows = []
    group_rows = []
    for reference_id in [run_id for run_id in RUN_FILES if run_id != "20260607_24"]:
        reference = load_metrics(reference_id)
        delta = station_delta(reference, candidate, reference_id)
        delta.to_csv(REPORT_DIR / f"network_accumulation_vs_{reference_id}_station_delta.csv", index=False, encoding="utf-8-sig")
    ...
    delta_summary.to_csv(REPORT_DIR / "network_accumulation_delta_summary.csv", index=False, encoding="utf-8-sig")
    pd.concat(group_rows, ignore_index=True).to_csv(
        REPORT_DIR / "network_accumulation_group_delta_summary.csv", index=False, encoding="utf-8-sig"
    )
