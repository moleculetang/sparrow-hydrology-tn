...
def parse_downstream_map() -> dict[int, list[tuple[int, float]]]:
    ...
    for row in topo.itertuples(index=False):
        ...
        downstream_raw = getattr(row, "downstream_reach")
        downstream: list[tuple[int, float]] = []
        if pd.notna(downstream_raw) and str(downstream_raw).strip():
            for token in str(downstream_raw).replace(";", ",").split(","):
                token = token.strip()
                if not token:
                    continue
                try:
...
def topological_order_upstream_first() -> list[int]:
    if not TOPO_PATH.exists():
        return []
    topo = pd.read_csv(TOPO_PATH, encoding="utf-8-sig")
    if "reach_id" not in topo.columns:
        return []
    if "hydseq" in topo.columns:
    ...
    return topo["reach_id"].astype(int).tolist()


def compute_network_accumulation_features(
    local: pd.DataFrame,
    value_cols: list[str],
    network_decay: float,
) -> pd.DataFrame:
    ...
    if not downstream_map or not order:
        for col in value_cols:
            local[f"network_accum_{col}"] = local[col]
            local[f"network_upstream_{col}"] = 0.0
        return local

    out_parts: list[pd.DataFrame] = []
    key_cols = ["year", "month"]
    for _, part in local.groupby(key_cols, sort=False):
        part = part.copy()
        reaches = part["comid"].astype(int).to_numpy()
        pos_by_reach = {rid: i for i, rid in enumerate(reaches)}
        local_values = part[value_cols].fillna(0).clip(lower=0).to_numpy(dtype=float)
        accum = local_values.copy()
        for rid in order:
            ...
            for downstream, frac in downstream_map.get(rid, []):
                ...
                if dst_i is not None:
                    accum[dst_i, :] += network_decay * frac * accum[src_i, :]
        for j, col in enumerate(value_cols):
            part[f"network_accum_{col}"] = accum[:, j]
            part[f"network_upstream_{col}"] = np.maximum(accum[:, j] - local_values[:, j], 0.0)
        out_parts.append(part)
    return pd.concat(out_parts, ignore_index=True)


def full_panel_local_water_features(
    rho: float,
    wm: float,
    et_gamma: float,
    sas_rho: float,
    ...
    full = all_df[[c for c in cols if c in all_df.columns]].copy()
    for col in full.columns:
        full[col] = pd.to_numeric(full[col], errors="coerce")
    full = full[full["year"].between(2006, 2022) & full["month"].between(1, 12) & full["comid"].notna()].copy()
    full["comid"] = full["comid"].astype(int)
    enriched = add_hydrologic_features(
        full,
        rho=rho,
        wm=wm,
        et_gamma=et_gamma,
    ...
    accumulated = compute_network_accumulation_features(enriched[["comid", "year", "month", *value_cols]], value_cols, network_decay)
    keep_cols = [
        "comid",
        "year",
        "month",
        *[f"network_accum_{c}" for c in value_cols],
        *[f"network_upstream_{c}" for c in value_cols],
    ]
    return accumulated[keep_cols]
...
def add_hydrologic_features(
    ...
    if add_network:
        network = full_panel_local_water_features(
            rho=rho,
            wm=wm,
            et_gamma=et_gamma,
            sas_rho=sas_rho,
        ...
        out = out.merge(network, on=["comid", "year", "month"], how="left")
        network_cols = [c for c in out.columns if c.startswith("network_")]
        out[network_cols] = out[network_cols].fillna(0.0)
    return out
...
FIXED_FEATURES = [
    ...
    "log_network_accum_local_net",
    "log_network_upstream_local_net",
    "log_network_accum_basin_net",
    "log_network_accum_quick",
    "log_network_upstream_quick",
    "log_network_accum_base",
    "log_network_accum_highflow",
    "network_upstream_net_share",
    "network_highflow_share",
    "network_base_share",
...
def feature_group(feature: str) -> str:
    if feature in NETWORK_FEATURES:
        return "topology_network_accumulation"
...
def prepare_design(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["log_network_accum_local_net"] = np.log1p(out["network_accum_local_net_cfs"].fillna(0).clip(lower=0))
    out["log_network_upstream_local_net"] = np.log1p(out["network_upstream_local_net_cfs"].fillna(0).clip(lower=0))
    out["log_network_accum_basin_net"] = np.log1p(out["network_accum_basin_net_cfs"].fillna(0).clip(lower=0))
    out["log_network_accum_quick"] = np.log1p(out["network_accum_production_quick_cfs"].fillna(0).clip(lower=0))
    out["log_network_upstream_quick"] = np.log1p(out["network_upstream_production_quick_cfs"].fillna(0).clip(lower=0))
    out["log_network_accum_base"] = np.log1p(out["network_accum_production_base_cfs"].fillna(0).clip(lower=0))
    out["log_network_accum_highflow"] = np.log1p(out["network_accum_production_highflow_mass_cfs"].fillna(0).clip(lower=0))
    out["network_upstream_net_share"] = (
        out["network_upstream_local_net_cfs"].fillna(0).clip(lower=0)
        / np.maximum(out["network_accum_local_net_cfs"].fillna(0).clip(lower=0), EPS)
    ).clip(0, 1)
    out["network_highflow_share"] = (
        out["network_accum_production_highflow_mass_cfs"].fillna(0).clip(lower=0)
        / np.maximum(out["network_accum_local_net_cfs"].fillna(0).clip(lower=0), EPS)
    ).clip(0, 5)
    out["network_base_share"] = (
        out["network_accum_production_base_cfs"].fillna(0).clip(lower=0)
        / np.maximum(out["network_accum_local_net_cfs"].fillna(0).clip(lower=0), EPS)
    ).clip(0, 5)
...
def fit_map_ridge(
    ...
    for i, feat in enumerate(FIXED_FEATURES, start=1):
        ...
        if feat in NETWORK_FEATURES:
            penalty[i] = 1.0 / max(network_sigma, EPS)
...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    for network_decay in [0.75, 0.90, 0.98]:
        ...
        for network_sigma in [0.6, 1.5, 3.0]:
            beta = fit_map_ridge(
                tr,
                stations,
                mean,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_24 topology network-accumulation Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_19 nonlinear production-store and quick/base routing mechanism, then adds an explicit topology-derived upstream accumulation layer. Local reach-month water and production-routing states are propagated downstream through topology_edges.csv before entering the Bayesian/MAP parameter equation.",
        "",
        "Base random slopes: " + ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "Production-routing features: " + ", ".join(PRODUCTION_FEATURES),
        "",
        "Topology network-accumulation features: " + ", ".join(NETWORK_FEATURES),
        "",
        ...
        "",
        "`logQ = fixed monthly hydrologic/SAS terms + nonlinear production-routing terms + topology upstream-accumulation terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The production store, saturation-excess runoff, quick routing, base routing, and upstream network accumulation states are computed from monthly PPT/AET/PET-derived effective water input, reach area, and topology only. No 2019-2022 observed Q is used to form states, gates, hyperparameters, or corrections. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_24.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    featured = prepare_design(
        add_hydrologic_features(
            ...
            network_decay=float(best["network_decay"]),
        )
    )
    ...
    beta = fit_map_ridge(
        ...
        network_sigma=float(best["network_sigma"]),
    ...
    pred_cols = [
        ...
        "routed_base_cfs",
        "production_highflow_mass_cfs",
        "network_accum_local_net_cfs",
        "network_upstream_local_net_cfs",
        "network_accum_basin_net_cfs",
        "network_accum_production_quick_cfs",
        "network_upstream_production_quick_cfs",
        "network_accum_production_base_cfs",
        "network_accum_production_highflow_mass_cfs",
        "network_upstream_net_share",
        "network_highflow_share",
        "network_base_share",
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(
                    best["production_sigma"]
                    if feat in PRODUCTION_FEATURES
                    else best["network_sigma"]
                    if feat in NETWORK_FEATURES
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_network_accumulation_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_network_accumulation_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
