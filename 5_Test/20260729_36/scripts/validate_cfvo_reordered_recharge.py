...
MODEL = RUN / "outputs" / "cfvo_reordered_recharge_reach_month.parquet"
CONTROL = RUN / "outputs" / "reach_cfvo_reordered_vertical_share.parquet"
FIXED_EXCLUSIONS = {
...
def main() -> None:
    ...
    station = pd.read_csv(REPORT / "station_current_vs_cfvo.csv")
    ...
    expected_retained = all(
        gate["checks"][key] for key in performance_keys
    )
    checks = {
        ...
        "two_scenarios": set(metrics["scenario"]) == {
            "current_combined", "cfvo_reordered"
        },
        "station_rows_97": (
        ...
        "model_rows_35880": (
            len(model) == 35880 and model["reach_id"].nunique() == 230
        ),
        ...
        "control_distribution_preserved": bool(np.array_equal(
            np.sort(control["vertical_share_current"].to_numpy(float)),
            np.sort(
                control["vertical_share_cfvo_reordered"].to_numpy(float)
            ),
        )),
        "negative_cfvo_direction": (
            control["cfvo_top_0_30_pct"].corr(
                control["vertical_share_cfvo_reordered"],
                method="spearman",
            ) < -0.999999
        ),
        "strict_closure": (
