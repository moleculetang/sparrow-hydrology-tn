...
REPORTS = RUN / "reports"
LOGS = RUN / "logs"
DAILY_LOG = ROOT / "5_Test" / "20260620.log"
        ...
        writer.writerow(
            {
                ...
                "generation_type": "strict_active_station_exclusion_rerun",
                "model_change": "rebuild_Qmain_from_source_inputs_inside_this_folder after excluding Guanliang and Pingshan(III) before station matching and model training while keeping Beiliu and Lishi(II) active; no_layered_upstream_transfer; keep_CHM_PRE_V2_1_PPT; keep_CMFD_FAO56_ET0_PET; keep_ERA5_Land_monthly_evaporation_AET_2006_2022",
...
def write_workflow_logs(step_rows: list[dict[str, object]], final: dict[str, object]) -> None:
    ...
    lines = [
        f"# {RUN.name} Strict Active-Station Exclusion Rerun Log",
        "",
        f"Updated: {datetime.now().isoformat(timespec='seconds')}",
        "",
        "## Purpose",
        "",
        "Run the pure-simulation Q_main hydrologic flow baseline in a single clean folder after excluding 官良站、平山（三）站 from the active input table before station matching and model training, while retaining 北流站 and 犁市（二）站. CHM_PRE V2.1 reach-catchment monthly rainfall, CMFD V2.0 six-variable FAO56 Penman-Monteith ET0/PET, and ERA5-Land monthly evaporation AET are retained. The case then runs base regression, mass-conserving intermediate baselines, alpha sweep, station-adaptive comparator, final reach-class light mass constraint, station-level diagnostics, and the full main-model document locally. It intentionally does not run layered upstream transfer.",
        "",
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
        "- This folder is the strict active-station exclusion rerun of the pure-simulation Q_main baseline.",
        ...
        "- 官良站、平山（三）站 do not participate in active model training in this run; 北流站 and 犁市（二）站 remain active.",
        ]
    )
    text = "\n".join(lines) + "\n"
...
def write_final_summary(final: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## Conclusion",
        "",
        "This folder is the strict active-station exclusion rerun of the clean pure-simulation Q_main baseline.",
        "",
        "It intentionally excludes layered upstream transfer and post-hoc residual correction. It also excludes 官良站、平山（三）站 from active input construction and model training because those stations are currently treated as suspected observation/representativeness problem stations, while 北流站 and 犁市（二）站 remain active.",
        "",
