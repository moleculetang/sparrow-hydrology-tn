...
EXCLUDED_ACTIVE_CALIBRATION_STATIONS = [
    "官良站",
    "平山（三）站",
]
...
def write_reports(indata: pd.DataFrame, reach: pd.DataFrame, topo: pd.DataFrame, discharge_m: pd.DataFrame, station_match: pd.DataFrame) -> None:
    ...
    summary = [
        ...
        "- 官良站、平山（三）站 are excluded from this active calibration input build because their observations or station-reach representativeness are under question. 北流站 and 犁市（二）站 remain active.",
...
- 官良站、平山（三）站 are excluded before station matching and model training as suspected observation/representativeness problem stations. 北流站 and 犁市（二）站 remain active.
...
def main() -> int:
    ...
    excluded_norms = {norm_name(name) for name in EXCLUDED_ACTIVE_CALIBRATION_STATIONS}
    ...
    pd.DataFrame(
        [
            {
                "station_name": name,
                "station_norm": norm_name(name),
                "reason": "suspected_observation_or_station_representativeness_problem; excluded before station matching and model training",
                "excluded_month_rows": int((excluded_active["station_norm"].astype(str) == norm_name(name)).sum()),
            }
            for name in EXCLUDED_ACTIVE_CALIBRATION_STATIONS
        ]
    ...
    log = [
        ...
        "官良站、平山（三）站 were excluded from the active calibration input table before station matching and model training; 北流站 and 犁市（二）站 remain active.",
