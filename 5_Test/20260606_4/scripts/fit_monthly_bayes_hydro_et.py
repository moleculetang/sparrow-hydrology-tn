...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    for rho in [0.50, 0.70, 0.85, 0.93]:
        for wm in [120.0, 240.0, 480.0]:
            for et_gamma in [0.0, 0.25, 0.50, 0.75]:
                ...
                for fixed_sigma in [1.0, 1.5, 3.0]:
                    for station_sigma in [0.30, 0.60, 1.00]:
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260606_4 ET-aware strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_et_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_et_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_et_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_et_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")

    featured = prepare_design(add_hydrologic_features(raw, rho=float(best["rho"]), wm=float(best["wm"]), et_gamma=float(best["et_gamma"])))
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_et_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_et_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_et_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_et_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_et_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(param_rows).to_csv(REPORT_DIR / "monthly_bayes_et_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_et_station_random_effects.csv", index=False, encoding="utf-8-sig")

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260606_4 ET-aware monthly hierarchical Bayesian hydrologic model",
        "",
        "This experiment extends 20260606_3 by adding explicit evapotranspiration terms from ERA5-Land monthly data under E:/SPARROW/0_reach_topology.",
        "",
        "## Equation",
        "",
        ...
        "",
        "Hydrologic terms include Q_calc, Q_ma, cumulative area, monthly basin water surplus, threshold surplus, antecedent wetness state, wetness-quickflow interaction, explicit AET/PET/ET-deficit terms, ET ratios, aridity, seasonal sine/cosine, and reservoir lag indicators.",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_et_metric_summary.csv",
        "- reports/monthly_bayes_et_metrics_by_station.csv",
        "- reports/monthly_bayes_et_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_et_fixed_parameters.csv",
        "- reports/monthly_bayes_et_station_random_effects.csv",
        "- figure/monthly_bayes_et_validation_scatter.png",
        "- figure/monthly_bayes_et_validation_metric_distributions.png",
        "- figure/monthly_bayes_et_representative_hydrographs.png",
        "",
    ]
    (RUN_DIR / "README_monthly_bayes_et_20260606_4.md").write_text("\n".join(lines), encoding="utf-8")
