...
LOCK = STAGE28 / "reports" / "full_development_parameter_lock.json"
STATE = STAGE28 / "outputs" / "full_development_model_state.parquet"
...
def main() -> None:
    ...
    if lock["status"] != "FULL_DEVELOPMENT_DYN2P_LOCKED" or lock["2019_2022_discharge_read"]:
        raise RuntimeError("Final lock is invalid or retrospective was already marked read")
    contract = {
        "stage": "20260826_29", "registered_before_retrospective_read": True,
        "model_lock": str(LOCK), "model_state": str(STATE),
        "evaluation": "locked 2019-2022 retrospective temporal check",
    ...
    decision = {
        "stage": "20260826_29",
        "status": "FINAL_DYN2P_PROMOTED_FOR_TN_HYDROLOGY" if promoted else "FINAL_DYN2P_NOT_PROMOTED_OLD_TOTAL_RETAINED",
    ...
    write_json(REPORTS / "stage29_decision.json", decision)
    write_json(REPORTS / "validation.json", {"stage": "20260826_29", "all_checks_pass": bool(spin["converged"] and checks["component_closure"] and checks["land_mass_bounded"]), "promotion_checks": checks})
    (RUN / "README.md").write_text("# 20260826_29 locked retrospective DYN2P evaluation\n", encoding="utf-8")
    (REPORTS / "technical_report.md").write_text(
        "# 20260826_29 锁后回顾\n\n"
        f"状态：`{decision['status']}`。2019–2022仅在完整开发模型锁定后读取一次。\n",
        encoding="utf-8",
    )
