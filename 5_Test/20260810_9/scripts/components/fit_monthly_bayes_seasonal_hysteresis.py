...
def simulate_production_variant(
    ...
    for _, idx in out.groupby("comid", sort=False).groups.items():
        ...
        for pos in idx:
            ...
            demand_mm = float(out.at[pos, "production_demand_mm"])
...
def add_hydrologic_features(
    ...
    # ET0 and actual ET are not interchangeable state fluxes.  Keep their
    # positive contrast as an empirical design covariate, but in the repaired
    # mode do not subtract it a second time from prescribed-AET net input or
    ...
    if FORCING_SEMANTICS_MODE == "prescribed_aet_balance":
        out["wet_input"] = (ppt - aet) / wm
        out["sas_effective_mm"] = net
        ...
        out["production_demand_mm"] = et_deficit
