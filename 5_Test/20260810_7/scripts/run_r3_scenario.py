...
SCENARIOS = {
    "R3_00": ("legacy_mixed_et", "legacy_local_scaled"),
    "R3_10": ("prescribed_aet_balance", "legacy_local_scaled"),
    "R3_01": ("legacy_mixed_et", "explicit_upstream_volume"),
    "R3_11": ("prescribed_aet_balance", "explicit_upstream_volume"),
}
...
def load_component():
    spec = importlib.util.spec_from_file_location("r3_q72_component", COMPONENT)
...
def run_fold(scenario: str, input_path: Path, output: Path, fold: dict[str, object]) -> dict[str, object]:
    semantics, accounting = SCENARIOS[scenario]
    ...
    module.FORCING_SEMANTICS_MODE = semantics
    module.MASS_ACCOUNTING_MODE = accounting
    ...
    def fixed_choice(_frame):
        row = {
            **FIXED_HYPERPARAMETERS,
            ...
            "forcing_semantics_mode": semantics,
            "mass_accounting_mode": accounting,
        }
    ...
    evaluation["forcing_semantics_mode"] = semantics
    evaluation["mass_accounting_mode"] = accounting
    ...
    return {
        **fold, "scenario_id": scenario, "evaluation_rows": len(evaluation),
        ...
        "input_sha256": sha256(input_path), "forcing_semantics_mode": semantics,
        "mass_accounting_mode": accounting,
    }
...
def main() -> None:
    ...
    parts = [pd.read_csv(output / "blocked_folds" / fold["fold_id"] / "evaluation_predictions.csv", encoding="utf-8-sig") for fold in FOLDS]
    ...
    payload = {"runtime": RUNTIME, "scenario": scenario, "input": str(input_path), "input_sha256": sha256(input_path), "folds": rows}
