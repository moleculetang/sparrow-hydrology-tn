...
def main() -> None:
    ...
    checks = {
        ...
        "at_least_270_station_period_rows": len(station) >= 270,
        "six_bfi_groups": len(groups) == 6,
        "all_subgroups_positive": bool(
            groups["spatial_bfi_delta"].gt(0).all()
        ),
        "promotion_matches_checks": (
            gate["candidate_promoted"] == all(gate["checks"].values())
        ),
