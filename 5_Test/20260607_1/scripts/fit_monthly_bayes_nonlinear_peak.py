...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    for rho in [0.70, 0.85]:
        for wm in [240.0, 480.0]:
            for et_gamma in [0.50, 0.75]:
                for peak_pet_factor in [0.40, 0.60, 0.80]:
                    for abstraction_mm in [0.0, 20.0, 50.0]:
                        ...
                        for fixed_sigma in [1.5, 3.0]:
                            for nonlinear_sigma in [0.8, 1.5, 3.0]:
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260607_1 nonlinear runoff/peak-gain strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_nonlinear_peak_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_nonlinear_peak_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_nonlinear_peak_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(best["nonlinear_sigma"]) if feat in NONLINEAR_FEATURES else float(best["fixed_sigma"]),
            }
        )
    n_fixed = 1 + len(FIXED_FEATURES)
    station_effect = pd.DataFrame({"q_site": stations, "station_random_intercept": beta[n_fixed:]})
    fixed_params = pd.DataFrame(param_rows)
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    (
        ...
        .to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_parameter_group_strength.csv", index=False, encoding="utf-8-sig")
    )
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_nonlinear_peak_station_random_effects.csv", index=False, encoding="utf-8-sig")

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260607_1 nonlinear runoff and peak-gain monthly empirical-Bayes model",
        "",
        "This experiment starts from the 20260606_4 ET-aware monthly empirical-Bayes base and adds nonlinear runoff-generation and peak-gain terms. It is the first mechanism line after stopping the TTD-reservoir combination sequence at 20260606_14.",
        "",
        "## Equation",
        "",
        ...
        "",
        "Hydrologic terms include Q_calc, Q_ma, cumulative area, monthly basin water surplus, threshold surplus, antecedent wetness state, wetness-quickflow interaction, explicit AET/PET/ET-deficit terms, ET ratios, aridity, seasonal sine/cosine, reservoir lag indicators, plus nonlinear peak-surplus, saturation-excess, rainfall-intensity, and wetness-amplified quickflow terms.",
        "",
        "The Bayesian component is empirical-Bayes/MAP partial pooling: ordinary fixed hydrologic coefficients, nonlinear peak/runoff coefficients, and station random intercepts have Gaussian priors. The nonlinear terms use their own prior strength `nonlinear_sigma`. Hyperparameters were selected using 2006-2015 training and 2016-2018 inner validation only, then refit on 2006-2018 and tested on hidden 2019-2022 observations.",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_nonlinear_peak_metric_summary.csv",
        "- reports/monthly_bayes_nonlinear_peak_metrics_by_station.csv",
        "- reports/monthly_bayes_nonlinear_peak_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_nonlinear_peak_fixed_parameters.csv",
        "- reports/monthly_bayes_nonlinear_peak_parameter_group_strength.csv",
        "- reports/monthly_bayes_nonlinear_peak_station_random_effects.csv",
        "- figure/monthly_bayes_nonlinear_peak_validation_scatter.png",
        "- figure/monthly_bayes_nonlinear_peak_validation_metric_distributions.png",
        "- figure/monthly_bayes_nonlinear_peak_representative_hydrographs.png",
        "",
    ]
