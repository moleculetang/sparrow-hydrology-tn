...
def build_s1_input(forcing: pd.DataFrame) -> None:
    ...
    catchments = gpd.read_file(CATCHMENTS)[["reach_id", "inc_km2", "tot_km2"]].copy()
    area = catchments.set_index("reach_id")
    base["IncAreaKm2"] = base["comid"].map(area["inc_km2"])
    base["CumAreaKm2"] = base["comid"].map(area["tot_km2"])
    if base[["IncAreaKm2", "CumAreaKm2"]].isna().any().any():
        raise RuntimeError("D8-supported area mapping is incomplete")
    merged = base.merge(
