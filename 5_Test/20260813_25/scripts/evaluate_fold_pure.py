...
def main() -> None:
    ...
    a0 = frames["P0"].sort_values(KEY).reset_index(drop=True)
    a1 = frames["P1"].sort_values(KEY).reset_index(drop=True)
    ...
    base_station = station[station.scenario.eq("P0")]
    ...
    checks = {
        ...
        "every_fold_log_nse_decline_le_0_003": bool((fold_pivot.P1 >= fold_pivot.P0 - .003).all()),
        ...
        "path_mean_downstream_mae_worsen_le_1pct": float(path_pivot.P1.mean()) <= float(path_pivot.P0.mean())*1.01,
    ...
    result = {
        "P0": overall0,
        "P1": overall1,
        "delta_P1_minus_P0": {key: float(overall1[key]-overall0[key]) for key in ["raw_nse", "log_nse", "pbias_pct", "rmse_cfs", "log_rmse"]},
        ...
        "path_mean_downstream_mae_change_pct": float(100*(path_pivot.P1.mean()/path_pivot.P0.mean()-1)),
        "abs_pbias_change_points": float(abs(overall1["pbias_pct"])-abs(overall0["pbias_pct"])),
        "fold_log_nse_delta": {index: float(row.P1-row.P0) for index, row in fold_pivot.iterrows()},
