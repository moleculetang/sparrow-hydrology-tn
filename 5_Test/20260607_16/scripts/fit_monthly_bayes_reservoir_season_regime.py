                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            for res_rho in [0.75, 0.85, 0.93]:
                                for release_alpha in [0.20, 0.35, 0.65]:
                                    ...
                                    for fixed_sigma in [3.0]:
                                        for reservoir_sigma in [0.6, 1.5, 3.0]:
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_12 season x flow-regime Bayesian random-slope model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the SAS young-water / old-storage release mechanism, but expands the parameter hierarchy from high/low flow regimes to season x flow regimes. It adds station-level Bayesian random slopes separately for wet-high, wet-low, dry-high, and dry-low hydrologic states.",
        "",
        ...
        "",
        "`logQ = fixed monthly hydrologic terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The flow-state gates are computed from modeled antecedent wetness and SAS young-water fraction, and the season gates are calendar wet/dry season indicators. No 2019-2022 observed Q is used to form gates. Optional calibration-period flow-contrast objective terms remain available, but the mechanism being tested is the enlarged season-regime parameter structure. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_16.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(best["reservoir_sigma"] if feat in RESERVOIR_FEATURES else best["fixed_sigma"]),
            }
        )
    n_fixed = 1 + len(FIXED_FEATURES)
    n_station = len(stations)
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_reservoir_season_regime_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_reservoir_season_regime_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
