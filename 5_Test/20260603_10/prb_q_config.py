...
def _reservoir_influence_strength_by_reach(max_order: int = 2) -> dict[int, float]:
    ...
    # Hydrologic prior: reservoir regulation should attenuate quickly with river-network order.
    # This avoids the 20260603_9 all-downstream binary influence that damaged stable trunk stations.
    order_strength = {0: 1.0, 1: 0.45, 2: 0.20}
    strength: dict[int, float] = {rid: order_strength[0] for rid in reservoir_ids}
    ...
    while frontier:
        ...
        for nxt in immediate.get(current, []):
            next_order = order + 1
            next_strength = order_strength.get(next_order, 0.0)
            if next_strength <= 0:
                continue
...
def apply_prb_q_modifications(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["wd"] = 0.0
    out["storage"] = 0.0
    reservoir_strength = _reservoir_influence_strength_by_reach(max_order=2)
    reach_id = _series(out, "comid").astype("Int64")
    out["res_decay"] = reach_id.map(reservoir_strength).fillna(0.0).astype(float)
    out = out.sort_values(["comid", "period"]).copy()
    out["res_lag"] = out.groupby("comid")["ppt"].shift(1).fillna(0.0) * out["res_decay"]
...
CONFIG: Dict[str, Any] = {
    ...
    "betailst": (
        ...
        "bppt 0.40 0:. "
        "bstorage 0.40 0:. "
        "bres_lag 0.12 0:0.35 "
        "bplaet -0.1 .:. "
        "bwd 0.0 0:0 "
        "bres_decay 0.02 0:0.12"
    ),
