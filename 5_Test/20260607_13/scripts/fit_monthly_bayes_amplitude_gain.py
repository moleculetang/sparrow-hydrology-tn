...
FIXED_FEATURES = [
    ...
    "high_threshold_excess",
    "peak_threshold_excess",
    "high_sas_young_excess",
    "wet_peak_gain",
    "dry_peak_gain",
    "wet_high_gain_interaction",
]
...
def feature_group(feature: str) -> str:
    if feature.startswith("amplitude_random_slope_") or feature in AMPLITUDE_GAIN_FEATURES:
        return "station_random_amplitude_gain"
...
def add_amplitude_gain_features(df: pd.DataFrame, reference: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    ref = reference.copy()
    needed = ["q_site", "log_basin_threshold", "log_sas_young"]
    if not set(needed).issubset(ref.columns):
        raise ValueError("reference must contain q_site, log_basin_threshold, and log_sas_young")
    thresholds = (
        ref.groupby("q_site", sort=False)
        .agg(
            threshold_q75=("log_basin_threshold", lambda x: float(np.nanquantile(x, 0.75))),
            threshold_q90=("log_basin_threshold", lambda x: float(np.nanquantile(x, 0.90))),
            sas_young_q75=("log_sas_young", lambda x: float(np.nanquantile(x, 0.75))),
        )
        .reset_index()
    )
    fallback = {
        "threshold_q75": float(np.nanquantile(ref["log_basin_threshold"], 0.75)),
        "threshold_q90": float(np.nanquantile(ref["log_basin_threshold"], 0.90)),
        "sas_young_q75": float(np.nanquantile(ref["log_sas_young"], 0.75)),
    }
    out = out.merge(thresholds, on="q_site", how="left")
    for col, value in fallback.items():
        out[col] = out[col].fillna(value)
    out["high_threshold_excess"] = (out["log_basin_threshold"] - out["threshold_q75"]).clip(lower=0.0)
    out["peak_threshold_excess"] = (out["log_basin_threshold"] - out["threshold_q90"]).clip(lower=0.0)
    out["high_sas_young_excess"] = (out["log_sas_young"] - out["sas_young_q75"]).clip(lower=0.0)
    out["wet_peak_gain"] = out["wet_high_regime_gate"] * out["peak_threshold_excess"]
    out["dry_peak_gain"] = out["dry_high_regime_gate"] * out["peak_threshold_excess"]
    out["wet_high_gain_interaction"] = out["wet_high_regime_gate"] * (
        out["high_threshold_excess"] + out["high_sas_young_excess"]
    )
    return out.drop(columns=["threshold_q75", "threshold_q90", "sas_young_q75"])
...
def build_matrix(df: pd.DataFrame, stations: list[str], mean: pd.Series, std: pd.Series) -> tuple[np.ndarray, np.ndarray]:
    ...
    for feat in AMPLITUDE_GAIN_FEATURES:
        block = np.zeros((len(df), len(stations)), dtype=float)
        block[np.arange(len(df)), site_codes] = x_fixed_df[feat].to_numpy(dtype=float)
        slope_blocks.append(block)
    for gate in REGIME_GATES:
...
def fit_map_ridge(
    ...
    n_amplitude_slope = len(AMPLITUDE_GAIN_FEATURES) * n_station
    n_regime_slope = len(REGIME_GATES) * len(REGIME_SLOPE_FEATURES) * n_station
    penalty = np.zeros(n_fixed + n_station + n_slope + n_amplitude_slope + n_regime_slope, dtype=float)
    ...
    penalty[n_fixed + n_station + n_slope : n_fixed + n_station + n_slope + n_amplitude_slope] = 1.0 / max(
        amplitude_sigma, EPS
    )
    penalty[n_fixed + n_station + n_slope + n_amplitude_slope :] = 1.0 / max(regime_slope_sigma, EPS)
                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            ...
                            threshold_ref = featured.loc[train_idx].copy()
                            featured = add_amplitude_gain_features(featured, threshold_ref)
                            ...
                            for fixed_sigma in [3.0]:
                                for station_sigma in [1.00]:
                                    for slope_sigma in [0.15]:
                                        for amplitude_sigma in [0.10, 0.25, 0.50, 1.00]:
                                            for regime_slope_sigma in [0.25]:
                                                for anomaly_weight in [0.0]:
                                                    for flow_contrast_weight in [0.0, 1.0]:
                                                        beta = fit_map_ridge(
                                                            tr,
                                                            stations,
                                                            mean,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_13 amplitude/peak-gain Bayesian random-slope model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the season x flow-regime SAS parameter hierarchy, but adds an explicit amplitude/peak-gain parameter layer. The goal is to address the remaining KGE alpha problem: predicted monthly flow variability remains too compressed at many stations.",
        "",
        ...
        "",
        "Amplitude-gain random slopes: " + ", ".join(AMPLITUDE_GAIN_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`logQ = fixed monthly hydrologic terms + station random intercept + station random slopes + amplitude-gain station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "Amplitude thresholds are fitted from training-period hydrologic drivers, not from validation-period observed discharge. No 2019-2022 observed Q is used to form gates or thresholds. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_13.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    threshold_ref = featured[featured["year"] <= CAL_END_YEAR].copy()
    featured = add_amplitude_gain_features(featured, threshold_ref)
    ...
    beta = fit_map_ridge(
        ...
        amplitude_sigma=float(best["amplitude_sigma"]),
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    amplitude_rows = []
    for feat_i, feat in enumerate(AMPLITUDE_GAIN_FEATURES):
        vals = beta[offset + feat_i * n_station : offset + (feat_i + 1) * n_station]
        station_effect[f"amplitude_random_slope_{feat}"] = vals
        for station, value in zip(stations, vals):
            amplitude_rows.append(
                {
                    "parameter": f"amplitude_random_slope_{feat}::{station}",
                    "coefficient_standardized": value,
                    "feature_group": "station_random_amplitude_gain",
                    "prior_sigma": float(best["amplitude_sigma"]),
                    "q_site": station,
                    "slope_feature": feat,
                }
            )
    offset += len(AMPLITUDE_GAIN_FEATURES) * n_station
    regime_rows = []
    ...
    amplitude_slope_params = pd.DataFrame(amplitude_rows)
    regime_slope_params = pd.DataFrame(regime_rows)
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    amplitude_slope_params.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_amplitude_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_amplitude_gain_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
            ...
            amplitude_slope_params[["parameter", "coefficient_standardized", "feature_group", "prior_sigma"]],
        ...
        REPORT_DIR / "monthly_bayes_amplitude_gain_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
