...
BASE_RANDOM_SLOPE_PRED = ROOT / "5_Test" / "20260607_5" / "reports" / "monthly_bayes_random_slope_prediction_vs_observed_2006_2022.csv"
...
def derive_calibration_ttd_gate() -> pd.DataFrame:
    base = pd.read_csv(BASE_RANDOM_SLOPE_PRED)
    cal = base[base["split"] == "calibration"].copy()
    rows = []
    for station, part in cal.groupby("q_site"):
        md = metric_dict(part["actual"].to_numpy(dtype=float), part["predict"].to_numpy(dtype=float))
        gate = (
            (md["NSE_log"] < 0.55)
            or (md["amplitude_ratio"] < 0.65)
            or (md["amplitude_ratio"] > 1.45)
            or (abs(md["peak_pct_error"]) > 60.0)
            or (abs(md["PBIAS_pct"]) > 35.0)
        )
        rows.append(
            {
                "q_site": station,
                "ttd_gate": float(gate),
                "gate_reason_low_nselog": bool(md["NSE_log"] < 0.55),
                "gate_reason_amplitude": bool((md["amplitude_ratio"] < 0.65) or (md["amplitude_ratio"] > 1.45)),
                "gate_reason_peak": bool(abs(md["peak_pct_error"]) > 60.0),
                "gate_reason_pbias": bool(abs(md["PBIAS_pct"]) > 35.0),
                "cal_base_NSE_log": md["NSE_log"],
                "cal_base_KGE": md["KGE_2012"],
                "cal_base_PBIAS_pct": md["PBIAS_pct"],
                "cal_base_amplitude_ratio": md["amplitude_ratio"],
                "cal_base_peak_pct_error": md["peak_pct_error"],
            }
        )
...
def add_hydrologic_features(df: pd.DataFrame, rho: float, wm: float, et_gamma: float, ttd_lag: int, ttd_tau: float, gate_map: dict[str, float]) -> pd.DataFrame:
    out = df.copy()
    out["ttd_gate"] = out["q_site"].map(gate_map).fillna(0.0).astype(float)
    ...
    out["ttd_net_cfs"] = out["ttd_net_cfs"] * out["ttd_gate"]
    out["ttd_threshold_cfs"] = out["ttd_threshold_cfs"] * out["ttd_gate"]
...
def feature_group(feature: str) -> str:
    if feature in RANDOM_SLOPE_FEATURES:
        return "station_random_gated_hydrologic_ttd_slope"
    if "ttd" in feature:
        return "ttd_sas_kernel"
...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    gate_df = derive_calibration_ttd_gate()
    gate_map = gate_df.set_index("q_site")["ttd_gate"].to_dict()
    rows = []
    for rho in [0.70]:
        for wm in [480.0]:
            for et_gamma in [0.50]:
                for ttd_lag in [3, 6, 12]:
                    for ttd_tau in [1.5, 3.0, 6.0]:
                        ...
                        for fixed_sigma in [3.0]:
                            for station_sigma in [1.00]:
                                for slope_sigma in [0.10, 0.15, 0.30]:
                                    beta = fit_map_ridge(
                                        tr,
                                        stations,
                                        mean,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_7 calibration-gated station-level Bayesian random-slope + TTD/SAS model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_5 parameter-level Bayesian random-slope structure and adds an explicit exponential TTD/SAS travel-time kernel only for stations whose 2006-2018 calibration-period base-model behavior shows strong amplitude, peak, PBIAS, or NSElog problems. Several gated TTD response terms also receive station-level random slopes:",
        "",
        ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`log(Q) = fixed ET/gated-TTD hydrologic effects + station random intercept + station random hydrologic/gated-TTD slope deviations + error`",
        "",
        "The random slopes and TTD coefficients are parameter-level Bayesian/MAP partial pooling. They are not post-processing corrections and are selected/fitted without 2019-2022 observations.",
        "",
    ...
    (RUN_DIR / "README_20260607_7.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    gate_df = derive_calibration_ttd_gate()
    gate_df.to_csv(REPORT_DIR / "calibration_derived_ttd_gate.csv", index=False, encoding="utf-8-sig")
    gate_map = gate_df.set_index("q_site")["ttd_gate"].to_dict()
    best, grid = choose_hyperparameters(raw)
    grid.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_metric_summary.csv", index=False, encoding="utf-8-sig")
            ...
            slope_rows.append(
                {
                    ...
                    "feature_group": "station_random_gated_hydrologic_ttd_slope",
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_gated_ttd_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
    ...
    kernel_df.to_csv(REPORT_DIR / "monthly_bayes_gated_ttd_kernel_weights.csv", index=False, encoding="utf-8-sig")
