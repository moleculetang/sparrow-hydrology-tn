...
def plot_comparison(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    ordered_ids = ["20260606_4", "20260606_7", "20260606_11", "20260607_3", "20260607_4", "20260607_5", "20260607_6", "20260607_7"]
    ...
    colors = ["#64748B", "#477998", "#8B5E34", "#B45309", "#0F766E", "#7C3AED", "#C44E24", "#2E7D32"]
    ...
    plot_delta = delta_summary.set_index("reference").loc[["20260606_4", "20260606_7", "20260606_11", "20260607_3", "20260607_4", "20260607_5", "20260607_6"]].reset_index()
    ...
    ax.set_title("20260607_7 good-station transitions")
    ax.grid(axis="y", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "gated_ttd_good_station_transitions.png", dpi=300)
    plt.close(fig)


def write_reports(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    best = pd.read_csv(REPORT_DIR / "monthly_bayes_gated_ttd_hyperparameter_grid.csv").iloc[0]
    val = summary[summary["run_id"] == "20260607_7"].iloc[0]
    param_groups = pd.read_csv(REPORT_DIR / "monthly_bayes_gated_ttd_parameter_group_strength.csv")
    lines = [
        "# 20260607_7 calibration-gated TTD/SAS Bayesian generation",
        "",
        "## Model Tested",
        "",
        "`20260607_7` keeps the 20260607_5 parameter-level Bayesian random-slope layer and adds explicit TTD/SAS only where a 2006-2018 calibration-derived gate indicates strong base-model amplitude, peak, PBIAS, or NSElog problems.",
        "",
        "Random-slope terms: `log_qcalc`, `log_basin_net`, `log_basin_threshold`, `log_ttd_net`, `antecedent_wetness`, `wet_quickflow`, `ttd_wet_interaction`, and `et_deficit_wetness`. TTD terms are zeroed for gate=0 stations.",
        "",
        ...
        "## Interpretation",
        "",
        f"`20260607_7` has validation median_NSE_raw={val['median_NSE_raw']:.6f}, median_NSE_log={val['median_NSE_log']:.6f}, median_KGE={val['median_KGE']:.6f}, median_abs_PBIAS={val['median_abs_PBIAS_pct']:.6f}%, and good stations={int(val['good_validation_station_count'])}.",
        "",
        "This is a balanced but not promoted mechanism result. The calibration-derived gate reduces the PBIAS/KGE penalty seen in full TTD and gives the best median NSEraw so far, but it still does not recover the 48 good stations of 20260607_5.",
        "",
        "Decision: keep_as_targeted_ttd_candidate_not_promoted_over_20260607_5.",
        "",
        "## Main Files",
        "",
        "- `reports/key_run_validation_comparison.csv`",
        "- `reports/gated_ttd_delta_summary.csv`",
        "- `reports/gated_ttd_group_delta_summary.csv`",
        "- `reports/monthly_bayes_gated_ttd_station_slope_parameters.csv`",
        "- `reports/monthly_bayes_gated_ttd_kernel_weights.csv`",
        "- `reports/calibration_derived_ttd_gate.csv`",
        "- `figure/random_slope_key_run_comparison.png`",
        "- `figure/gated_ttd_good_station_transitions.png`",
    ]
...
def append_daily_log(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    best = pd.read_csv(REPORT_DIR / "monthly_bayes_gated_ttd_hyperparameter_grid.csv").iloc[0]
    line = (
        "[2026-06-07] Created and ran 20260607_7 as a calibration-derived gated TTD/SAS mechanism experiment on top of the station/reach-level Bayesian random-slope base. "
        "Model change: kept the 20260607_5 parameter-level Bayesian random-slope structure, but opened explicit TTD/SAS terms only for stations whose 2006-2018 calibration-period base-model behavior showed strong low NSElog, amplitude distortion, peak error, or PBIAS. This avoids using hidden 2019-2022 validation diagnostics for gate selection. "
        ...
        "Decision: keep_as_targeted_ttd_candidate_not_promoted_over_20260607_5. This is useful but not decisive: gated TTD gives the best median NSEraw so far and slightly improves median KGE/PBIAS over 20260607_5, but good station count remains 47 versus 48 for 20260607_5 and median NSElog is essentially tied. The gate idea is better than full TTD stacking for KGE/PBIAS, but not enough to replace the random-slope base. "
        "Main files: 20260607_7/reports/key_run_validation_comparison.csv, gated_ttd_delta_summary.csv, gated_ttd_group_delta_summary.csv, calibration_derived_ttd_gate.csv, monthly_bayes_gated_ttd_station_slope_parameters.csv, README_20260607_7.md.\n"
    )
...
def main() -> None:
    ...
    candidate = metrics["20260607_7"]
    failure = pd.read_csv(REPORT_DIR / "failure_taxonomy.csv")
    delta_rows = []
    group_rows = []
    for reference_id in ["20260606_4", "20260606_7", "20260606_11", "20260607_3", "20260607_4", "20260607_5", "20260607_6"]:
        delta = station_delta(metrics[reference_id], candidate, reference_id)
        delta.to_csv(REPORT_DIR / f"gated_ttd_vs_{reference_id}_station_delta.csv", index=False, encoding="utf-8-sig")
    ...
    delta_summary.to_csv(REPORT_DIR / "gated_ttd_delta_summary.csv", index=False, encoding="utf-8-sig")
    group_summary.to_csv(REPORT_DIR / "gated_ttd_group_delta_summary.csv", index=False, encoding="utf-8-sig")
