...
def load_production_gate(mode: str) -> dict[str, float]:
    """Calibration-only gate based on whether production-routing helped over 20260607_12."""
    base_path = ROOT / "5_Test" / "20260607_12" / "reports" / "monthly_bayes_season_regime_slopes_metrics_by_station.csv"
    prod_path = ROOT / "5_Test" / "20260607_19" / "reports" / "monthly_bayes_production_routing_metrics_by_station.csv"
    if mode == "all":
        return {}
    if not base_path.exists() or not prod_path.exists():
        return {}
    cols = ["q_site", "cal_NSE_log", "cal_KGE_2012", "cal_PBIAS_pct", "cal_amplitude_ratio"]
    base = pd.read_csv(base_path)[cols]
    prod = pd.read_csv(prod_path)[cols]
    merged = prod.merge(base, on="q_site", suffixes=("_prod", "_base"))
    merged["delta_nse"] = merged["cal_NSE_log_prod"] - merged["cal_NSE_log_base"]
    merged["delta_kge"] = merged["cal_KGE_2012_prod"] - merged["cal_KGE_2012_base"]
    merged["delta_abs_pbias"] = merged["cal_PBIAS_pct_prod"].abs() - merged["cal_PBIAS_pct_base"].abs()
    merged["delta_abs_alpha"] = (merged["cal_amplitude_ratio_prod"] - 1.0).abs() - (
        merged["cal_amplitude_ratio_base"] - 1.0
    ).abs()
    benefit = (merged["delta_kge"] > 0.01) | (merged["delta_nse"] > 0.01) | (merged["delta_abs_pbias"] < -1.0)
    if mode == "benefit":
        gate = benefit
    elif mode == "benefit_alpha":
        gate = benefit & (merged["delta_abs_alpha"] < 0.0)
    else:
        raise ValueError(f"Unknown production gate mode: {mode}")
    return dict(zip(merged["q_site"].astype(str), gate.astype(float)))
...
def add_hydrologic_features(
    ...
    gate_by_station = load_production_gate(gate_mode)
    ...
    if gate_mode == "all":
        out["production_gate"] = 1.0
    else:
        out["production_gate"] = out["q_site"].astype(str).map(gate_by_station).fillna(0.0).astype(float)
    gated_cols = [
    ...
    for col in gated_cols:
        out[col] = out[col] * out["production_gate"]
                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            for prod_capacity in [240.0]:
                                for runoff_gamma in [2.5]:
                                    for quick_rho in [0.25]:
                                        for base_rho in [0.85]:
                                            for base_release in [0.10]:
                                                for gate_mode in ["benefit", "benefit_alpha"]:
                                                    ...
                                                    for fixed_sigma in [3.0]:
                                                        for production_sigma in [0.6, 1.5, 3.0]:
                                                                    ...
                                                                    for regime_slope_sigma in [0.25]:
                                                                        for anomaly_weight in [0.0]:
                                                                            for flow_contrast_weight in [1.0]:
                                                                                ...
                                                                                pred = np.exp(
                                                                                    np.clip(predict_log(iv, beta, stations, mean, std), -20, 20)
                                                                                )
                                                                                ...
                                                                                inner_score = (
                                                                                    (
                                                                                        sm["median_NSE_log"]
                                                                                        if np.isfinite(sm["median_NSE_log"])
                                                                                        else -99
                                                                                    )
                                                                                    + 1.5
                                                                                    * (
                                                                                        sm["median_KGE"]
                                                                                        if np.isfinite(sm["median_KGE"])
                                                                                        else -99
                                                                                    )
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_21 calibration-gated production-routing Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the nonlinear production-routing mechanism, but opens it only for stations where production-routing improved calibration-period behavior over 20260607_12. This tests whether production-routing gains can be targeted without globally worsening alpha/KGE.",
        "",
        ...
        "",
        "`logQ = fixed monthly hydrologic/SAS terms + nonlinear production-routing terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The production gate is computed only from 2006-2018 calibration metrics comparing 20260607_19 with 20260607_12. No 2019-2022 observed Q is used to form states, gates, hyperparameters, or corrections. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_21.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(best["production_sigma"] if feat in PRODUCTION_FEATURES else best["fixed_sigma"]),
            }
        )
    n_fixed = 1 + len(FIXED_FEATURES)
    n_station = len(stations)
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_gated_production_routing_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_gated_production_routing_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
