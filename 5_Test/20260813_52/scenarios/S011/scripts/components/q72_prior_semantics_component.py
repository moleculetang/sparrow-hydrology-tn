...
# Stage-A control.  ``0.35`` reproduces I0 exactly; ``1.0`` removes the
# undocumented empirical scale without adding or estimating a parameter.
# It changes only the forcing-only Qcalc/Qma predictor path.  The already
# mass-closed production-store process remains unchanged.
NETWORK_INPUT_SCALE = 0.35
NETWORK_INPUT_SEMANTICS = "legacy_empirical_scale"
# The repair runner keeps this False for the exact A1 reproduction control and
# enables it only for the engineering-repaired scenarios.  This makes every
# behavioral change auditable against one immutable component copy.
ENGINEERING_REPAIR_MODE = False
ZERO_PRESERVING_FULL_PRIOR_MODE = False
FULL_GAUSSIAN_PRIOR_SPACE = False
# Deterministic initial-state repair.  When enabled, each reach is initialized
# at the periodic fixed point obtained by cycling its 2006--2011 forcing.  The
# calculation uses forcing only and is identical for every outer fold.
DETERMINISTIC_SPINUP_MODE = False
SPINUP_END_YEAR = 2011
SPINUP_TOLERANCE = 1.0e-6
SPINUP_MAX_CYCLES = 1000
...
def _aggregate_variant(out: pd.DataFrame, values: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    depth_fields = {"storage_mm", "saturation", "et_withdrawn_mm", "et_unmet_mm", "routing_et_withdrawn_mm", "quick_routing_storage_mm", "base_routing_storage_mm", "mass_balance_error_mm"}
...
def _spinup_positions(out: pd.DataFrame, idx) -> list[int]:
    return [int(pos) for pos in idx if int(out.at[pos, "year"]) <= SPINUP_END_YEAR]


def _periodic_scalar(initial: float, positions: list[int], update, scale: float) -> float:
    state = float(initial)
    for _ in range(SPINUP_MAX_CYCLES):
        previous = state
        for pos in positions:
            state = float(update(state, pos))
        if abs(state - previous) / max(float(scale), EPS) < SPINUP_TOLERANCE:
            return state
    raise RuntimeError("Deterministic scalar spin-up failed to converge")


def _periodic_production_state(
    out: pd.DataFrame,
    positions: list[int],
    prod_capacity: float,
    runoff_gamma: float,
    ...
    state = np.array([0.50 * prod_capacity, 0.0, 0.0], dtype=float)
    for _ in range(SPINUP_MAX_CYCLES):
        previous = state.copy()
        soil_store, quick_store, base_store = state.tolist()
        for pos in positions:
            ...
            soil_store = max(soil_store + max(eff_mm - quick_generated_mm, 0.0), 0.0)
            ...
            soil_store = min(soil_store, prod_capacity)
            quick_input = quick_generated_mm + overflow_mm
            slow_mm = base_release * soil_store
            soil_store = max(soil_store - slow_mm, 0.0)
            _, _, quick_store = retention_step(quick_store, quick_input, quick_rho)
            _, _, base_store = retention_step(base_store, slow_mm, base_rho)
        state = np.array([soil_store, quick_store, base_store], dtype=float)
        if np.max(np.abs(state - previous)) / max(prod_capacity, EPS) < SPINUP_TOLERANCE:
            return tuple(float(x) for x in state)
    raise RuntimeError("Deterministic production spin-up failed to converge")
...
def simulate_production_variant(
    ...
    mass_balance_error_mm = np.zeros(len(out), dtype=float)
    for _, idx in out.groupby("comid", sort=False).groups.items():
        positions = [int(pos) for pos in idx]
        if DETERMINISTIC_SPINUP_MODE:
            soil_store, quick_store, base_store = _periodic_production_state(
                out,
                _spinup_positions(out, idx),
                prod_capacity,
                runoff_gamma,
                quick_rho,
                base_rho,
            ...
            base_store = 0.0
        for pos in positions:
            initial_total_store = soil_store + quick_store + base_store
            eff_mm = float(out.at[pos, "sas_effective_mm"])
            sat0 = float(np.clip(soil_store / max(prod_capacity, EPS), 0.0, 1.5))
            quick_generated_mm = min(eff_mm, eff_mm * (sat0**runoff_gamma) * highflow_scale)
            ...
            if overflow_mm > 0:
                ...
                # Overflow is a water flux, not a response-amplitude feature.
                # Scaling it created water for highflow_scale>1 and destroyed
                # water for highflow_scale<1.  The legacy expression remains
                # available only in the exact-reproduction control.
                quick_mm += overflow_mm if ENGINEERING_REPAIR_MODE else overflow_mm * highflow_scale
            pre_release_soil_store = soil_store
            slow_mm = base_release * soil_store
            soil_store = max(soil_store - slow_mm, 0.0)
            # Saturation and storage now refer to the same post-release state.
            # Preserve the historical pre-release saturation only in B0.
            sat1 = float(np.clip(
                (soil_store if ENGINEERING_REPAIR_MODE else pre_release_soil_store)
                / max(prod_capacity, EPS),
                0.0,
                1.5,
            ))
            ...
            final_total_store = soil_store + quick_store + base_store
            mass_balance_error_mm[pos] = (
                initial_total_store + eff_mm - withdrawn_mm
                - final_total_store - quick_release_mm - base_release_mm
            )
    ...
    return {
        ...
        "mass_balance_error_mm": mass_balance_error_mm,
    }
...
def add_hydrologic_features(
    ...
    for _, idx in out.groupby("comid", sort=False).groups.items():
        positions = [int(pos) for pos in idx]
        if DETERMINISTIC_SPINUP_MODE:
            spin_positions = _spinup_positions(out, idx)
            last = _periodic_scalar(
                0.0,
                spin_positions,
                lambda previous, pos: np.clip(
                    rho * previous + float(out.at[pos, "wet_input"]), -3.0, 3.0
                ),
                3.0,
            )
        else:
    ...
    # Stage A changes only a forcing-predictor scale.  The previously rejected
    # lag-wetness multiplier and all physical-sounding partition fields are
    # absent from this implementation.
    if abs(float(DYNAMIC_BETA_W)) > 1e-15:
        raise ValueError("Stage A forbids the previously falsified lag-wetness multiplier")
    input_scale = float(NETWORK_INPUT_SCALE)
    if not np.isfinite(input_scale) or input_scale <= 0:
        raise ValueError(f"NETWORK_INPUT_SCALE must be positive, got {input_scale}")
    local_net_cfs = out["local_net_cfs"].to_numpy(dtype=float)
    scaled_local_predictor = input_scale * local_net_cfs
    out["Q_calc_cfs"] = _aggregate_local_cfs(out, scaled_local_predictor)
    # Neutral, auditable aliases.  The unscaled field is a potential positive
    # forcing-equivalent volume rate, not simulated streamflow.
    out["local_positive_input_equivalent_cfs"] = local_net_cfs
    out["upstream_positive_input_equivalent_cfs"] = _aggregate_local_cfs(out, local_net_cfs)
    out["network_input_predictor_cfs"] = out["Q_calc_cfs"]
    out["network_input_scale"] = input_scale
    out["network_input_semantics"] = NETWORK_INPUT_SEMANTICS
    # Q_ma is a deployment-style, outer-fold training-forcing climatology.
    base_qcalc = input_scale * out["explicit_upstream_net_cfs"].to_numpy(dtype=float)
    out["Q_ma_cfs"], qma_threshold = fold_pure_qma_reference(out, base_qcalc, CAL_END_YEAR)
    out["network_input_climatology_cfs"] = out["Q_ma_cfs"]
    ...
    for _, idx in out.groupby("comid", sort=False).groups.items():
        ...
        if DETERMINISTIC_SPINUP_MODE:
            ...
            def sas_update(previous, pos):
                eff_mm = float(out.at[pos, "sas_effective_mm"])
                wet = float(out.at[pos, "antecedent_wetness"])
                young_frac = float(np.clip(1.0 / (1.0 + np.exp(-young_k * wet)), 0.05, 0.95))
                available = previous + (1.0 - young_frac) * eff_mm
                demand = float(out.at[pos, "aet_storage_demand_mm"])
                if et_operator == "storage_withdrawal":
                    available = max(available - min(demand, available), 0.0)
                return sas_rho * available

            last_storage = _periodic_scalar(
                0.0, spin_positions, sas_update, storage_scale
            )
        else:
            last_storage = 0.0
        for pos in positions:
    ...
    out["production_mass_balance_error_mm"] = primary["mass_balance_error_mm"]
...
_FULL_MULTISTORE_FEATURES = tuple(MULTISTORE_FEATURES)
_FULL_SPATIAL_GROUP_GATES = tuple(SPATIAL_GROUP_GATES)


def configure_engineering_repair(enabled: bool) -> None:
    """Switch between exact A1 reproduction and the repaired parameterization.

    The repaired contract removes only algebraically redundant columns.  It
    does not select columns from validation performance.
    """
    global ENGINEERING_REPAIR_MODE
    global FIXED_FEATURES, MULTISTORE_FEATURES, SPATIAL_GROUP_GATES
    ENGINEERING_REPAIR_MODE = bool(enabled)
    if not ENGINEERING_REPAIR_MODE:
        MULTISTORE_FEATURES = list(_FULL_MULTISTORE_FEATURES)
        SPATIAL_GROUP_GATES = list(_FULL_SPATIAL_GROUP_GATES)
        FIXED_FEATURES = list(_FULL_FIXED_FEATURES)
        return
    # sas_old_fraction is exactly 1-sas_young_fraction.  The two highflow
    # columns below are exact aliases of their routed_quick counterparts.
    redundant = {
        "sas_old_fraction",
        "log_ms_headwater_flash_highflow",
        "log_ms_wet_large_highflow",
    ...
    MULTISTORE_FEATURES = [
        feature for feature in _FULL_MULTISTORE_FEATURES
        if feature not in redundant
    ]
    FIXED_FEATURES = [
        feature for feature in _FULL_FIXED_FEATURES
        if feature not in redundant
    ]
    # headwater + mid + large = 1, so mid is the reference area class.
    SPATIAL_GROUP_GATES = [
        gate for gate in _FULL_SPATIAL_GROUP_GATES
        if gate != "group_mid_area"
    ]


def configure_full_gaussian_prior_space(enabled: bool) -> None:
    """Restore the original independent Gaussian coefficient space.

    This is deliberately independent from ENGINEERING_REPAIR_MODE: physical
    mass/accounting fixes remain enabled while the statistical column space is
    restored after the 20260813_43 prior-equivalence failure.
    """
    global FULL_GAUSSIAN_PRIOR_SPACE
    global FIXED_FEATURES, MULTISTORE_FEATURES, SPATIAL_GROUP_GATES
    FULL_GAUSSIAN_PRIOR_SPACE = bool(enabled)
    if FULL_GAUSSIAN_PRIOR_SPACE:
        FIXED_FEATURES = list(_FULL_FIXED_FEATURES)
        MULTISTORE_FEATURES = list(_FULL_MULTISTORE_FEATURES)
        SPATIAL_GROUP_GATES = list(_FULL_SPATIAL_GROUP_GATES)


def matrix_regime_gates() -> list[str]:
    """Return independent gates; dry-low is the repaired reference regime."""
    return list(
        REGIME_GATES
        if FULL_GAUSSIAN_PRIOR_SPACE or not ENGINEERING_REPAIR_MODE
        else REGIME_GATES[:-1]
    )


def station_deviation_count(stations: list[str]) -> int:
    """Use sum-to-zero effect coding in repaired mode."""
    return (
        len(stations)
        if FULL_GAUSSIAN_PRIOR_SPACE or not ENGINEERING_REPAIR_MODE
        else max(len(stations) - 1, 0)
    )


def station_deviation_block(
    values: np.ndarray,
    site_codes: np.ndarray,
    stations: list[str],
) -> np.ndarray:
    """Build legacy one-hot or repaired sum-to-zero station deviations."""
    n = len(stations)
    if not ENGINEERING_REPAIR_MODE or FULL_GAUSSIAN_PRIOR_SPACE:
        block = np.zeros((len(site_codes), n), dtype=float)
        block[np.arange(len(site_codes)), site_codes] = values
        return block
    if n <= 1:
        return np.zeros((len(site_codes), 0), dtype=float)
    block = np.zeros((len(site_codes), n - 1), dtype=float)
    nonreference = site_codes < (n - 1)
    block[np.arange(len(site_codes))[nonreference], site_codes[nonreference]] = values[nonreference]
    reference = site_codes == (n - 1)
    block[reference, :] = -values[reference, None]
    return block


def expand_station_deviations(values: np.ndarray, station_count: int) -> np.ndarray:
    """Expand n-1 fitted deviations to n coefficients summing to zero."""
    values = np.asarray(values, dtype=float)
    if not ENGINEERING_REPAIR_MODE or FULL_GAUSSIAN_PRIOR_SPACE:
        return values
    if station_count <= 1:
        return np.zeros(station_count, dtype=float)
    return np.concatenate([values, [-float(values.sum())]])
...
def set_et_feature_block_mode(mode: str) -> None:
    ...
    blocked = set(ET_CONTRACT_FEATURES if selected == "state_only_et" else frozenset())
    if ENGINEERING_REPAIR_MODE:
        blocked.update(
            {
                "sas_old_fraction",
                "log_ms_headwater_flash_highflow",
    ...
    permitted_fixed = set(FIXED_FEATURES) if ENGINEERING_REPAIR_MODE else set(_FULL_FIXED_FEATURES)
    FIXED_FEATURES = [
        feature for feature in _FULL_FIXED_FEATURES
        if feature in permitted_fixed and feature not in blocked
    ]
...
def build_matrix(df: pd.DataFrame, stations: list[str], mean: pd.Series, std: pd.Series) -> tuple[np.ndarray, np.ndarray]:
    ...
    z_intercept = station_deviation_block(
        np.ones(len(df), dtype=float), site_codes, stations
    )

    slope_blocks = []
    for feat in RANDOM_SLOPE_FEATURES:
        slope_blocks.append(station_deviation_block(
            x_fixed_df[feat].to_numpy(dtype=float), site_codes, stations
        ))
    for gate in matrix_regime_gates():
        if ENGINEERING_REPAIR_MODE or ZERO_PRESERVING_FULL_PRIOR_MODE:
            gate_values = df[gate].fillna(0.0).clip(0.0, 1.0).to_numpy(dtype=float)
        else:
...
def design_column_metadata(stations: list[str]) -> pd.DataFrame:
    ...
    rows.extend(
        ...
        for station in stations[:station_deviation_count(stations)]
    )
    rows.extend(
        {
...
def fit_map_ridge(
    ...
    n_station_total = len(stations)
    n_station_deviation = station_deviation_count(stations)
    n_slope = len(RANDOM_SLOPE_FEATURES) * n_station_deviation
    n_regime_slope = len(matrix_regime_gates()) * len(REGIME_SLOPE_FEATURES) * n_station_total
    penalty = np.zeros(n_fixed + n_station_deviation + n_slope + n_regime_slope, dtype=float)
    ...
    penalty[n_fixed : n_fixed + n_station_deviation] = 1.0 / max(station_sigma, EPS)
    penalty[n_fixed + n_station_deviation : n_fixed + n_station_deviation + n_slope] = 1.0 / max(slope_sigma, EPS)
    penalty[n_fixed + n_station_deviation + n_slope :] = 1.0 / max(regime_slope_sigma, EPS)
    ...
    def atomic_save_npz(path: Path, matrix: sparse.csr_matrix) -> None:
        temporary = path.with_name(path.name + ".tmp.npz")
        sparse.save_npz(temporary, matrix, compressed=True)
        # Verify the complete archive before exposing it under the contract
        # filename on the synchronized Test volume.
        verified = sparse.load_npz(temporary)
        if verified.shape != matrix.shape or verified.nnz != matrix.nnz:
            raise RuntimeError(f"Sparse matrix verification failed: {path}")
...
def main() -> None:
    ...
    pred_cols = [
        ...
        "Q_calc_cfs",
        "Q_ma_cfs",
        "local_positive_input_equivalent_cfs",
        "upstream_positive_input_equivalent_cfs",
        "network_input_predictor_cfs",
        "network_input_climatology_cfs",
        "network_input_scale",
        "network_input_semantics",
        "PPT",
        "AET",
        "PET",
        "aet_source",
    ...
    n_station_deviation = station_deviation_count(stations)
    intercept_values = expand_station_deviations(
        beta[n_fixed : n_fixed + n_station_deviation], n_station
    )
    station_effect = pd.DataFrame({"q_site": stations, "station_random_intercept": intercept_values})
    offset = n_fixed + n_station_deviation
    slope_rows = []
    for feat_i, feat in enumerate(RANDOM_SLOPE_FEATURES):
        fitted = beta[
            offset + feat_i * n_station_deviation
            : offset + (feat_i + 1) * n_station_deviation
        ]
        vals = expand_station_deviations(fitted, n_station)
    ...
    offset += len(RANDOM_SLOPE_FEATURES) * n_station_deviation
    regime_rows = []
    for gate_i, gate in enumerate(matrix_regime_gates()):
