...
RUN_FILES = {
    ...
    "20260607_9": {"summary": "monthly_bayes_sas_kge_objective_metric_summary.csv", "metrics": "monthly_bayes_sas_kge_objective_metrics_by_station.csv"},
}
...
def plot_comparison(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    ordered_ids = ["20260606_4", "20260606_7", "20260606_11", "20260607_3", "20260607_4", "20260607_5", "20260607_6", "20260607_7", "20260607_8", "20260607_9"]
    ...
    colors = ["#64748B", "#477998", "#8B5E34", "#B45309", "#0F766E", "#7C3AED", "#C44E24", "#2E7D32", "#A21CAF", "#0E7490"]
    ...
    plot_delta = delta_summary.set_index("reference").loc[["20260606_4", "20260606_7", "20260606_11", "20260607_3", "20260607_4", "20260607_5", "20260607_6", "20260607_7", "20260607_8"]].reset_index()
    ...
    ax.set_title("20260607_9 good-station transitions")
    ax.grid(axis="y", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "sas_kge_objective_good_station_transitions.png", dpi=300)
    plt.close(fig)


def write_reports(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    best = pd.read_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_hyperparameter_grid.csv").iloc[0]
    val = summary[summary["run_id"] == "20260607_9"].iloc[0]
    param_groups = pd.read_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_parameter_group_strength.csv")
    lines = [
        "# 20260607_9 SAS + KGE-oriented anomaly objective Bayesian generation",
        "",
        "## Model Tested",
        "",
        "`20260607_9` keeps the 20260607_8 SAS young/old storage mechanism and changes the MAP objective by adding station-centered standardized log-flow anomaly constraints.",
        "",
        "The goal is to preserve the SAS good-station gain while forcing stronger station-internal variability/correlation behavior. This is a training-objective change, not prediction post-processing.",
        "",
        ...
        "## Interpretation",
        "",
        f"`20260607_9` has validation median_NSE_raw={val['median_NSE_raw']:.6f}, median_NSE_log={val['median_NSE_log']:.6f}, median_KGE={val['median_KGE']:.6f}, median_abs_PBIAS={val['median_abs_PBIAS_pct']:.6f}%, and good stations={int(val['good_validation_station_count'])}.",
        "",
        "This is a strong good-station result but still not a KGE recovery. The anomaly objective raises good stations and NSElog, but median KGE remains well below 20260607_5.",
        "",
        "Decision: promote_as_best_good_station_candidate_but_kge_recovery_failed.",
        "",
        "## Main Files",
        "",
        "- `reports/key_run_validation_comparison.csv`",
        "- `reports/sas_kge_objective_delta_summary.csv`",
        "- `reports/sas_kge_objective_group_delta_summary.csv`",
        "- `reports/monthly_bayes_sas_kge_objective_station_slope_parameters.csv`",
        "- `figure/random_slope_key_run_comparison.png`",
        "- `figure/sas_kge_objective_good_station_transitions.png`",
    ]
    (RUN_DIR / "README_20260607_9.md").write_text("\n".join(lines), encoding="utf-8")
...
def append_daily_log(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    vs8 = lookup.loc["20260607_8"]
    best = pd.read_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_hyperparameter_grid.csv").iloc[0]
    line = (
        "[2026-06-07] Created and ran 20260607_9 as a SAS young/old storage plus KGE-oriented multi-objective MAP experiment. "
        "Model change: kept the 20260607_8 SAS young-water / old-storage release mechanism, but changed the training objective from logQ-only MAP to logQ plus station-centered standardized log-flow anomaly residuals, intended to improve within-station variability/correlation and recover KGE without post-processing predictions. "
        "Best hyperparameters selected without 2019-2022 observations: "
        f"rho={best['rho']:.2f}, wm={best['wm']:.1f}, et_gamma={best['et_gamma']:.2f}, sas_rho={best['sas_rho']:.2f}, young_k={best['young_k']:.1f}, storage_scale={best['storage_scale']:.1f}, fixed_sigma={best['fixed_sigma']:.1f}, station_sigma={best['station_sigma']:.1f}, slope_sigma={best['slope_sigma']:.2f}, anomaly_weight={best['anomaly_weight']:.2f}. "
        ...
        f"Against 20260607_8: NSE_raw improved at {int(vs8['NSE_raw_improved_count'])}/104, NSE_log improved at {int(vs8['NSE_log_improved_count'])}/104, KGE improved at {int(vs8['KGE_improved_count'])}/104, abs(PBIAS) improved at {int(vs8['abs_PBIAS_improved_count'])}/104, good +{int(vs8['new_good_count'])}-{int(vs8['lost_good_count'])}. "
        "Decision: promote_as_best_good_station_candidate_but_kge_recovery_failed. This is a substantial positive good-station result because good stations rise to 53 and median NSElog improves, but the intended KGE recovery fails: median KGE remains near 0.639 and below the balanced 20260607_5 model. The anomaly objective is useful for threshold/good-station behavior but not sufficient for KGE restoration. "
        "Main files: 20260607_9/reports/key_run_validation_comparison.csv, sas_kge_objective_delta_summary.csv, sas_kge_objective_group_delta_summary.csv, monthly_bayes_sas_kge_objective_station_slope_parameters.csv, README_20260607_9.md.\n"
    )
...
def main() -> None:
    ...
    candidate = metrics["20260607_9"]
    failure = pd.read_csv(REPORT_DIR / "failure_taxonomy.csv")
    delta_rows = []
    group_rows = []
    for reference_id in ["20260606_4", "20260606_7", "20260606_11", "20260607_3", "20260607_4", "20260607_5", "20260607_6", "20260607_7", "20260607_8"]:
        delta = station_delta(metrics[reference_id], candidate, reference_id)
        delta.to_csv(REPORT_DIR / f"sas_kge_objective_vs_{reference_id}_station_delta.csv", index=False, encoding="utf-8-sig")
    ...
    delta_summary.to_csv(REPORT_DIR / "sas_kge_objective_delta_summary.csv", index=False, encoding="utf-8-sig")
    group_summary.to_csv(REPORT_DIR / "sas_kge_objective_group_delta_summary.csv", index=False, encoding="utf-8-sig")
