                            ...
                            for fixed_sigma in [3.0]:
                                for station_sigma in [1.00]:
                                    for slope_sigma in [0.10, 0.15]:
                                        for anomaly_weight in [0.25, 0.50, 1.00, 2.00]:
                                            beta = fit_map_ridge(
                                                tr,
                                                stations,
                                                mean,
                                            ...
                                            rows.append(
                                                {
                                                    ...
                                                    + 1.5 * (md["KGE_2012"] if np.isfinite(md["KGE_2012"]) else -99)
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_9 SAS young/old storage + KGE-oriented multi-objective MAP model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_8 SAS young-water / old-storage release mechanism and changes the MAP objective. In addition to logQ fitting, it adds station-centered standardized anomaly constraints to better preserve within-station variability and correlation:",
        "",
        ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`objective = logQ residuals + anomaly_weight * station-centered standardized logQ anomaly residuals + Gaussian priors`",
        "",
        "This is still parameter-level Bayesian/MAP fitting, not prediction post-processing. The anomaly constraints use only fitting-period observations and are selected without 2019-2022 observations.",
        "",
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_sas_kge_objective_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_sas_kge_objective_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
