...
def feature_group(feature: str) -> str:
    if feature in RANDOM_SLOPE_FEATURES:
        return "station_random_hydrologic_ttd_slope"
    if "ttd" in feature:
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_6 station-level Bayesian random-slope + TTD/SAS model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_5 parameter-level Bayesian random-slope structure and adds an explicit exponential TTD/SAS travel-time kernel. Several TTD response terms also receive station-level random slopes:",
        "",
        ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`log(Q) = fixed ET/TTD hydrologic effects + station random intercept + station random hydrologic/TTD slope deviations + error`",
        "",
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_random_slope_ttd_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
    ...
    kernel_df.to_csv(REPORT_DIR / "monthly_bayes_random_slope_ttd_kernel_weights.csv", index=False, encoding="utf-8-sig")
