...
REFERENCE_I0 = ROOT / "inputs" / "reference" / "A1_q72_three_fold_oof_predictions.parquet"
FOLDS = [
...
SCENARIOS = {
    "B0": {"scale": 1.0, "semantics": "exact_20260812_5_A1_control", "repair": False},
    "B1": {"scale": 1.0, "semantics": "engineering_repaired_q72", "repair": True},
}
...
def exact_a0_gate() -> dict[str, object]:
    a0 = pd.read_parquet(ROOT / "outputs" / "B0" / "q72_three_fold_oof_predictions.parquet")
    ...
    (ROOT / "logs" / "B0_exact_reproduction_gate.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    if not result["pass"]:
        raise RuntimeError(f"B0 exact reproduction failed: {result}")
    return result


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("scenario", choices=["B0", "B1", "all"])
    args = parser.parse_args()
    payload: dict[str, object] = {}
    if args.scenario in {"B0", "all"}:
        payload["B0"] = assemble("B0")
        payload["B0_reproduction"] = exact_a0_gate()
    if args.scenario in {"B1", "all"}:
        if not (ROOT / "logs" / "B0_exact_reproduction_gate.json").exists():
            raise RuntimeError("B1 is blocked until B0 exact reproduction has passed")
        gate = json.loads((ROOT / "logs" / "B0_exact_reproduction_gate.json").read_text(encoding="utf-8"))
        if not gate.get("pass"):
            raise RuntimeError("B1 is blocked by failed B0 reproduction")
        payload["B1"] = assemble("B1")
