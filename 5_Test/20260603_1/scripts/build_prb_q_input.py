...
RUN_DIR = ROOT / "5_Test" / "20260603_1"
...
def build_discharge_quarterly(files: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped.to_csv(RUN_DIR / "reports" / "discharge_coverage_by_station_quarter.csv", index=False, encoding="utf-8-sig")
    return grouped[grouped["Q_obsv_cfs"].notna()].copy()
    ...
    for _, station in matched.iterrows():
        ...
        for _, cat in catchments.iterrows():
            if point_in_polygon(x, y, cat["geometry_obj"]):
                found = (int(cat["reach_id"]), "within_catchment", 0.0)
                break
        if found is None:
            best_reach = None
            best_dist = float("inf")
            for _, cat in catchments.iterrows():
                dist = point_polygon_distance(x, y, cat["geometry_obj"])
                if dist < best_dist:
                    best_dist = dist
                    best_reach = int(cat["reach_id"])
            method = "nearest_catchment" if best_dist <= MAX_SNAP_DISTANCE_M else "too_far"
            found = (best_reach, method, best_dist)
        rec = station.to_dict()
...
def era5_climate_quarterly() -> pd.DataFrame:
    ...
    for year in YEARS:
        ...
        if not nc_path.exists():
            raise RuntimeError(f"Missing ERA5-Land file: {nc_path}")
    ...
    quarterly["climate_source"] = "ERA5-Land monthly tp/e/t2m aggregated in 20260531_1"
...
def build_panel(topo: pd.DataFrame, discharge_q: pd.DataFrame, station_match: pd.DataFrame) -> pd.DataFrame:
    ...
    station_strength = obs.groupby("station_norm")["Q_obsv_cfs"].size().rename("station_total_usable_quarters")
    obs = obs.merge(station_strength, on="station_norm", how="left")
    obs = obs.sort_values(
        ["reach_id", "year", "quarter", "station_total_usable_quarters", "station_norm"],
        ascending=[True, True, True, False, True],
    )
    obs_chosen = obs.groupby(["reach_id", "year", "quarter"], as_index=False).head(1).copy()
    obs_conflicts = obs.merge(
        obs_chosen[["reach_id", "year", "quarter", "station_norm"]].rename(columns={"station_norm": "chosen_station_norm"}),
        on=["reach_id", "year", "quarter"],
        how="left",
    )
    obs_conflicts = obs_conflicts[obs_conflicts["station_norm"] != obs_conflicts["chosen_station_norm"]]
    obs_conflicts.to_csv(RUN_DIR / "reports" / "multiple_station_same_reach_quarter.csv", index=False, encoding="utf-8-sig")
    station_counts_by_reach = station_match.groupby("reach_id")["station_norm"].nunique()
    conflict_reaches = set(station_counts_by_reach[station_counts_by_reach > 1].index)
    conflict_station_norms = set(station_match.loc[station_match["reach_id"].isin(conflict_reaches), "station_norm"])
    dropped_same_reach = obs_chosen[obs_chosen["station_norm"].isin(conflict_station_norms)].copy()
    dropped_same_reach.to_csv(
        RUN_DIR / "reports" / "dropped_same_reach_conflict_observations.csv",
        index=False,
        encoding="utf-8-sig",
    )
    obs_chosen = obs_chosen[~obs_chosen["station_norm"].isin(conflict_station_norms)].copy()
    panel = panel.merge(
        obs_chosen[
            [
                "reach_id",
    ...
    # Climate-water baseline flow for mean-flow fields, used by reports and SPARROW labels.
...
def write_reports(indata: pd.DataFrame, reach: pd.DataFrame, topo: pd.DataFrame, discharge_q: pd.DataFrame, station_match: pd.DataFrame) -> None:
    ...
    required_mapping = pd.DataFrame(
        [
            ...
            ("Q_obsv_cfs", "quarter mean discharge, m3/s converted to cfs", "1_Inputs/DischargeData/complete_2010_2022"),
            ("PPT", "ERA5-Land monthly total precipitation aggregated to reach-quarter", "0_reach_topology/data/raw/era5_land/monthly_prb_buffer"),
            ("AET", "absolute ERA5-Land total evaporation aggregated to reach-quarter", "0_reach_topology/data/raw/era5_land/monthly_prb_buffer"),
            ("PET", "Thornthwaite PET from ERA5-Land 2m temperature aggregated to reach-quarter", "0_reach_topology/data/raw/era5_land/monthly_prb_buffer"),
        ],
    ...
    summary = [
        "# 20260603_1 PRB Q input build summary",
        "",
        "## Source boundary",
        "- Topology and catchments came from `E:/SPARROW/0_reach_topology/results`.",
        "- Discharge observations came from `E:/SPARROW/1_Inputs/DischargeData/complete_2010_2022`.",
        "- New files were written only under `E:/SPARROW/5_Test/20260603_1`.",
        "",
        "## Counts",
        f"- reaches: {len(reach)}",
        ...
        "- same-reach multi-station conflict observations were removed before the native run.",
        "",
        "## Climate inputs",
        "- PPT/AET/PET are 2010-2022 reach-quarter values derived from downloaded ERA5-Land monthly files under `0_reach_topology/data/raw/era5_land/monthly_prb_buffer`.",
...
def write_readme() -> None:
    text = """# 20260603_1 PRB Q input build
...
conda --no-plugins run -n sparrow python E:/SPARROW/run_sparrow.py --config E:/SPARROW/5_Test/20260603_1/prb_q_config.py
```
"""
...
def main() -> int:
    ...
    log = [
        "# 20260603_1 run log",
        "",
        "Built PRB Q input table from existing topology results and complete discharge data in 1_Inputs/DischargeData/complete_2010_2022.",
        f"Rows: {len(indata)}",
