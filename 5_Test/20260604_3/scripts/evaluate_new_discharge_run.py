...
LOG_PATH = ROOT / "5_Test" / "20260604.log"
EPS = 1.0e-6
...
def main() -> None:
    ...
    lines = [
        "# 20260604_3 empirical-Bayes weighted strict geographic Q run diagnostics",
        "",
        "This run keeps the strict geographic station-reach correction and empirical-Bayes station likelihood weights from the 20260604 workflow, but excludes 坪岭站/坪岭河 from active discharge inputs.",
        "`res_lag_self/res_decay_self` apply only to reach-attribute reservoir reaches where `src_id` contains `水库`; `res_lag_down/res_decay_down` apply only to first and second downstream reaches.",
        "Station-reach matching uses nearest reach-line geometry first; same-reach conflicts use the topology-plausible significant-station rule; station likelihood weights are built from 20260604_1 training-period residual precision, excluding validation periods. 龙州（二）站 is retained without a cross-border boundary-flow correction.",
        ...
        "- `reports/same_reach_significant_station_audit.csv`",
        "- `reports/large_reservoir_station_context.csv`",
        "- `reports/validation_resids_native_run.csv`",
    ]
    (RUN_DIR / "README.md").write_text("\n".join(lines), encoding="utf-8")
    with LOG_PATH.open("a", encoding="utf-8") as fh:
        fh.write(
            "\n[2026-06-04] 20260604_3 reran the empirical-Bayes strict geographic Q model after excluding 坪岭站/坪岭河 from active inputs; 龙州（二）站 retained without cross-border boundary-flow correction, "
