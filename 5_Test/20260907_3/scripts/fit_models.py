...
from tn_autograd import LocalN, RiverN, monthly_sum, observation_operator,boundary_mass
...
class FitObjective:
    def __init__(self,data,train,model,loss,prior_scale=1.):
        ...
        variance=stats.variance.where(stats.n.ge(2),floor).clip(lower=floor)
        self.station_variance_audit=stats.assign(effective_variance=variance,floor=floor)
        ...
        self.water_official_month=data.monthly_sum(water['official'])
        self.water_release_month=data.monthly_sum(water['releases'])
    ...
    def observations_cache(self,obs):
        ...
        types=obs.station_type.to_numpy() if 'station_type' in obs else np.full(len(obs),'ordinary_internal')
        f=np.where(types=='predam',1.,f)
        codes=np.where(types=='dam_outlet',1,np.where(types=='postdam_mixed',2,0))
        ridx=obs.reservoir_index.to_numpy(int) if 'reservoir_index' in obs else np.full(len(obs),-1)
        if np.any((codes==1)&(ridx<0)):raise ValueError('Unregistered reservoir release boundary')
        ridx=np.maximum(ridx,0)
        w=self.water_inlet_month[ti,ri]+f*self.water_local_month[ti,ri]
        w=np.where(codes==1,self.water_release_month[ti,ridx],np.where(codes==2,self.water_official_month[ti,ri],w))
        ...
        baseline=np.where(codes[None,:]==1,self.water_release_month[train_month][:,ridx],np.where(codes[None,:]==2,self.water_official_month[train_month][:,ri],baseline))
        ...
        result={'ti':torch.tensor(ti),'ri':torch.tensor(ri),'f':torch.tensor(f),'water':torch.tensor(w),
                'boundary_code':torch.tensor(codes),'reservoir_index':torch.tensor(ridx),
    ...
    def predict(self,theta,obs):
        ...
        inlet,official,releases=RiverN.apply(local,v['v_f'],self.data,'monthly',True)
        ml=monthly_sum(local,self.data)
        c=self.observations_cache(obs);ti,ri,f,h=c['ti'],c['ri'],c['f'],c['h']
        load=boundary_mass(inlet,official,releases,ml,v['v_f'],c)
