...
def add_hydrologic_features(
    ...
    wm: float,
    et_gamma: float,
    peak_pet_factor: float,
    abstraction_mm: float,
    rho_quick: float,
    rho_base: float,
    rho_gw: float,
) -> pd.DataFrame:
    ...
    peak_surplus = np.maximum(ppt - peak_pet_factor * pet - abstraction_mm, 0.0)
    saturation_excess = np.maximum(ppt - aet - et_gamma * et_deficit - abstraction_mm, 0.0)
    rain_intensity = ppt / np.maximum(days.astype(float), 1.0)
    ...
    out["basin_peak_surplus_cfs"] = peak_surplus / 1000.0 * cumarea * 1_000_000.0 / seconds * 35.3146667
    out["local_peak_surplus_cfs"] = peak_surplus / 1000.0 * incarea * 1_000_000.0 / seconds * 35.3146667
    out["basin_saturation_excess_cfs"] = saturation_excess / 1000.0 * cumarea * 1_000_000.0 / seconds * 35.3146667
    out["rain_intensity_mm_day"] = rain_intensity
    out["peak_surplus_mm"] = peak_surplus
    out["saturation_excess_mm"] = saturation_excess
    ...
    quick_store = np.zeros(len(out), dtype=float)
    base_store = np.zeros(len(out), dtype=float)
    gw_store = np.zeros(len(out), dtype=float)
    for _, idx in out.groupby("comid", sort=False).groups.items():
        ...
        for pos in idx:
            ...
            state[pos] = last
            quick_input = float(out.at[pos, "basin_peak_surplus_cfs"])
            base_input = float(out.at[pos, "basin_net_cfs"])
            gw_input = max(float(out.at[pos, "basin_net_cfs"]) - float(out.at[pos, "basin_peak_surplus_cfs"]), 0.0)
            q_last = rho_quick * q_last + quick_input
            b_last = rho_base * b_last + base_input
            g_last = rho_gw * g_last + gw_input
            quick_store[pos] = min(max(q_last, 0.0), 1.0e8)
            base_store[pos] = min(max(b_last, 0.0), 1.0e8)
            gw_store[pos] = min(max(g_last, 0.0), 1.0e8)
    out["antecedent_wetness"] = state
    out["quickflow_store_cfs"] = quick_store
    out["baseflow_store_cfs"] = base_store
    out["groundwater_store_cfs"] = gw_store
    out["quickflow_release_cfs"] = (1.0 - rho_quick) * out["quickflow_store_cfs"]
    out["baseflow_release_cfs"] = (1.0 - rho_base) * out["baseflow_store_cfs"]
    out["groundwater_release_cfs"] = (1.0 - rho_gw) * out["groundwater_store_cfs"]
    total_release = out["quickflow_release_cfs"] + out["baseflow_release_cfs"] + out["groundwater_release_cfs"]
    out["quickflow_fraction"] = out["quickflow_release_cfs"] / np.maximum(total_release, 1.0)
    out["baseflow_fraction"] = (out["baseflow_release_cfs"] + out["groundwater_release_cfs"]) / np.maximum(total_release, 1.0)
    ...
    pos_wet = np.maximum(state, 0.0)
    neg_wet = np.maximum(-state, 0.0)
    log_peak = np.log1p(out["basin_peak_surplus_cfs"].clip(lower=0.0))
    log_sat = np.log1p(out["basin_saturation_excess_cfs"].clip(lower=0.0))
    log_local_peak = np.log1p(out["local_peak_surplus_cfs"].clip(lower=0.0))
    out["peak_quickflow_power"] = log_peak**2
    out["saturation_excess_power"] = log_sat**2
    out["wet_peak_gain"] = log_peak * pos_wet
    out["wet_saturation_gain"] = log_sat * pos_wet
    out["dry_infiltration_loss"] = log_peak * neg_wet
    out["local_peak_contrast"] = log_local_peak - log_peak
    out["rain_intensity_log"] = np.log1p(out["rain_intensity_mm_day"].clip(lower=0.0))
    out["rain_intensity_power"] = out["rain_intensity_log"] ** 2
    out["intensity_wet_gain"] = out["rain_intensity_log"] * pos_wet
    out["quick_base_balance"] = np.log1p(out["quickflow_release_cfs"].clip(lower=0.0)) - np.log1p(
        (out["baseflow_release_cfs"] + out["groundwater_release_cfs"]).clip(lower=0.0)
    )
    out["baseflow_wetness_buffer"] = np.log1p((out["baseflow_release_cfs"] + out["groundwater_release_cfs"]).clip(lower=0.0)) * neg_wet
    out["quickflow_wet_gain"] = np.log1p(out["quickflow_release_cfs"].clip(lower=0.0)) * pos_wet
...
FIXED_FEATURES = [
    ...
    "antecedent_wetness",
    "wet_quickflow",
    "log_basin_peak_surplus",
    "log_local_peak_surplus",
    "log_basin_saturation_excess",
    "peak_quickflow_power",
    "saturation_excess_power",
    "wet_peak_gain",
    "wet_saturation_gain",
    "dry_infiltration_loss",
    "local_peak_contrast",
    "rain_intensity_log",
    "rain_intensity_power",
    "intensity_wet_gain",
    "log_quickflow_release",
    "log_baseflow_release",
    "log_groundwater_release",
    "quickflow_fraction",
    "baseflow_fraction",
    "quick_base_balance",
    "baseflow_wetness_buffer",
    "quickflow_wet_gain",
    "aridity",
    "log_aet",
    "log_pet",
...
def feature_group(feature: str) -> str:
    if feature in COMPONENT_FEATURES:
        return "quick_baseflow_component"
    if feature in NONLINEAR_FEATURES:
        return "nonlinear_peak_runoff"
...
def prepare_design(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["log_basin_peak_surplus"] = np.log1p(out["basin_peak_surplus_cfs"].fillna(0).clip(lower=0))
    out["log_local_peak_surplus"] = np.log1p(out["local_peak_surplus_cfs"].fillna(0).clip(lower=0))
    out["log_basin_saturation_excess"] = np.log1p(out["basin_saturation_excess_cfs"].fillna(0).clip(lower=0))
    out["log_quickflow_release"] = np.log1p(out["quickflow_release_cfs"].fillna(0).clip(lower=0))
    out["log_baseflow_release"] = np.log1p(out["baseflow_release_cfs"].fillna(0).clip(lower=0))
    out["log_groundwater_release"] = np.log1p(out["groundwater_release_cfs"].fillna(0).clip(lower=0))
...
def fit_map_ridge(
    ...
    for i, feat in enumerate(FIXED_FEATURES, start=1):
        if feat in NONLINEAR_FEATURES:
            penalty[i] = 1.0 / max(nonlinear_sigma, EPS)
        if feat in COMPONENT_FEATURES:
            penalty[i] = 1.0 / max(component_sigma, EPS)
...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    for rho in [0.70, 0.85]:
        for wm in [480.0]:
            for et_gamma in [0.50, 0.75]:
                for peak_pet_factor in [0.40]:
                    for abstraction_mm in [0.0, 20.0]:
                        for rho_quick in [0.15, 0.35]:
                            for rho_base in [0.70, 0.85]:
                                for rho_gw in [0.93, 0.98]:
                                    featured = prepare_design(
                                        add_hydrologic_features(
                                            ...
                                            wm=wm,
                                            et_gamma=et_gamma,
                                            peak_pet_factor=peak_pet_factor,
                                            abstraction_mm=abstraction_mm,
                                            rho_quick=rho_quick,
                                            rho_base=rho_base,
                                            rho_gw=rho_gw,
                                        )
                                    ...
                                    for fixed_sigma in [3.0]:
                                        for nonlinear_sigma in [0.8, 1.5]:
                                            for component_sigma in [1.5, 3.0]:
                                                for station_sigma in [0.60, 1.00]:
                                                    beta = fit_map_ridge(
                                                        ...
                                                        nonlinear_sigma=nonlinear_sigma,
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260607_2 quickflow/baseflow two-component strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_two_component_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_two_component_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_two_component_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_two_component_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    featured = prepare_design(
        add_hydrologic_features(
            ...
            peak_pet_factor=float(best["peak_pet_factor"]),
            abstraction_mm=float(best["abstraction_mm"]),
            rho_quick=float(best["rho_quick"]),
            rho_base=float(best["rho_base"]),
            rho_gw=float(best["rho_gw"]),
        )
    )
    ...
    beta = fit_map_ridge(
        ...
        nonlinear_sigma=float(best["nonlinear_sigma"]),
        component_sigma=float(best["component_sigma"]),
    ...
    pred_cols = [
        ...
        "antecedent_wetness",
        "peak_surplus_mm",
        "saturation_excess_mm",
        "rain_intensity_mm_day",
        "basin_peak_surplus_cfs",
        "local_peak_surplus_cfs",
        "basin_saturation_excess_cfs",
        "peak_quickflow_power",
        "saturation_excess_power",
        "wet_peak_gain",
        "wet_saturation_gain",
        ...
        "quickflow_store_cfs",
        "baseflow_store_cfs",
        "groundwater_store_cfs",
        "quickflow_release_cfs",
        "baseflow_release_cfs",
        "groundwater_release_cfs",
        "quickflow_fraction",
        "baseflow_fraction",
        "quick_base_balance",
        "baseflow_wetness_buffer",
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_two_component_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_two_component_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_two_component_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_two_component_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_two_component_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(best["nonlinear_sigma"])
                if feat in NONLINEAR_FEATURES
                else float(best["component_sigma"])
                if feat in COMPONENT_FEATURES
                else float(best["fixed_sigma"]),
            }
        )
    n_fixed = 1 + len(FIXED_FEATURES)
    station_effect = pd.DataFrame({"q_site": stations, "station_random_intercept": beta[n_fixed:]})
    fixed_params = pd.DataFrame(param_rows)
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_two_component_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    (
        ...
        .to_csv(REPORT_DIR / "monthly_bayes_two_component_parameter_group_strength.csv", index=False, encoding="utf-8-sig")
    )
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_two_component_station_random_effects.csv", index=False, encoding="utf-8-sig")

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260607_2 quickflow/baseflow two-component monthly empirical-Bayes model",
        "",
        "This experiment starts from the 20260607_1 nonlinear peak-gain result and adds explicit quickflow, baseflow, and groundwater stores so high-flow gain and low/mid-flow conservation can be represented separately.",
        "",
        "## Equation",
        "",
        ...
        "",
        "Hydrologic terms include Q_calc, Q_ma, cumulative area, monthly basin water surplus, threshold surplus, antecedent wetness state, ET terms, reservoir lag indicators, nonlinear peak-surplus terms, and explicit quickflow/baseflow/groundwater storage and release terms.",
        "",
        "The Bayesian component is empirical-Bayes/MAP partial pooling: ordinary fixed hydrologic coefficients, nonlinear peak/runoff coefficients, quickflow/baseflow component coefficients, and station random intercepts have Gaussian priors. The nonlinear terms use `nonlinear_sigma`; the two-component storage/release terms use `component_sigma`. Hyperparameters were selected using 2006-2015 training and 2016-2018 inner validation only, then refit on 2006-2018 and tested on hidden 2019-2022 observations.",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_two_component_metric_summary.csv",
        "- reports/monthly_bayes_two_component_metrics_by_station.csv",
        "- reports/monthly_bayes_two_component_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_two_component_fixed_parameters.csv",
        "- reports/monthly_bayes_two_component_parameter_group_strength.csv",
        "- reports/monthly_bayes_two_component_station_random_effects.csv",
        "- figure/monthly_bayes_two_component_validation_scatter.png",
        "- figure/monthly_bayes_two_component_validation_metric_distributions.png",
        "- figure/monthly_bayes_two_component_representative_hydrographs.png",
        "",
    ]
