...
REPORTS = RUN / "reports"
LOGS = RUN / "logs"
DAILY_LOG = ROOT / "5_Test" / "20260721.log"
        ...
        writer.writerow(
            {
                ...
                "current_best_run": "20260620_44",
                "generation_type": "dynamic_station_screening_reproducibility_baseline",
                "model_change": f"no model change; exclusions loaded from station_screening_policy.csv: {excluded_text}",
                ...
                "promotion_decision": "must_reproduce_20260620_44_before_screening",
                "notes": f"First run of the dynamic station-screening chain. No Q72/Q78 structural or hyperparameter changes. Active exclusions ({len(excluded_names)}): {excluded_text}. Reservoir-related stations are deferred from screening, not newly removed.",
            }
        )
...
def write_workflow_logs(step_rows: list[dict[str, object]], final: dict[str, object]) -> None:
    ...
    lines = [
        f"# {RUN.name} Dynamic Station-Screening Baseline Log",
        "",
        f"Updated: {datetime.now().isoformat(timespec='seconds')}",
        "",
        "## Purpose",
        "",
        f"Reproduce 20260620_44 before dynamic screening. Active exclusions are loaded from station_screening_policy.csv: {excluded_text}.",
        "",
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
        "- This folder is the reproducibility baseline for a dynamic station-screening chain.",
...
def write_final_summary(final: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## Conclusion",
        "",
        "This folder is the reproducibility baseline for the dynamic non-reservoir station-screening chain.",
        "",
