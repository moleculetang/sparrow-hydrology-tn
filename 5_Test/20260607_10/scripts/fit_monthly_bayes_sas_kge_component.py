                                ...
                                for station_sigma in [1.00]:
                                    for slope_sigma in [0.10, 0.15]:
                                        for anomaly_weight in [0.0, 0.25]:
                                            for flow_contrast_weight in [0.5, 1.0, 2.0, 4.0]:
                                                beta = fit_map_ridge(
                                                    tr,
                                                    stations,
                                                    mean,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_10 SAS young/old storage + flow-duration contrast MAP model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_8/20260607_9 SAS young-water / old-storage release mechanism, but changes the MAP objective after the KGE component diagnosis showed that the SAS branch mainly loses variability ratio alpha rather than temporal correlation or bias ratio.",
        "",
        ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`objective = logQ residuals + anomaly_weight * station-centered standardized logQ anomaly residuals + flow_contrast_weight * station high-low/peak-mid logQ contrast residuals + Gaussian priors`",
        "",
        "The flow-duration contrast rows are built only from fitting-period observations. They constrain parameter estimates to preserve within-station high-flow versus low-flow and peak-flow versus middle-flow dynamic range. This is still parameter-level Bayesian/MAP fitting, not prediction post-processing, and hyperparameters are selected without 2019-2022 observations.",
        "",
    ...
    (RUN_DIR / "README_20260607_10.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_sas_flow_contrast_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_sas_flow_contrast_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
