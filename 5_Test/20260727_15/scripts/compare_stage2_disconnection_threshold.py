...
from experiment_settings import (
    ...
    NSELOG_EFFECT_SIZE,
    REFERENCE_RUN_ID,
    SLOW_FLOW_DISCONNECTION_THRESHOLD_FRACTION,
)


RUN = Path(__file__).resolve().parents[1]
REFERENCE = RUN.parent / REFERENCE_RUN_ID
OUT = RUN / "reports" / "stage2_disconnection_threshold_diagnostics"
TOL = 1.0e-9
...
def main() -> None:
    ...
    threshold_audit = pd.read_csv(
        RUN
        / "reports"
        / "intermediate"
        / "base_regression"
        / "disconnection_threshold_structure_audit.csv",
        encoding="utf-8-sig",
    )
    if len(threshold_audit) != 1:
        raise RuntimeError("disconnection threshold audit must contain exactly one row")
    audit_row = threshold_audit.iloc[0]
    threshold_structure_gate = {
        "threshold_fraction": float(audit_row["threshold_fraction"]),
        "production_capacity_mm": float(
            audit_row["production_capacity_mm"]
        ),
        "threshold_storage_mm": float(audit_row["threshold_storage_mm"]),
        ...
        "candidate_release_not_above_linear_reference": bool(
            audit_row["candidate_release_not_above_linear_reference"]
        ),
        "changed_reference_release_rows": int(
            audit_row["changed_reference_release_rows"]
        ),
        "disconnected_zero_release_rows": int(
            audit_row["disconnected_zero_release_rows"]
        ),
        "disconnection_gate_active_rows": int(
            audit_row["disconnection_gate_active_rows"]
        ),
        "minimum_candidate_to_reference_ratio": float(
            audit_row["minimum_candidate_to_reference_ratio"]
        ),
        "median_candidate_to_reference_ratio": float(
            audit_row["median_candidate_to_reference_ratio"]
        ),
        "maximum_candidate_to_reference_ratio": float(
            audit_row["maximum_candidate_to_reference_ratio"]
        ),
    }
    threshold_structure_gate["passed"] = bool(
        threshold_structure_gate["threshold_fraction"]
        == SLOW_FLOW_DISCONNECTION_THRESHOLD_FRACTION
        and threshold_structure_gate["threshold_storage_mm"]
        == (
            threshold_structure_gate["production_capacity_mm"]
            * SLOW_FLOW_DISCONNECTION_THRESHOLD_FRACTION
        )
        and threshold_structure_gate["maximum_fitting_year"] <= 2018
        and threshold_structure_gate["fitting_rows"] > 0
        and threshold_structure_gate["stations"] > 0
        and threshold_structure_gate["candidate_release_nonnegative"]
        and threshold_structure_gate[
            "candidate_release_not_above_linear_reference"
        ]
        and threshold_structure_gate["changed_reference_release_rows"] > 0
        and 0.0
        <= threshold_structure_gate["minimum_candidate_to_reference_ratio"]
        <= threshold_structure_gate["maximum_candidate_to_reference_ratio"]
        <= 1.0 + TOL
        and threshold_structure_gate["minimum_candidate_to_reference_ratio"]
        < 1.0 - TOL
        and np.isfinite(
            threshold_structure_gate["median_candidate_to_reference_ratio"]
        )
    )
    ...
    hard_gate = bool(
        ...
        and threshold_structure_gate["passed"]
    ...
    decision = (
        "accept_20260727_15_as_disconnection_threshold_candidate"
        if promote
    ...
    result = {
        ...
        "hypothesis": (
            "apply a fixed storage disconnection threshold to the "
            "primary production-store slow release"
        ),
        "tolerance": TOL,
        "effect_sizes": {
        ...
        "disconnection_threshold_structure_gate": threshold_structure_gate,
    ...
    (OUT / "disconnection_threshold_gate.json").write_text(
    ...
    lines = [
        f"# {RUN.name} Stage-2A3 Disconnection-Threshold Gate",
        "",
        ...
        f"- disconnection-threshold structure audit: {threshold_structure_gate['passed']}",
    ...
    (OUT / "disconnection_threshold_gate.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print("\n".join(lines))
    if not hard_gate:
