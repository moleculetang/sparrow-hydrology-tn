...
try:
    ...
    SLOW_FLOW_DISCONNECTION_THRESHOLD_FRACTION = 0.0
...
def threshold_slow_release_mm(
    storage_mm: float,
    capacity_mm: float,
    release_rate: float,
    threshold_fraction: float,
) -> float:
    """Return slow release only from storage above a fixed disconnection threshold."""
    storage = max(float(storage_mm), 0.0)
    capacity = max(float(capacity_mm), EPS)
    fraction = float(threshold_fraction)
    if not 0.0 <= fraction < 1.0:
        raise ValueError("disconnection threshold fraction must be within [0, 1)")
    connected_storage = max(storage - fraction * capacity, 0.0)
    release = float(release_rate) * connected_storage
...
def add_hydrologic_features(
    ...
    for _, idx in out.groupby("comid", sort=False).groups.items():
        ...
        for pos in idx:
            ...
            slow_mm = threshold_slow_release_mm(
                soil_store,
                prod_capacity,
                base_release,
                SLOW_FLOW_DISCONNECTION_THRESHOLD_FRACTION,
