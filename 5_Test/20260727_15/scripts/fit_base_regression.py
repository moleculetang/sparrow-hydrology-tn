...
def main() -> None:
    ...
    fitting = frame[frame["year"].le(2018)].copy()
    reference_release = fitting["production_base_linear_reference_cfs"]
    candidate_release = fitting["production_base_cfs"]
    positive_reference = reference_release.gt(EPS)
    ratio = (
        candidate_release[positive_reference]
        / reference_release[positive_reference]
    )
        ...
        [
            {
                ...
                "production_capacity_mm": hp["prod_capacity"],
                "threshold_storage_mm": (
                    SLOW_FLOW_DISCONNECTION_THRESHOLD_FRACTION
                    * hp["prod_capacity"]
                ),
                "minimum_fitting_year": int(fitting["year"].min()),
                "maximum_fitting_year": int(fitting["year"].max()),
                "fitting_rows": int(len(fitting)),
                "stations": int(fitting["q_site"].nunique()),
                "candidate_release_nonnegative": bool(
                    candidate_release.ge(0).all()
                ),
                "candidate_release_not_above_linear_reference": bool(
                    (candidate_release <= reference_release + EPS).all()
                ),
                "changed_reference_release_rows": int(
                    (
                        (candidate_release - reference_release).abs()
                        > EPS
                    ).sum()
                ),
                "disconnected_zero_release_rows": int(
                    (
                        candidate_release.le(EPS)
                        & reference_release.gt(EPS)
                    ).sum()
                ),
                "disconnection_gate_active_rows": int(
                    fitting["production_disconnection_active"].gt(0.5).sum()
                ),
                "minimum_candidate_to_reference_ratio": float(ratio.min()),
                "median_candidate_to_reference_ratio": float(ratio.median()),
                "maximum_candidate_to_reference_ratio": float(ratio.max()),
            }
        ]
    )
    threshold_audit.to_csv(
        REPORTS / "disconnection_threshold_structure_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
