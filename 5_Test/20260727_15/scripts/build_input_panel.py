...
def write_readme() -> None:
    ...
    phase = str(experiment.get("phase", "model_development"))
    decision_state = str(
        experiment.get(
            "decision_after_run",
    ...
    text = f"""# {RUN_DIR.name} {phase} Single-Factor Model-Development Experiment

This folder tests one major factor against the accepted `20260727_6` baseline: `{model_change}`. The core logQ objective, all auxiliary losses, Q78 mass model, fusion, input data and station policy remain frozen.
...
- The experimental structure and constants are fixed before execution and apply uniformly; they are not selected from 2019-2022 observations.
