...
from experiment_settings import (
    Q78_OBSERVATION_END_YEAR,
    Q78_OBSERVATION_START_YEAR,
    REFERENCE_RUN_ID,
)
...
def main() -> None:
    ...
    frozen = frozen[
        [
            ...
            "good",
            "protected_station",
            "primary_failure_group",
        ]
    ].drop_duplicates("q_site")
    frozen = frozen.rename(columns={"good": "baseline_good"})
    ...
    ].merge(frozen, on="q_site", how="left", validate="one_to_one")
    evaluation["good"] = (
    ...
    evaluation.to_csv(
        OUT / "candidate_station_metrics_by_frozen_group.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    group_summary.to_csv(
        OUT / "candidate_frozen_failure_group_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    population.to_csv(
        OUT / "candidate_population_summary.csv", index=False, encoding="utf-8-sig"
    )
        ...
        coverage_rows.append(
            {
                ...
                "q78_currently_includes_year": bool(
                    predictions["year"].eq(year).any()
                ),
            }
        )
    ...
    report = [
        f"# {RUN.name} Stage-2A3 Disconnection-Threshold Diagnostics",
        "",
        "## Candidate populations",
        "",
        markdown_table(population),
        "",
        "## Metrics by frozen failure group",
        "",
        ...
        f"- Q78 remains frozen at {Q78_OBSERVATION_START_YEAR}-{Q78_OBSERVATION_END_YEAR}.",
        "",
        "## Boundaries",
        "",
        "- The only experimental factor is applying a fixed storage disconnection threshold to the primary production-store slow release.",
        "- All auxiliary losses remain disabled; Q78 and the fusion rule remain frozen.",
        "- Low-flow rows are the bottom 25% of each station's observed fitting-period discharge; every eligible station has equal total auxiliary-loss weight.",
        "- The baseline monthly logQ loss, Q72 structure, all hydrologic features, priors, auxiliary flow-contrast loss, Q78, fusion rule, and station exclusions are frozen.",
        f"- Failure groups and reservoir labels are loaded from {REFERENCE.name}; they are not reclassified from candidate holdout results.",
    ...
    (OUT / "stage2_disconnection_threshold_diagnostic_report.md").write_text(
        "\n".join(report) + "\n", encoding="utf-8"
    )
    print("\n".join(report))
