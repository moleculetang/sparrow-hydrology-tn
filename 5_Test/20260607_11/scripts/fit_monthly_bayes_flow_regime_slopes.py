...
REGIME_GATES = ["high_flow_regime_gate", "low_flow_regime_gate"]


def feature_group(feature: str) -> str:
    if feature.startswith("regime_random_slope_"):
        return "station_random_flow_regime_slope"
                            ...
                            for fixed_sigma in [3.0]:
                                for station_sigma in [1.00]:
                                    for slope_sigma in [0.10, 0.15]:
                                        for regime_slope_sigma in [0.10, 0.25, 0.50]:
                                            for anomaly_weight in [0.0, 0.25]:
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_11 flow-regime Bayesian random-slope model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the SAS young-water / old-storage release mechanism, but changes the parameter structure instead of merely changing objective weights. It adds station-level Bayesian random slopes that are gated by high-flow and low-flow hydrologic regimes.",
        "",
        "Base random slopes: " + ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "Regime-gated random slopes: " + ", ".join(REGIME_SLOPE_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`logQ = fixed monthly hydrologic terms + station random intercept + station random slopes + high/low-flow gated station random slopes + Gaussian priors`",
        "",
        "The high-flow and low-flow gates are computed from modeled antecedent wetness and SAS young-water fraction, not from validation-period observed Q. Optional anomaly and flow-contrast objective terms remain available, but the mechanism being tested is the enlarged parameter structure. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_11.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_metric_summary.csv", index=False, encoding="utf-8-sig")
                ...
                regime_rows.append(
                    {
                        ...
                        "coefficient_standardized": value,
                        "feature_group": "station_random_flow_regime_slope",
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_flow_regime_slopes_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_flow_regime_slopes_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
