...
def feature_group(feature: str) -> str:
    if feature.startswith("amplitude_gain_"):
        return "station_random_mean_anomaly_gain"
...
def station_feature_centers(train: pd.DataFrame, stations: list[str], mean: pd.Series, std: pd.Series) -> pd.DataFrame:
    x_train = ((train[FIXED_FEATURES] - mean) / std).replace([np.inf, -np.inf], 0.0).fillna(0.0)
    x_train = x_train.assign(q_site=train["q_site"].astype(str).to_numpy())
    centers = x_train.groupby("q_site")[AMPLITUDE_GAIN_FEATURES].mean()
    return centers.reindex(stations).fillna(0.0)
...
def build_matrix(
    ...
    site_center_rows = centers.loc[df["q_site"].astype(str).to_numpy(), AMPLITUDE_GAIN_FEATURES].to_numpy(dtype=float)
    for feat_i, feat in enumerate(AMPLITUDE_GAIN_FEATURES):
        block = np.zeros((len(df), len(stations)), dtype=float)
        values = x_fixed_df[feat].to_numpy(dtype=float) - site_center_rows[:, feat_i]
        block[np.arange(len(df)), site_codes] = values
        slope_blocks.append(block)
    for gate in REGIME_GATES:
...
def fit_map_ridge(
    ...
    x, y = build_matrix(train, stations, mean, std, centers)
    x_parts = [x]
    y_parts = [y]
    if anomaly_weight > 0:
    ...
    n_amplitude = len(AMPLITUDE_GAIN_FEATURES) * n_station
    n_regime_slope = len(REGIME_GATES) * len(REGIME_SLOPE_FEATURES) * n_station
    penalty = np.zeros(n_fixed + n_station + n_slope + n_amplitude + n_regime_slope, dtype=float)
    ...
    penalty[n_fixed + n_station + n_slope : n_fixed + n_station + n_slope + n_amplitude] = 1.0 / max(
        amplitude_sigma, EPS
    )
    penalty[n_fixed + n_station + n_slope + n_amplitude :] = 1.0 / max(regime_slope_sigma, EPS)
...
def predict_log(
    ...
    x, _ = build_matrix(df, stations, mean, std, centers)
    return x @ beta
                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            ...
                            centers = station_feature_centers(tr, stations, mean, std)
                            for fixed_sigma in [3.0]:
                                for station_sigma in [1.00]:
                                    for slope_sigma in [0.15]:
                                        for amplitude_sigma in [0.15, 0.30, 0.60, 1.00]:
                                            for regime_slope_sigma in [0.25]:
                                                for anomaly_weight in [0.0]:
                                                    for flow_contrast_weight in [0.0, 1.0]:
                                                        ...
                                                        pred = np.exp(
                                                            np.clip(predict_log(iv, beta, stations, mean, std, centers), -20, 20)
                                                        )
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_18 mean-anomaly amplitude-gain Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_12 SAS season x flow-regime hierarchy, but adds a new station-level mean-anomaly amplitude-gain layer. The added parameters operate on station-centered hydrologic feature anomalies, so they target within-station monthly variability rather than station mean bias.",
        "",
        "Base random slopes: " + ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "Mean-anomaly amplitude gain features: " + ", ".join(AMPLITUDE_GAIN_FEATURES),
        "",
        ...
        "",
        "`logQ = fixed monthly hydrologic terms + station random intercept + station random slopes + station mean-anomaly amplitude slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The station centers used by the amplitude-gain layer are computed from the fitting period only. No 2019-2022 observed Q is used to form centers, gates, hyperparameters, or corrections. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_18.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    centers = station_feature_centers(train, stations, mean, std)
    beta = fit_map_ridge(
        train,
        stations,
        mean,
    ...
    featured["predict_log"] = predict_log(featured, beta, stations, mean, std, centers)
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    for feat_i, feat in enumerate(AMPLITUDE_GAIN_FEATURES):
        vals = beta[offset + feat_i * n_station : offset + (feat_i + 1) * n_station]
        station_effect[f"amplitude_gain_{feat}"] = vals
        for station, value in zip(stations, vals):
            amplitude_rows.append(
                {
                    "parameter": f"amplitude_gain_{feat}::{station}",
                    "coefficient_standardized": value,
                    "feature_group": "station_random_mean_anomaly_gain",
    ...
    amplitude_gain_params = pd.DataFrame(amplitude_rows)
    regime_slope_params = pd.DataFrame(regime_rows)
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    amplitude_gain_params.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_amplitude_gain_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_mean_anomaly_gain_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
            ...
            amplitude_gain_params[["parameter", "coefficient_standardized", "feature_group", "prior_sigma"]],
        ...
        REPORT_DIR / "monthly_bayes_mean_anomaly_gain_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
