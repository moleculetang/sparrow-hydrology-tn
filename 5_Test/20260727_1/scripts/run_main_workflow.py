...
REPORTS = RUN / "reports"
LOGS = RUN / "logs"
DAILY_LOG = ROOT / "5_Test" / "20260727.log"
        ...
        writer.writerow(
            {
                ...
                "promotion_decision": "pending_common_station_ablation_gate",
                "notes": f"Dynamic station-screening run. No Q72/Q78 structural or hyperparameter changes. Active exclusions ({len(excluded_names)}): {excluded_text}. Reservoir-related stations are deferred from screening, not newly removed.",
            }
        )
...
def write_workflow_logs(step_rows: list[dict[str, object]], final: dict[str, object]) -> None:
    ...
    lines = [
        f"# {RUN.name} Dynamic Station-Screening Run Log",
        "",
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
        "- This folder is one sequential experiment in the dynamic station-screening chain.",
...
def write_final_summary(final: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## Conclusion",
        "",
        "This folder is one sequential experiment in the dynamic non-reservoir station-screening chain.",
        "",
