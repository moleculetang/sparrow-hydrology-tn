                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            for prod_capacity in [240.0, 480.0]:
                                for runoff_gamma in [1.5, 2.5]:
                                    for quick_rho in [0.25, 0.55]:
                                        for base_rho in [0.85, 0.95]:
                                            for base_release in [0.04, 0.10]:
                                                ...
                                                for fixed_sigma in [3.0]:
                                                    for production_sigma in [0.6, 1.5]:
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_19 nonlinear production-routing Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the SAS season x flow-regime Bayesian hierarchy, but adds a nonlinear monthly production-store and quick/base routing mechanism before parameter fitting. The new terms target physical high-flow mass generation rather than post-hoc amplitude correction.",
        "",
        ...
        "",
        "The production store, saturation-excess runoff, quick routing, and base routing states are computed from monthly PPT/AET/PET-derived effective water input and topology reach-month records. No 2019-2022 observed Q is used to form states, gates, hyperparameters, or corrections. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_19.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_production_routing_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_production_routing_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_production_routing_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_production_routing_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_production_routing_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_production_routing_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_production_routing_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_production_routing_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_production_routing_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_production_routing_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_production_routing_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
