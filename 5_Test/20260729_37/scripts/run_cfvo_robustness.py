...
PARENT_GATE = (
    ...
    / "cfvo_reordered_recharge" / "gate.json"
)
STATION_RESULT = (
    ROOT / "5_Test" / "20260729_36" / "reports"
...
CANDIDATE_MODEL = (
    ROOT / "5_Test" / "20260729_36" / "outputs"
    / "cfvo_reordered_recharge_reach_month.parquet"
)
OBSERVED = (
...
def bfi_robustness() -> tuple[pd.DataFrame, dict]:
    ...
    station["sensitivity_group"] = np.where(
        ...
        "lower_sensitivity", "higher_sensitivity",
    )
    station["area_group"] = pd.qcut(
        station["upstream_incremental_area_km2"], q=3,
    ...
    def delta(frame: pd.DataFrame) -> float:
        ...
        candidate = observed.corr(
            frame["simulated_bfi_cfvo_reordered"], method="spearman"
        )
        return float(candidate - current)

    rows = [{
        "group_type": "all", "group": "all_97",
        "n": len(station), "spatial_bfi_delta": delta(station),
    }]
    ...
    rng = np.random.default_rng(2026072937)
    ...
    leave_one_out = [
        delta(station.drop(station.index[omitted]))
    ...
    metrics = {
        ...
        "leave_one_out_positive_fraction": float(
            np.mean(np.asarray(leave_one_out) > 0)
        ),
        "minimum_subgroup_delta": float(
            groups.loc[
                ~groups["group_type"].eq("all"),
                "spatial_bfi_delta",
            ].min()
        ),
    }
...
def main() -> None:
    ...
    if parent["authorized_next_action"] != (
        ...
        raise RuntimeError("Parent gate does not authorize this test")
    ...
    period_index = period_summary.set_index("period")
    dense_periods = period_index.loc[["2010_2013", "2014_2018"]]
    sparse_period = period_index.loc["2006_2009"]
    checks = {
        ...
        "dense_periods_at_least_90_stations": bool(
            dense_periods["station_count"].ge(90).all()
        ),
        "sparse_early_period_at_least_5_stations": (
            int(sparse_period["station_count"]) >= 5
        ),
        ...
        "all_period_lowflow_guardrails": bool(
            period_summary[
                "median_lowflow_log_error_delta"
            ].le(0.05).all()
        ),
    ...
    if promoted:
        decision = "PROMOTE_CFVO_CONTROL_TO_DEVELOPMENT_BASELINE"
        next_action = "DIAGNOSE_REMAINING_LOW_FLOW_AND_BFI_FAILURES"
    elif not (
        checks["bfi_bootstrap_lower_positive"]
        and checks["bfi_leave_one_out_positive_at_least_95pct"]
        and checks["all_bfi_subgroups_positive"]
    ):
        decision = "DO_NOT_PROMOTE_CFVO_CONTROL"
        next_action = (
            "ACQUIRE_MISSING_SAND_CLAY_BULK_DENSITY_AND_DRAINAGE_ATTRIBUTES"
        )
    else:
        decision = "DO_NOT_PROMOTE_CFVO_CONTROL"
        next_action = "DIAGNOSE_TEMPORAL_LOW_FLOW_TRADEOFF"
    gate = {
        "run_id": "20260729_37",
        "phase": "cfvo_temporal_station_robustness_gate",
        "created_utc": utc_now(),
        "checks": checks,
...
"""
    ...
    sources = [
        ...
        RUN / "scripts" / "run_cfvo_robustness.py",
        RUN / "scripts" / "validate_cfvo_robustness.py",
    ]
    products = [
        REPORT / "station_period_metrics.csv",
        REPORT / "period_summary.csv",
    ...
    provenance = {
        ...
        "created_utc": utc_now(),
        "sources": [
            record(path, "cfvo_robustness_source", "reported_or_derived")
            for path in sources
        ],
        "products": [
            record(path, "cfvo_robustness_product", "derived")
    ...
    (LOGS / "runtime_log.md").write_text(
        ...
        "E:\\SPARROW\\5_Test\\20260729_37\\scripts\\"
        "run_cfvo_robustness.py`\n\n"
        f"Decision: `{decision}`\n",
        encoding="utf-8",
    )
    print(json.dumps({
