...
DISCHARGE_SOURCES = (
    ("complete_2010_2022", ROOT / "1_Inputs" / "DischargeData" / "complete_2010_2022", 100),
    ("noncomplete_2010_2022", ROOT / "1_Inputs" / "DischargeData" / "noncomplete_2010_2022", 0),
)
EARLY_DISCHARGE_MONTHLY = ROOT / "1_Inputs" / "DischargeData" / "monthly_mean_2006_2009" / "DischargeData_2006_2009.xlsx"
STATION_SHP = ROOT / "0_reach_topology" / "data" / "raw" / "vector" / "PRB水文站_全部.shp"
...
def discover_discharge_csvs() -> pd.DataFrame:
    ...
    for source_group, source_dir, source_priority in DISCHARGE_SOURCES:
        for path in source_dir.rglob("*.csv"):
            ...
            for part in parts:
                if re.fullmatch(r"supplement\d+", part, flags=re.IGNORECASE):
                    source_detail = part.lower()
                    break
            supplement_priority = 0 if source_detail == "main" else int(re.search(r"\d+", source_detail).group(0))
            station = path.stem
            rows.append(
                {
                    ...
                    "source": f"{source_group}_{source_detail}",
                    "priority": source_priority + supplement_priority,
                    "path": str(path),
                }
            )
    files = pd.DataFrame(rows)
    if files.empty:
        raise RuntimeError("No discharge CSV files found in the configured 2010-2022 discharge sources.")
    ...
    selection = files.merge(chosen[["station_norm", "year", "path"]].assign(selected_for_input=True), on=["station_norm", "year", "path"], how="left")
    selection["selected_for_input"] = selection["selected_for_input"].fillna(False)
    selection.groupby(["source_group", "selected_for_input"], as_index=False).agg(
        station_year_files=("path", "size"),
        stations=("station_norm", "nunique"),
    ).to_csv(RUN_DIR / "reports" / "discharge_source_selection_summary.csv", index=False, encoding="utf-8-sig")
...
def read_daily_csv(row: pd.Series) -> pd.DataFrame:
    ...
    long["source_group"] = row["source_group"]
    valid_day = []
...
def build_discharge_monthly(files: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = (
        ...
        .agg(
            ...
            source=("source", "last"),
            source_group=("source_group", "last"),
        )
    )
    ...
    grouped["source_window"] = grouped["source_group"].astype(str) + "_daily"
    ...
    canonical_name_by_norm = daily_station_names.set_index("station_norm")["canonical_station_name"].to_dict()
    early = read_early_discharge_monthly(canonical_name_by_norm)
    combined = pd.concat([early, grouped], ignore_index=True, sort=False) if not early.empty else grouped
    combined["source_precedence"] = combined["source_window"].astype(str).str.endswith("_daily").astype(int)
    duplicate_months = combined[combined.duplicated(["station_norm", "year", "month"], keep=False)].copy()
    combined = combined.sort_values(["station_norm", "year", "month", "source_precedence", "source"], kind="stable")
    combined["selected_for_input"] = ~combined.duplicated(["station_norm", "year", "month"], keep="last")
    if not duplicate_months.empty:
        duplicate_months = duplicate_months.merge(
            combined[["station_norm", "year", "month", "source", "selected_for_input"]],
            on=["station_norm", "year", "month", "source"],
            how="left",
        )
    duplicate_months.to_csv(
        RUN_DIR / "reports" / "discharge_month_source_precedence_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
    combined = combined[combined["selected_for_input"]].drop(columns=["source_precedence", "selected_for_input"])
...
def read_early_discharge_monthly(canonical_name_by_norm: dict[str, str]) -> pd.DataFrame:
        ...
        [
            {
                ...
                "early_rows_after_all_station_inclusion": len(early),
                "early_stations_after_all_station_inclusion": early["station_norm"].nunique(),
                "excluded_rows_noncomplete_station": len(excluded),
                "excluded_stations_noncomplete_station": excluded["station_norm"].nunique(),
                "filter_rule": "all 2006-2009 monthly mean stations are retained before explicit exclusion and station-to-reach matching",
            }
        ]
    ).to_csv(RUN_DIR / "reports" / "monthly_mean_2006_2009_complete_station_filter_summary.csv", index=False, encoding="utf-8-sig")
    early.to_csv(RUN_DIR / "reports" / "monthly_mean_2006_2009_monthly_all_stations.csv", index=False, encoding="utf-8-sig")
...
def build_panel(topo: pd.DataFrame, discharge_m: pd.DataFrame, station_match: pd.DataFrame) -> pd.DataFrame:
    ...
    station_reliability["station_count_on_reach"] = station_reliability.groupby("reach_id")["station_norm"].transform("size")
    ...
    station_reliability[station_reliability["station_count_on_reach"] > 1].to_csv(
        RUN_DIR / "reports" / "same_reach_multi_station_selection_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def write_reports(indata: pd.DataFrame, reach: pd.DataFrame, topo: pd.DataFrame, discharge_m: pd.DataFrame, station_match: pd.DataFrame) -> None:
    ...
    summary = [
        ...
        "- Discharge observations came from complete and noncomplete 2010-2022 daily sources plus the full 2006-2009 monthly mean workbook.",
        ...
        "- same-reach multi-station conflicts retain only the highest-median-flow station among topology/flow-plausible candidates; coverage and distance are deterministic tie-breakers.",
        "",
        "## Climate inputs",
...
def write_readme() -> None:
    ...
    excluded_names = policy.loc[policy["exclude_before_training"], "station_name"].astype(str).tolist()
    excluded_text = "、".join(excluded_names) if excluded_names else "none"
    experiment_path = RUN_DIR / "inputs" / "source_metadata" / "run_experiment.json"
    experiment = json.loads(experiment_path.read_text(encoding="utf-8")) if experiment_path.exists() else {}
    parent_run = str(experiment.get("parent_run", "20260620_44"))
    candidate = str(experiment.get("candidate_station", ""))
    generation_type = str(experiment.get("generation_type", "dynamic_station_screening_baseline"))
    hypothesis_id = str(experiment.get("hypothesis_id", "unspecified"))
    model_change = str(experiment.get("model_change", "unspecified"))
    decision_state = str(
        experiment.get(
            "decision_after_run",
            "pending_stage2a_nonlinear_recession_gate",
        )
    )
    text = f"""# {RUN_DIR.name} Stage-2A Fixed Nonlinear-Recession Experiment

This folder tests one major factor against the accepted `20260727_6` baseline: replace the primary production store's linear recession exponent `1.0` with a fixed power-law exponent `1.5`. The core logQ objective, all auxiliary losses, Q78 mass model, fusion, input data and station policy remain frozen.

- accepted parent before this run: `{parent_run}`
- generation type: `{generation_type}`
- hypothesis: `{hypothesis_id}`
- single model change: `{model_change}`
- candidate station: `{candidate or 'none'}`
- decision state: `{decision_state}`

Important modeling boundary:

- This run rebuilds the current hydrologic mainline inputs from source data.
- It uses `20260727_6` only as a frozen comparison reference; it does not reuse fitted outputs as model inputs.
- The exponent is fixed before execution and applies uniformly; it is not selected from 2019-2022 observations.
- The frozen 2019-2022 failure labels are used only for evaluation, not to activate station-specific behavior.
- Same-reach multi-station conflicts retain the highest-median-flow station among topology/flow-plausible candidates; coverage and distance are tie-breakers.
...
- Stations excluded before station matching and model training in this run: {excluded_text}.
...
- `E:/SPARROW/1_Inputs/DischargeData/noncomplete_2010_2022`
...
- No new station removal is permitted in this model-development series.
- 2019-2022 is reported only after each model structure and parameter grid is frozen.
- Later experiments use sequential `20260727_N` folders, copy only from an accepted parent, and change one major hypothesis per folder.
- The authoritative boundaries are `../20260727_5/SPARROW_final_plan_and_boundaries.md` and `../20260727_5/SPARROW_10_papers_formula_reference.md`.
...
def main() -> int:
    ...
    log = [
        ...
        "",
        "Built PRB Q input table from complete and noncomplete 2010-2022 daily discharge sources.",
        "All available 2006-2009 monthly mean observations were retained before explicit station exclusions and station-to-reach matching.",
        ...
        "Same-reach multiple stations were resolved by selecting the highest-median-flow topology-plausible station for each reach; coverage and distance were tie-breakers.",
