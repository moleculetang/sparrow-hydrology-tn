...
try:
    ...
    NONLINEAR_RECESSION_EXPONENT = 1.0
...
def nonlinear_recession_release_mm(
    storage_mm: float,
    capacity_mm: float,
    release_rate: float,
    exponent: float,
) -> float:
    """Return a bounded power-law recession release for one monthly step."""
    storage = max(float(storage_mm), 0.0)
    capacity = max(float(capacity_mm), EPS)
    power = float(exponent)
    if not np.isfinite(power) or power <= 0:
        raise ValueError("nonlinear recession exponent must be positive and finite")
    saturation = float(np.clip(storage / capacity, 0.0, 1.0))
    release = float(release_rate) * capacity * saturation**power
    return float(np.clip(release, 0.0, storage))
...
def add_hydrologic_features(
    ...
    for _, idx in out.groupby("comid", sort=False).groups.items():
        ...
        for pos in idx:
            ...
            sat1 = float(np.clip(soil_store / max(prod_capacity, EPS), 0.0, 1.5))
            linear_slow_mm = base_release * soil_store
            slow_mm = nonlinear_recession_release_mm(
                soil_store,
                prod_capacity,
                base_release,
    ...
    out["production_recession_ratio_to_linear"] = np.divide(
        production_base_cfs,
        production_base_linear_reference_cfs,
        out=np.ones_like(production_base_cfs),
        where=production_base_linear_reference_cfs > EPS,
    )
