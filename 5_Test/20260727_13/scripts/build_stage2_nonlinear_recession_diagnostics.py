...
def main() -> None:
    ...
    report = [
        f"# {RUN.name} Stage-2A Nonlinear-Recession Diagnostics",
        "",
        ...
        "## Boundaries",
        "",
        "- The only experimental factor is replacing the primary production store's linear recession exponent 1.0 with a fixed power-law exponent 1.5.",
    ...
    (OUT / "stage2_nonlinear_recession_diagnostic_report.md").write_text(
        "\n".join(report) + "\n", encoding="utf-8"
    )
    print("\n".join(report))
