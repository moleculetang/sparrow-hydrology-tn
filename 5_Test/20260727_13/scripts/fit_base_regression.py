...
def main() -> None:
    ...
    recession_audit = pd.DataFrame(
        [
            {
                "reference_exponent": LINEAR_RECESSION_REFERENCE_EXPONENT,
                "candidate_exponent": NONLINEAR_RECESSION_EXPONENT,
                ...
                "minimum_candidate_to_linear_ratio": float(
                    fitting["production_recession_ratio_to_linear"].min()
                ),
                "median_candidate_to_linear_ratio": float(
                    fitting["production_recession_ratio_to_linear"].median()
                ),
                "maximum_candidate_to_linear_ratio": float(
                    fitting["production_recession_ratio_to_linear"].max()
                ),
                "candidate_release_nonnegative": bool(
                    fitting["production_base_cfs"].ge(0).all()
                ),
                "candidate_release_not_above_linear": bool(
                    (
                        fitting["production_base_cfs"]
                        <= fitting["production_base_linear_reference_cfs"]
                        + EPS
                    ).all()
                ),
    ...
    recession_audit.to_csv(
        REPORTS / "nonlinear_recession_structure_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
