    ...
    for rho in [0.70]:
        for wm in [480.0]:
            for et_gamma in [0.50, 0.75]:
                for res_rho in [0.65, 0.85, 0.95]:
                    for release_alpha in [0.35, 0.65, 0.85]:
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260606_11 reservoir-proxy strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_reservoir_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_reservoir_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_reservoir_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_reservoir_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_reservoir_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_reservoir_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_reservoir_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_reservoir_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_reservoir_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(param_rows).to_csv(REPORT_DIR / "monthly_bayes_reservoir_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_reservoir_station_random_effects.csv", index=False, encoding="utf-8-sig")

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260606_11 explicit reservoir release/attenuation proxy model",
        "",
        "This numeric generation returns to the 20260606_4 ET-aware baseline and adds explicit reservoir storage, release, attenuation, and seasonal release proxy terms by reservoir influence order.",
        "",
        "## Equation",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_reservoir_metric_summary.csv",
        "- reports/monthly_bayes_reservoir_metrics_by_station.csv",
        "- reports/monthly_bayes_reservoir_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_reservoir_fixed_parameters.csv",
        "- reports/monthly_bayes_reservoir_station_random_effects.csv",
        "",
    ]
    (RUN_DIR / "README_20260606_11.md").write_text("\n".join(lines), encoding="utf-8")
