...
def transform_q(q: np.ndarray, target_lambda: float) -> np.ndarray:
    q = np.asarray(q, dtype=float)
    q = np.maximum(q, EPS)
    if abs(target_lambda) < 1.0e-12:
        return np.log(q)
    return (np.power(q, target_lambda) - 1.0) / target_lambda


def inverse_transform_q(y: np.ndarray, target_lambda: float) -> np.ndarray:
    y = np.asarray(y, dtype=float)
    if abs(target_lambda) < 1.0e-12:
        return np.exp(np.clip(y, -20, 20))
    base = np.maximum(target_lambda * y + 1.0, EPS)
    return np.power(base, 1.0 / target_lambda)
...
def build_matrix(
    ...
    y = transform_q(df["Q_obsv_cfs"].to_numpy(dtype=float), target_lambda)
    return x, y
...
def fit_map_ridge(
    ...
    x, y = build_matrix(train, stations, mean, std, target_lambda)
    x_parts = [x]
    y_parts = [y]
    if anomaly_weight > 0:
...
def predict_target(
    ...
    x, _ = build_matrix(df, stations, mean, std, target_lambda)
    return x @ beta
                ...
                for sas_rho in [0.93]:
                    for young_k in [1.5]:
                        for storage_scale in [720.0]:
                            ...
                            for target_lambda in [0.0, 0.25, 0.50]:
                                        ...
                                        for slope_sigma in [0.10, 0.15]:
                                            for regime_slope_sigma in [0.10, 0.25]:
                                                for anomaly_weight in [0.0]:
                                                    for flow_contrast_weight in [0.0]:
                                                        beta = fit_map_ridge(
                                                            tr,
                                                            stations,
                                                            mean,
                                                        ...
                                                        pred = inverse_transform_q(
                                                            predict_target(iv, beta, stations, mean, std, target_lambda),
                                                            target_lambda,
                                                        )
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_23 Box-Cox / power-flow target Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the SAS season x flow-regime hierarchy, but changes the fitted target from fixed logQ to a selectable Box-Cox / power-flow target. Predictions are inverse-transformed back to Q for strict validation.",
        "",
        ...
        "",
        "`BoxCox(Q; lambda) = fixed monthly hydrologic terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The target transform is selected using 2006-2015 / 2016-2018 inner validation only. No 2019-2022 observed Q is used for transform selection, gates, hyperparameters, or corrections. This is target-level MAP/Bayesian fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_23.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    beta = fit_map_ridge(
        ...
        mean,
        std,
        target_lambda=float(best["target_lambda"]),
    ...
    featured["predict_target"] = predict_target(featured, beta, stations, mean, std, float(best["target_lambda"]))
    featured["predict"] = inverse_transform_q(featured["predict_target"].to_numpy(dtype=float), float(best["target_lambda"]))
    featured["predict_log"] = np.log(np.maximum(featured["predict"], EPS))
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_power_flow_target_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_power_flow_target_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
