...
STEPS = [
    ...
    ("run_three_blocked_q72_folds", "run_q72_baseline.py"),
    ("build_model_audit", "build_model_audit.py"),
    ("write_readme", "write_readme.py"),
    ("validate_independent_baseline", "validate_baseline.py"),
]
...
def main() -> None:
    ...
    for step, script_name in STEPS:
        ...
        completed = subprocess.run([sys.executable, str(RUN / "scripts" / script_name)], cwd=RUN, text=True, encoding="utf-8", errors="replace", capture_output=True, check=False)
    ...
    (logs / "run_all_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
