...
def station_display(value: object) -> str:
    text = clean_text(value)
    if not text:
        raise ValueError("Empty station name cannot be converted to a display name")
...
def read_authority() -> tuple[pd.DataFrame, list[str], dict[str, object]]:
    ...
    # The authority workbook contains one demonstrable omitted station label:
    # Excel row 2306 has river=中州河, year=2006, month=1, followed immediately by
    # 大象（二）站 on the same river/year for months 2--12.  Fill a blank only
    # when the immediately following source row is the next calendar month on the
    # same river and in the same year.  This is a structural workbook repair, not
    # a hydrological or model-performance inference.
    source["station_name_fill_rule"] = "AS_RECORDED"
    blank_station = source["station_raw"].map(clean_text).eq("")
    for idx in source.index[blank_station]:
        position = source.index.get_loc(idx)
        if position + 1 >= len(source):
            continue
        current = source.iloc[position]
        following = source.iloc[position + 1]
        same_river = clean_text(current["river_raw"]) == clean_text(following["river_raw"])
        same_year = int(current["year"]) == int(following["year"])
        next_month = int(following["month"]) == int(current["month"]) + 1
        following_station = clean_text(following["station_raw"])
        if same_river and same_year and next_month and following_station:
            source.at[idx, "station_raw"] = following["station_raw"]
            source.at[idx, "station_name_fill_rule"] = "BLANK_FILLED_FROM_NEXT_CONTIGUOUS_MONTH_SAME_RIVER_YEAR"
    ...
    if source["station_key"].eq("").any():
        rows = source.loc[source["station_key"].eq(""), "source_excel_row"].astype(int).tolist()
        raise RuntimeError(f"Authority contains unresolved blank station names at Excel rows: {rows}")
    ...
    info = {
        ...
        "station_name_fill_rule_counts": {str(k): int(v) for k, v in source["station_name_fill_rule"].value_counts().items()},
...
def main() -> None:
    ...
    assert station_base("沾益（五）站") == "沾益(五)"
    assert station_base("三岔站") == "三岔"
    assert river_key("中州河") == "中州河"
    try:
        station_display("")
    except ValueError:
        pass
    else:
        raise AssertionError("Empty station fixture must fail")
