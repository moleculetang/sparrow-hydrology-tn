...
def _reservoir_reach_ids() -> set[int]:
    path = ROOT / "0_reach_topology" / "results" / "tables" / "topology_edges.csv"
    if not path.exists():
        return set()
    topo = pd.read_csv(path, encoding="utf-8-sig")
    if "reach_id" not in topo.columns or "src_id" not in topo.columns:
        return set()
    is_reservoir = topo["src_id"].astype(str).str.contains("\u6c34\u5e93", na=False, regex=False)
    return set(pd.to_numeric(topo.loc[is_reservoir, "reach_id"], errors="coerce").dropna().astype(int))
...
def apply_prb_q_modifications(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["wd"] = 0.0
    out["storage"] = 0.0
    reservoir_ids = _reservoir_reach_ids()
