...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidates-json", help="JSON array of stations for a joint ablation.")
    parser.add_argument("--experiment-run", type=Path, help="Run holding run_experiment.json; defaults to --trial.")
    parser.add_argument("--output-dir", type=Path, help="Comparison output directory; defaults inside --trial.")
...
def main() -> None:
    ...
    experiment_run = args.experiment_run.resolve() if args.experiment_run else trial
    experiment_path = experiment_run / "inputs" / "source_metadata" / "run_experiment.json"
    experiment = json.loads(experiment_path.read_text(encoding="utf-8"))
    if args.candidates_json:
        candidates = [str(x) for x in json.loads(args.candidates_json)]
    elif experiment.get("candidate_stations"):
        candidates = [str(x) for x in experiment["candidate_stations"]]
    else:
        candidates = [str(args.candidate or experiment["candidate_station"])]
    if not candidates or len(set(candidates)) != len(candidates):
        raise ValueError(f"Candidate station set must be non-empty and unique: {candidates}")
    candidate_set = set(candidates)
    candidate = ";".join(candidates)
    accepted_parent = str(experiment["parent_run"])
    output = args.output_dir.resolve() if args.output_dir else trial / "reports" / "station_screening" / "ablation_comparison"
    ...
    reach_column = "reach_id" if "reach_id" in base_main.columns else "comid"
    base_reach_map = base_main.drop_duplicates("q_site").set_index("q_site")[reach_column].astype(str)
    trial_reach_map = trial_main.drop_duplicates("q_site").set_index("q_site")[reach_column].astype(str)
    candidate_reaches = {str(base_reach_map.get(station, "")) for station in candidates}
    ...
    def add_scope(base_frame: pd.DataFrame, trial_frame: pd.DataFrame, obs: str, pred: str, scope: str) -> None:
        ...
        added_reaches = {str(trial_reach_map.get(station, "")) for station in added}
        set_audits.append(
            {
                "scope": scope,
                ...
                "added_station_reaches": ";".join(sorted(added_reaches)),
                "all_added_stations_are_same_reach_alternatives": added_reaches.issubset(candidate_reaches),
                "candidate_absent_from_trial": candidate_set.isdisjoint(trial_sites),
                "no_unexpected_missing_from_trial": set(missing).issubset(candidate_set),
            }
        )
        ...
        candidate_metric = station_metrics(base_frame[base_frame["q_site"].isin(candidates)], obs, pred).set_index("q_site")
        for station in candidates:
            if station not in candidate_metric.index:
                candidate_rows.append({"scope": scope, "q_site": station, "n": 0, "NSElog": np.nan, "KGE": np.nan, "PBIAS_pct": np.nan, "good": False})
            else:
                candidate_rows.append({"scope": scope, "q_site": station, **candidate_metric.loc[station].to_dict()})
    ...
    calibration_metric = station_metrics(calibration_base[calibration_base["q_site"].isin(candidates)], "Q_obsv_cfs", "Q_pred_cfs").set_index("q_site")
    calibration_rows = []
    for station in candidates:
        if station not in calibration_metric.index:
            calibration_rows.append({"scope": "calibration_2010_2018", "q_site": station, "n": 0, "NSElog": np.nan, "KGE": np.nan, "PBIAS_pct": np.nan, "good": False})
        else:
            calibration_rows.append({"scope": "calibration_2010_2018", "q_site": station, **calibration_metric.loc[station].to_dict()})
    candidate_rows = calibration_rows + candidate_rows
    ...
    fold_trial_frames = {
        fold_id: read_csv(trial / "reports" / "station_screening" / "blocked_folds" / fold_id / "evaluation_predictions.csv")
        for fold_id in FOLDS
    }
    zero_by_station = {}
    for station in candidates:
        policy_match = trial_policy[trial_policy["station_name"].astype(str).eq(station)]
        policy_excludes = bool(
            not policy_match.empty
            and str(policy_match.iloc[0]["exclude_before_training"]).strip().lower() in {"true", "1", "yes"}
        )
        input_rows = int(
            trial_input["q_site"].astype(str).eq(station).sum()
            + trial_input["station_id"].astype(str).eq(station).sum()
        )
        main_rows = int(trial_main["q_site"].astype(str).eq(station).sum())
        fold_rows = int(sum(frame["q_site"].astype(str).eq(station).sum() for frame in fold_trial_frames.values()))
        zero_by_station[station] = {
            "policy_excludes_before_training": policy_excludes,
            "input_rows": input_rows,
            "main_Q72_Q78_rows": main_rows,
            "blocked_Q72_rows": fold_rows,
            "passed": bool(policy_excludes and input_rows == 0 and main_rows == 0 and fold_rows == 0),
        }
    candidate_input_rows = int(sum(x["input_rows"] for x in zero_by_station.values()))
    candidate_main_rows = int(sum(x["main_Q72_Q78_rows"] for x in zero_by_station.values()))
    candidate_fold_rows = int(sum(x["blocked_Q72_rows"] for x in zero_by_station.values()))
    policy_excludes_candidate = bool(all(x["policy_excludes_before_training"] for x in zero_by_station.values()))
    zero_participation = {
        "candidate_station": candidate,
        "candidate_stations": candidates,
        "trial_run": experiment_run.name,
        "excluded_model_run": trial.name,
    ...
    persistent_by_station = {}
    for station in candidates:
        station_rows = candidate_metrics[candidate_metrics["q_site"].astype(str).eq(station)]
        station_folds = station_rows[station_rows["scope"].isin(FOLDS)]
        station_calibration = station_rows[station_rows["scope"].eq("calibration_2010_2018")].iloc[0]
        failed_folds = int((~station_folds["good"].astype(bool)).sum())
        persistent_by_station[station] = {
            "calibration_good": bool(station_calibration["good"]),
            "failed_blocked_folds": failed_folds,
            "persistent_failure": bool(
                not bool(station_calibration["good"])
                and failed_folds >= int(persistent_policy["minimum_failed_blocked_folds"])
            ),
        }
    candidate_persistent_failure = bool(all(x["persistent_failure"] for x in persistent_by_station.values()))
    candidate_calibration_good = bool(all(x["calibration_good"] for x in persistent_by_station.values()))
    candidate_failed_folds = int(min(x["failed_blocked_folds"] for x in persistent_by_station.values()))
    ...
    no_unexpected_added = bool(set_audit["all_added_stations_are_same_reach_alternatives"].all())
    ...
    gates = {
        ...
        "candidate_persistent_failure_by_station": persistent_by_station,
        "candidate_calibration_good": candidate_calibration_good,
        ...
        "no_unexpected_added_stations": no_unexpected_added,
    ...
    report = f"""# {experiment_run.name} Station Ablation Decision — Policy {policy['policy_version']}
...
- all candidates calibration good: {candidate_calibration_good}
- minimum candidate failed blocked folds: {candidate_failed_folds}/3
- all candidates persistent failure: {candidate_persistent_failure}
