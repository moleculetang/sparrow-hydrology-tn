...
def shapefile_parts(path: Path) -> list[Path]:
    # GIS applications can leave transient ``*.lock`` files alongside the
    # authoritative shapefile parts.  They are not data inputs, can be held
    # open by another process, and must not make an otherwise reproducible
    # source manifest fail.
    return sorted(
        candidate
        for candidate in path.parent.glob(f"{path.stem}.*")
        if not candidate.name.lower().endswith(".lock")
    )
