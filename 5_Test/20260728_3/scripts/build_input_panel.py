...
def write_readme() -> None:
    ...
    text = f"""# {RUN_DIR.name} Stage-0 Baseline and Diagnostic Run

This is the first executable run in the `20260727_5` model-development series. It rebuilds the Pearl River Basin Q model from source data while keeping the `20260727_4` model structure and hyperparameters unchanged.

- accepted parent before this run: `{parent_run}`
- generation type: `{generation_type}`
- candidate station: `{candidate or 'none'}`
- decision state: pending the identical-common-station ablation gate
...
- Later experiments use sequential `20260727_N` folders and change one major hypothesis per folder.
