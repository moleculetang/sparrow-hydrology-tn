...
def load_data(product='formal',calendar='CENTRAL',verify_hashes=True):
    ...
    source=pd.read_parquet(SOURCE[product])
    source=source.loc[source.calendar_scenario.eq(calendar)].sort_values(['year','month','reach_id'])
...
@njit(cache=False)
def local_daily_kernel(probability,survival,fast_fraction,lower_release,source,crop,mid,days_per_month,uniform):
    ...
    for d in range(nd):
        ...
        for r in range(nr):
            ...
            demand=crop[m,r]/days_per_month[m] if uniform else (crop[m,r] if first else 0.0)
