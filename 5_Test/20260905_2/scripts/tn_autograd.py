...
@njit(cache=False)
def local_forward(p,s,f,l,source,crop,mid,days,uniform):
    nd,nr=p.shape
    ...
    for d in range(nd):
        ...
        for r in range(nr):
            ...
            demand=crop[m,r]/days[m] if uniform else (crop[m,r] if first else 0.)
...
def local_backward(gfast,gslow,p,s,f,l,a):
    nd,nr=p.shape
    gp=np.zeros_like(p);gs=np.zeros(nr)
...
class LocalN(torch.autograd.Function):
    @staticmethod
    def forward(ctx,p,s,data,uniform):
        ...
        f,slow,a=local_forward(pp,ss,data.fast_fraction,data.lower_release,data.source,data.crop,
                              data.mid,data.stops-data.starts,uniform)
    ...
    def backward(ctx,gf,gs):
        ...
        gp,g_survival=local_backward(gf,gs,np.ascontiguousarray(p.detach().numpy()),np.ascontiguousarray(s.detach().numpy()),
                                    data.fast_fraction,data.lower_release,ctx.available)
        return torch.from_numpy(gp),torch.from_numpy(g_survival),None,None


class RiverN(torch.autograd.Function):
    @staticmethod
    def forward(ctx,local,vf,data,exposure):
        v=float(vf.detach())
        ...
        return torch.from_numpy(data.monthly_sum(result['inlet'])),torch.from_numpy(data.monthly_sum(result['official']))

    @staticmethod
    def backward(ctx,g_inlet_month,g_official_month):
        data=ctx.data
            ...
            if r not in by_control:
                ...
                if len(meta['controls'])>1:
                    if k not in capture_adjoints:
                        capture_adjoints[k]=reservoir_backward(np.ascontiguousarray(gi[:,meta['target']]),
                                                                np.ascontiguousarray(data.release_fraction[:,k]))
                    gp=go[:,r]+capture_adjoints[k]
                    gdel=gp
                else:
                    gout=go[:,r]+gi[:,meta['target']]
                    gp=reservoir_backward(np.ascontiguousarray(gout),np.ascontiguousarray(data.release_fraction[:,k]))
        ...
        return torch.from_numpy(gl),torch.tensor(gv,dtype=torch.float64),None,None
