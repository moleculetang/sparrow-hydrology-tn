...
def add_hydrologic_features(
    ...
    out["ttd_wet_interaction"] = np.log1p(out["ttd_net_cfs"].clip(lower=0.0)) * np.maximum(state, 0.0)
    out["ttd_slow_release"] = np.log1p(out["ttd_net_cfs"].clip(lower=0.0)) * np.maximum(-state, 0.0)
...
FIXED_FEATURES = [
    ...
    "ttd_wet_interaction",
    "ttd_slow_release",
    "ttd_self_log_net_dev",
    "ttd_self_log_threshold_dev",
    "ttd_self_net_ratio_dev",
    "ttd_self_threshold_ratio_dev",
    "ttd_self_wet_dev",
    "ttd_self_slow_dev",
    "ttd_down1_log_net_dev",
    "ttd_down1_log_threshold_dev",
    "ttd_down1_net_ratio_dev",
    "ttd_down1_threshold_ratio_dev",
    "ttd_down1_wet_dev",
    "ttd_down1_slow_dev",
    "ttd_down2_log_net_dev",
    "ttd_down2_log_threshold_dev",
    "ttd_down2_net_ratio_dev",
    "ttd_down2_threshold_ratio_dev",
    "ttd_down2_wet_dev",
    "ttd_down2_slow_dev",
    "aridity",
    "log_aet",
...
TTD_GROUP_DEVIATION_FEATURES = [
    feat
    for feat in FIXED_FEATURES
    if feat.startswith("ttd_self_") or feat.startswith("ttd_down1_") or feat.startswith("ttd_down2_")
]


def feature_group(feature: str) -> str:
    if feature in TTD_GROUP_DEVIATION_FEATURES:
        return "ttd_group_deviation"
...
def prepare_design(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["log_ttd_net"] = np.log1p(out["ttd_net_cfs"].fillna(0).clip(lower=0))
    out["log_ttd_threshold"] = np.log1p(out["ttd_threshold_cfs"].fillna(0).clip(lower=0))
    for label, mask_col in [
        ("self", "res_order_self"),
        ("down1", "res_order_down1"),
        ("down2", "res_order_down2"),
        ...
        mask = out[mask_col].fillna(0.0).astype(float)
        out[f"ttd_{label}_log_net_dev"] = out["log_ttd_net"] * mask
        out[f"ttd_{label}_log_threshold_dev"] = out["log_ttd_threshold"] * mask
        out[f"ttd_{label}_net_ratio_dev"] = out["ttd_net_ratio"].fillna(0.0) * mask
        out[f"ttd_{label}_threshold_ratio_dev"] = out["ttd_threshold_ratio"].fillna(0.0) * mask
        out[f"ttd_{label}_wet_dev"] = out["ttd_wet_interaction"].fillna(0.0) * mask
        out[f"ttd_{label}_slow_dev"] = out["ttd_slow_release"].fillna(0.0) * mask
...
def fit_map_ridge(
    ...
    for i, feat in enumerate(FIXED_FEATURES, start=1):
        if feat in TTD_GROUP_DEVIATION_FEATURES:
            penalty[i] = 1.0 / max(ttd_group_sigma, EPS)
                        ...
                        for res_rho in [0.85, 0.95]:
                            for release_alpha in [0.35, 0.65]:
                                ...
                                for fixed_sigma in [1.5, 3.0]:
                                    for ttd_group_sigma in [0.20, 0.40, 0.80]:
                                        for station_sigma in [0.60, 1.00]:
                                            beta = fit_map_ridge(
                                                ...
                                                ttd_group_sigma=ttd_group_sigma,
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260606_14 grouped TTD + reservoir-proxy strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_grouped_ttd_reservoir_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_grouped_ttd_reservoir_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_grouped_ttd_reservoir_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    beta = fit_map_ridge(
        ...
        ttd_group_sigma=float(best["ttd_group_sigma"]),
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(best["ttd_group_sigma"]) if feat in TTD_GROUP_DEVIATION_FEATURES else float(best["fixed_sigma"]),
            }
        )
    n_fixed = 1 + len(FIXED_FEATURES)
    station_effect = pd.DataFrame({"q_site": stations, "station_random_intercept": beta[n_fixed:]})
    fixed_params = pd.DataFrame(param_rows)
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    group_strength = (
    ...
    group_strength.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_parameter_group_strength.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_station_random_effects.csv", index=False, encoding="utf-8-sig")
    ...
    kernel_df.to_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_kernel_weights.csv", index=False, encoding="utf-8-sig")
    fig, ax = plt.subplots(figsize=(6.2, 3.8))
    ax.bar(kernel_df["lag_month"], kernel_df["weight"], color="#477998")
    ax.set_xlabel("Lag month")
    ax.set_ylabel("Kernel weight")
    ax.set_title(f"Best grouped TTD kernel: L={int(best['ttd_lag'])}, tau={float(best['ttd_tau']):.1f}")
    ax.grid(axis="y", alpha=0.22)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_grouped_ttd_reservoir_kernel_weights.png", dpi=300)
    plt.close(fig)

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260606_14 grouped TTD + explicit reservoir release/attenuation proxy model",
        "",
        "This numeric generation tests a grouped partial-pooling TTD structure: all reaches share a global short-TTD response, while reservoir self/down1/down2 reaches receive additional class-specific TTD deviation terms with a separate stronger Gaussian prior.",
        "",
        "## Equation",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_grouped_ttd_reservoir_metric_summary.csv",
        "- reports/monthly_bayes_grouped_ttd_reservoir_metrics_by_station.csv",
        "- reports/monthly_bayes_grouped_ttd_reservoir_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_grouped_ttd_reservoir_fixed_parameters.csv",
        "- reports/monthly_bayes_grouped_ttd_reservoir_station_random_effects.csv",
        "- reports/monthly_bayes_grouped_ttd_reservoir_kernel_weights.csv",
        "",
    ]
    (RUN_DIR / "README_20260606_14.md").write_text("\n".join(lines), encoding="utf-8")
