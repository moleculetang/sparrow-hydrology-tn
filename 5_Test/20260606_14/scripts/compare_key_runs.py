...
def parameter_group_summary() -> pd.DataFrame:
    params = pd.read_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_fixed_parameters.csv")
    params = params[params["parameter"] != "intercept"].copy()

    if "feature_group" in params.columns:
        params["parameter_group"] = params["feature_group"]
    else:
        params["parameter_group"] = np.where(params["parameter"].str.contains("ttd", case=False, na=False), "ttd", "other")
    if "prior_sigma" not in params.columns:
        params["prior_sigma"] = np.nan
...
def plot_comparison(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    ordered = summary.set_index("run_id").loc[["20260606_4", "20260606_7", "20260606_11", "20260606_12", "20260606_13", "20260606_14"]].reset_index()
    ...
    colors = ["#64748B", "#477998", "#8B5E34", "#C44E24", "#2E7D32", "#7C3AED"]
    ...
    plot_delta = delta_summary.set_index("reference").loc[["20260606_4", "20260606_7", "20260606_11", "20260606_12", "20260606_13"]].reset_index()
    ...
    ax.set_title("20260606_14 good-station transitions")
    ax.grid(axis="y", alpha=0.22)
...
def write_markdown(summary: pd.DataFrame, delta_summary: pd.DataFrame, param_groups: pd.DataFrame) -> None:
    best = pd.read_csv(REPORT_DIR / "monthly_bayes_grouped_ttd_reservoir_hyperparameter_grid.csv").iloc[0]
    val = summary[summary["run_id"] == "20260606_14"].iloc[0]
    lines = [
        "# 20260606_14 numeric generation",
        "",
        "## Model Tested",
        "",
        "`20260606_14` tests a grouped partial-pooling TTD structure without using 2019-2022 observations in fitting:",
        "",
        "- All reaches share the global TTD response from the successful short-TTD family.",
        "- Reservoir self/down1/down2 reaches receive additional class-specific TTD deviation parameters.",
        "- Those class-specific TTD deviations use a separate stronger prior `ttd_group_sigma`.",
        ...
        "## Interpretation",
        "",
        f"`20260606_14` has validation median_NSE_log={val['median_NSE_log']:.6f}, median_KGE={val['median_KGE']:.6f}, median_abs_PBIAS={val['median_abs_PBIAS_pct']:.6f}%, and good stations={int(val['good_validation_station_count'])}.",
        "",
        "This was the intended large mechanism test after `20260606_12` and `20260606_13`: not a hard gate and not simple stacking, but global TTD plus shrunken reservoir-class deviations. The result is clear negative evidence for continuing the TTD-reservoir combination line: it behaves much like the simple stacked model, does not recover the KGE/PBIAS strength of `20260606_4` or `20260606_11`, and does not recover the good-station count of `20260606_7`.",
        "",
        "Decision: diagnostic_negative_not_promoted_and_stop_ttd_reservoir_combination_line. The next major mechanism should move to nonlinear runoff/peak-gain behavior or boundary/cross-border inflow treatment rather than more TTD-reservoir tuning.",
        "",
        "## Main Files",
        "",
...
def append_daily_log(summary: pd.DataFrame, delta_summary: pd.DataFrame) -> None:
    ...
    vs13 = delta_summary[delta_summary["reference"] == "20260606_13"].iloc[0]
    line = (
        "[2026-06-06] Created and ran 20260606_14 as a grouped partial-pooling TTD plus reservoir-proxy mechanism experiment. "
        "Model change: kept the ET-aware monthly empirical-Bayes/MAP base and 20260606_11 reservoir storage/release/attenuation terms, kept a global short-TTD response for all reaches, and added reservoir self/down1/down2 class-specific TTD deviation parameters with a separate Gaussian prior strength ttd_group_sigma. "
        "This was a deliberately large parameter-structure change after 20260606_12 simple stacking and 20260606_13 hard gating, not another lag/tau micro-tweak. "
        "Best hyperparameters selected without 2019-2022 observations: rho=0.70, wm=480.0, et_gamma=0.50, ttd_lag=3 months, ttd_tau=1.5 months, res_rho=0.85, release_alpha=0.35, fixed_sigma=3.0, ttd_group_sigma=0.80, station_sigma=1.0. "
        ...
        f"Against 20260606_13: NSE_log improved at {int(vs13['NSE_log_improved_count'])}/104, KGE improved at {int(vs13['KGE_improved_count'])}/104, abs(PBIAS) improved at {int(vs13['abs_PBIAS_improved_count'])}/104, good +{int(vs13['new_good_count'])}-{int(vs13['lost_good_count'])}. "
        "Decision: diagnostic_negative_not_promoted_and_stop_ttd_reservoir_combination_line. The grouped partial-pooling version does not solve the KGE/PBIAS and good-station tradeoff, so the next significant mechanism should move to nonlinear runoff/peak-gain terms or boundary/cross-border inflow handling rather than further TTD-reservoir tuning. "
        "Main files: 20260606_14/reports/key_run_validation_comparison.csv, ttd_reservoir_delta_summary.csv, ttd_reservoir_group_delta_summary.csv, monthly_bayes_grouped_ttd_reservoir_parameter_group_strength.csv, README_20260606_14.md.\n"
    )
...
def main() -> None:
    ...
    candidate = metrics["20260606_14"]
    failure = pd.read_csv(REPORT_DIR / "failure_taxonomy.csv")

    delta_rows = []
    group_rows = []
    for reference_id in ["20260606_4", "20260606_7", "20260606_11", "20260606_12", "20260606_13"]:
