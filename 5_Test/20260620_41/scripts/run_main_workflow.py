        ...
        writer.writerow(
            {
                ...
                "model_change": "rebuild_Qmain_from_source_inputs_inside_this_folder after excluding Beiliu, Guanliang, Pingshan(III), and Lishi(II) before station matching and model training; no_layered_upstream_transfer; keep_CHM_PRE_V2_1_PPT; keep_CMFD_FAO56_ET0_PET; keep_ERA5_Land_monthly_evaporation_AET_2006_2022",
...
def write_workflow_logs(step_rows: list[dict[str, object]], final: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## Purpose",
        "",
        "Run the pure-simulation Q_main hydrologic flow baseline in a single clean folder after excluding 北流站、官良站、平山（三）站、犁市（二）站 from the active input table before station matching and model training. CHM_PRE V2.1 reach-catchment monthly rainfall, CMFD V2.0 six-variable FAO56 Penman-Monteith ET0/PET, and ERA5-Land monthly evaporation AET are retained. The case then runs base regression, mass-conserving intermediate baselines, alpha sweep, station-adaptive comparator, final reach-class light mass constraint, station-level diagnostics, and the full main-model document locally. It intentionally does not run layered upstream transfer.",
        "",
    ...
    lines.extend(
        ...
        "- 北流站、官良站、平山（三）站、犁市（二）站 do not participate in active model training in this run.",
        ]
    )
    text = "\n".join(lines) + "\n"
...
def write_final_summary(final: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        "It intentionally excludes layered upstream transfer and post-hoc residual correction. It also excludes 北流站、官良站、平山（三）站、犁市（二）站 from active input construction and model training because those stations are currently treated as suspected observation/representativeness problem stations.",
        "",
