...
def _station_weight_map() -> dict[str, float]:
    if os.environ.get("PRB_Q_DISABLE_STATION_WEIGHTS") == "1":
        return {}
...
def _reservoir_influence_class_by_reach(max_order: int = 2) -> dict[int, tuple[float, float]]:
    ...
    # Hydrologic prior: separate reservoir-body behavior from near-downstream behavior.
    # Both are strongly distance-limited, avoiding the 20260603_9 all-downstream binary influence.
...
def apply_prb_q_modifications(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Strict validation split: 2019-2022 observed Q is hidden from calibration.
    # The original Q_obsv_cfs stays in inputs/indata.parquet for independent
    # validation reporting, but the model sees load only through 2006-2018.
    year = _series(out, "year")
    validation_mask = year.between(VALIDATION_START_YEAR, VALIDATION_END_YEAR)
    out.loc[validation_mask, "load"] = np.nan
    ...
    out["wd"] = 0.0
    out["storage"] = 0.0
    reservoir_class = _reservoir_influence_class_by_reach(max_order=2)
    reach_id = _series(out, "comid").astype("Int64")
    reservoir_self = reach_id.map(lambda rid: reservoir_class.get(int(rid), (0.0, 0.0))[0] if pd.notna(rid) else 0.0)
    reservoir_down = reach_id.map(lambda rid: reservoir_class.get(int(rid), (0.0, 0.0))[1] if pd.notna(rid) else 0.0)
    out["res_decay_self"] = reservoir_self.fillna(0.0).astype(float)
    out["res_decay_down"] = reservoir_down.fillna(0.0).astype(float)
    out = out.sort_values(["comid", "period"]).copy()
    lagged_ppt = out.groupby("comid")["ppt"].shift(1).fillna(0.0)
    out["res_lag_self"] = lagged_ppt * out["res_decay_self"]
    out["res_lag_down"] = lagged_ppt * out["res_decay_down"]
    out.loc[out["res_decay_self"] <= 0, "res_lag_self"] = 0.0
    out.loc[out["res_decay_down"] <= 0, "res_lag_down"] = 0.0
...
CONFIG: Dict[str, Any] = {
    ...
    "adjust_units": 1,
    "ndt_per_year": 12,
    "srcvar": "foreign_cfs point1_cfs ppt res_lag_self res_lag_down",
    "bsrcvar": "bforeign_cfs bpoint1_cfs bppt bres_lag_self bres_lag_down",
    "dlvvar": "plaet",
    "bdlvvar": "bplaet",
    "dlvdsgn": "0, 0, 1, 1, 1",
    ...
    "decvar": "wd",
    "bdecvar": "bwd",
    "resvar": "res_decay_self res_decay_down",
    "bresvar": "bres_decay_self bres_decay_down",
    "othvar": "",
    "bothvar": "",
    "betailst": (
        ...
        "bppt 0.40 0:. "
        "bres_lag_self 0.12 0:0.35 "
        "bres_lag_down 0.05 0:0.20 "
        "bplaet -0.1 .:. "
        "bwd 0.0 0:0 "
        "bres_decay_self 0.02 0:0.12 "
        "bres_decay_down 0.01 0:0.06"
    ),
    ...
    "staid": "staid",
    "optional_station_information": "year month quarter period q_site HUC12 WRIA Q_calc_cfs Q_ma_cfs",
    "lat": "",
    "lon": "",
    "ls_weight": "ls_weight",
    "waterid": "time_comid",
    "optional_reach_information": "year month quarter period q_site HUC12 WRIA Q_calc_cfs Q_ma_cfs",
    "inc_area": "incarea",
    "tot_area": "cumarea",
    "mean_flow": "flow_cfs",
