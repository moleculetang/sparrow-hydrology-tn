...
def era5_climate_monthly() -> pd.DataFrame:
    ...
    monthly["climate_source"] = "ERA5-Land monthly tp/e/t2m where available; missing years filled later from reach-month climatology"
...
def write_reports(indata: pd.DataFrame, reach: pd.DataFrame, topo: pd.DataFrame, discharge_m: pd.DataFrame, station_match: pd.DataFrame) -> None:
    ...
    required_mapping = pd.DataFrame(
        [
            ...
            ("PPT", "ERA5-Land monthly total precipitation; missing 2006-2009 filled from reach-month climatology", "0_reach_topology/data/raw/era5_land/monthly_prb_buffer"),
            ("AET", "absolute ERA5-Land total evaporation monthly value; missing 2006-2009 filled from reach-month climatology", "0_reach_topology/data/raw/era5_land/monthly_prb_buffer"),
            ("PET", "Thornthwaite PET from ERA5-Land 2m temperature monthly value; missing 2006-2009 filled from reach-month climatology", "0_reach_topology/data/raw/era5_land/monthly_prb_buffer"),
        ],
    ...
    summary = [
        "# 20260606_2 monthly PRB Q input build summary",
        "",
        "## Source boundary",
        ...
        "- New files were written only under `E:/SPARROW/5_Test/20260606_2`.",
        ...
        "",
        "## Climate inputs",
        "- PPT/AET/PET are reach-month values derived from downloaded ERA5-Land monthly files under `0_reach_topology/data/raw/era5_land/monthly_prb_buffer`.",
        "- ERA5 files were absent for 2006-2009, so those years use each reach-month's 2010-2022 climatological median. This is a data-length experiment, not a final climate reconstruction.",
...
def write_readme() -> None:
    text = """# 20260606_2 monthly PRB Q input build

This folder rebuilds the Pearl River Basin Q input table with complete 2010-2022 discharge plus the 2006-2009 monthly mean workbook at monthly scale.
...
- ERA5 climate is only available for 2010-2022 here; 2006-2009 PPT/AET/PET are filled from reach-month climatology and must be treated as a first-round data-length approximation.
...
conda --no-plugins run -n sparrow python E:/SPARROW/run_sparrow.py --config E:/SPARROW/5_Test/20260606_2/prb_q_config.py
conda --no-plugins run -n sparrow python scripts/evaluate_strict_validation_run.py
```
"""
...
def main() -> int:
    ...
    write_readme()
    log = [
        "# 20260606_2 monthly complete-station-only strict input rebuild log",
        "",
        ...
        "ERA5 2006-2009 climate was missing and filled from 2010-2022 reach-month climatology.",
        f"Rows: {len(indata)}",
