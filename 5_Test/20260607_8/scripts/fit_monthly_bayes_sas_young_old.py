    ...
    for rho in [0.70]:
        for wm in [480.0]:
            for et_gamma in [0.50, 0.75]:
                for sas_rho in [0.85, 0.93]:
                    for young_k in [0.8, 1.5]:
                        for storage_scale in [360.0, 720.0]:
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_8 station-level Bayesian random-slope + SAS young/old storage model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_5 parameter-level Bayesian random-slope structure and replaces simple TTD convolution with an explicit SAS-inspired young-water / old-storage release mechanism:",
        "",
        ", ".join(RANDOM_SLOPE_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`log(Q) = fixed ET/SAS hydrologic effects + station random intercept + station random hydrologic/SAS slope deviations + error`",
        "",
        "The SAS state variables and random slopes are parameter-level Bayesian/MAP partial pooling. They are not post-processing corrections and are selected/fitted without 2019-2022 observations.",
        "",
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_sas_young_old_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_sas_young_old_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
