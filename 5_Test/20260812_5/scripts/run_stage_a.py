...
REFERENCE_I0 = Path(r"E:\SPARROW\5_Test\20260812_3\outputs\I0\q72_three_fold_oof_predictions.parquet")
FOLDS = [
...
SCENARIOS = {
    "A0": {"scale": 0.35, "semantics": "legacy_empirical_scale_exact_i0_control"},
    "A1": {"scale": 1.0, "semantics": "unscaled_positive_input_equivalent_predictor"},
}
...
def exact_a0_gate() -> dict[str, object]:
    a0 = pd.read_parquet(ROOT / "outputs" / "A0" / "q72_three_fold_oof_predictions.parquet")
    ...
    (ROOT / "logs" / "A0_exact_reproduction_gate.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    if not result["pass"]:
        raise RuntimeError(f"A0 exact reproduction failed: {result}")
    return result


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("scenario", choices=["A0", "A1", "all"])
    args = parser.parse_args()
    payload: dict[str, object] = {}
    if args.scenario in {"A0", "all"}:
        payload["A0"] = assemble("A0")
        payload["A0_reproduction"] = exact_a0_gate()
    if args.scenario in {"A1", "all"}:
        if not (ROOT / "logs" / "A0_exact_reproduction_gate.json").exists():
            raise RuntimeError("A1 is blocked until A0 exact reproduction has passed")
        gate = json.loads((ROOT / "logs" / "A0_exact_reproduction_gate.json").read_text(encoding="utf-8"))
        if not gate.get("pass"):
            raise RuntimeError("A1 is blocked by failed A0 reproduction")
        payload["A1"] = assemble("A1")
