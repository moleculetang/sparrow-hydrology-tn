...
def main() -> None:
    ...
    a0 = frames["A0"].sort_values(KEY).reset_index(drop=True)
    a1 = frames["A1"].sort_values(KEY).reset_index(drop=True)
    ...
    base_station = station[station.scenario.eq("A0")]
    ...
    checks = {
        ...
        "every_fold_log_nse_decline_le_0_003": bool((fold_pivot.A1 >= fold_pivot.A0 - .003).all()),
        ...
        "path_mean_downstream_mae_worsen_le_1pct": float(path_pivot.A1.mean()) <= float(path_pivot.A0.mean())*1.01,
    ...
    result = {
        "A0": overall0,
        "A1": overall1,
        "delta_A1_minus_A0": {key: float(overall1[key]-overall0[key]) for key in ["raw_nse", "log_nse", "pbias_pct", "rmse_cfs", "log_rmse"]},
        ...
        "path_mean_downstream_mae_change_pct": float(100*(path_pivot.A1.mean()/path_pivot.A0.mean()-1)),
        "abs_pbias_change_points": float(abs(overall1["pbias_pct"])-abs(overall0["pbias_pct"])),
        "fold_log_nse_delta": {index: float(row.A1-row.A0) for index, row in fold_pivot.iterrows()},
    ...
    result["terminal"] = (
        ...
        if all_pass else "EMPIRICAL_035_SCALE_IS_PREDICTIVELY_ACTIVE_RETAIN_I0_PENDING_REDESIGN"
    )
