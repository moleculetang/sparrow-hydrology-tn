...
def crossborder_strength_by_reach(external_fraction: float, decay: float, max_order: int = 8) -> dict[int, float]:
    ...
    start_reach = 112  # 龙州（二）站所在左江跨境敏感 reach
...
def add_hydrologic_features(
    ...
    crossborder_class = crossborder_strength_by_reach(external_fraction=external_fraction, decay=crossborder_decay, max_order=8)
        ...
        for wm in [480.0]:
            for et_gamma in [0.50, 0.75]:
                for external_fraction in [0.25, 0.36, 0.50]:
                    for crossborder_decay in [0.60, 0.80]:
                        ...
                        for fixed_sigma in [1.5, 3.0]:
                            for crossborder_sigma in [0.6, 1.5, 3.0]:
...
def write_plots(pred_obs: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title("20260607_3 cross-border inflow proxy strict validation observed vs predicted")
    ax.grid(True, which="both", alpha=0.22)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_crossborder_validation_scatter.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "monthly_bayes_crossborder_validation_metric_distributions.png", dpi=300)
    plt.close(fig)
    ...
    fig.savefig(FIG_DIR / "monthly_bayes_crossborder_representative_hydrographs.png", dpi=300)
    plt.close(fig)
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_crossborder_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_crossborder_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_crossborder_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_crossborder_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_crossborder_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_crossborder_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(best["crossborder_sigma"]) if feat in CROSSBORDER_FEATURES else float(best["fixed_sigma"]),
            }
        )
    n_fixed = 1 + len(FIXED_FEATURES)
    station_effect = pd.DataFrame({"q_site": stations, "station_random_intercept": beta[n_fixed:]})
    fixed_params = pd.DataFrame(param_rows)
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_crossborder_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    (
        ...
        .to_csv(REPORT_DIR / "monthly_bayes_crossborder_parameter_group_strength.csv", index=False, encoding="utf-8-sig")
    )
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_crossborder_station_random_effects.csv", index=False, encoding="utf-8-sig")

    write_plots(pred_obs, metrics)

    lines = [
        "# 20260607_3 cross-border inflow proxy monthly empirical-Bayes model",
        "",
        "This experiment starts from the 20260606_4 ET-aware monthly empirical-Bayes base and tests a cross-border inflow proxy for the 左江/龙州 downstream chain. The input `boundary_cfs` column exists but is zero for all observed station-month rows, so this generation does not use an observed boundary-flow field.",
        "",
        "## Equation",
        "",
        ...
        "",
        "Hydrologic terms include the 20260606_4 ET-aware base terms plus cross-border proxy terms on the downstream path from reach 112: crossborder strength, base inflow proxy, dynamic water-balance inflow proxy, threshold inflow proxy, wetness interaction, dry-buffer interaction, and seasonal interactions.",
        "",
        "The Bayesian component is empirical-Bayes/MAP partial pooling: ordinary hydrologic coefficients, cross-border proxy coefficients, and station random intercepts have Gaussian priors. Cross-border terms use their own `crossborder_sigma`. Hyperparameters were selected using 2006-2015 training and 2016-2018 inner validation only, then refit on 2006-2018 and tested on hidden 2019-2022 observations.",
        "",
        ...
        "## Main Files",
        "",
        "- reports/monthly_bayes_crossborder_metric_summary.csv",
        "- reports/monthly_bayes_crossborder_metrics_by_station.csv",
        "- reports/monthly_bayes_crossborder_prediction_vs_observed_2006_2022.csv",
        "- reports/monthly_bayes_crossborder_fixed_parameters.csv",
        "- reports/monthly_bayes_crossborder_parameter_group_strength.csv",
        "- reports/monthly_bayes_crossborder_station_random_effects.csv",
        "- figure/monthly_bayes_crossborder_validation_scatter.png",
        "- figure/monthly_bayes_crossborder_validation_metric_distributions.png",
        "- figure/monthly_bayes_crossborder_representative_hydrographs.png",
        "",
    ]
