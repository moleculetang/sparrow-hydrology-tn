...
RUN_DIR = ROOT / "5_Test" / "20260601_1"
TOPO_RESULTS = ROOT / "0_reach_topology" / "results"
DISCHARGE_DIR = ROOT / "1_Inputs" / "DischargeData"
...
def discover_discharge_csvs() -> pd.DataFrame:
    ...
    if files.empty:
        raise RuntimeError("No discharge CSV files found in 1_Inputs/DischargeData.")
...
def write_reports(indata: pd.DataFrame, reach: pd.DataFrame, topo: pd.DataFrame, discharge_q: pd.DataFrame, station_match: pd.DataFrame) -> None:
    ...
    required_mapping = pd.DataFrame(
        [
            ...
            ("Q_obsv_cfs", "quarter mean discharge, m3/s converted to cfs", "1_Inputs/DischargeData"),
    ...
    summary = [
        "# 20260601_1 PRB Q input build summary",
        "",
        "## Source boundary",
        "- Topology and catchments came from `E:/SPARROW/0_reach_topology/results`.",
        "- Discharge observations came from `E:/SPARROW/1_Inputs/DischargeData`.",
        "- New files were written only under `E:/SPARROW/5_Test/20260601_1`.",
        "",
        "## Counts",
        f"- reaches: {len(reach)}",
...
def write_readme() -> None:
    text = """# 20260601_1 PRB Q input build

This folder rebuilds the Pearl River Basin Q input table with the complete discharge data in `E:/SPARROW/1_Inputs/DischargeData`.

Source data are read only from:

- `E:/SPARROW/0_reach_topology/results`
- `E:/SPARROW/1_Inputs/DischargeData`
...
conda --no-plugins run -n sparrow python E:/SPARROW/run_sparrow.py --config E:/SPARROW/5_Test/20260601_1/prb_q_config.py
```
"""
...
def main() -> int:
    ...
    log = [
        "# 20260601_1 run log",
        "",
        "Built PRB Q input table from existing topology results and complete discharge data in 1_Inputs/DischargeData.",
        f"Rows: {len(indata)}",
