...
def crossborder_strength_by_reach(external_fraction: float, decay: float, max_order: int = 2) -> dict[int, float]:
    path = ROOT / "0_reach_topology" / "results" / "tables" / "topology_edges.csv"
    if not path.exists():
        return {}
    topo = pd.read_csv(path, encoding="utf-8-sig")
...
def feature_group(feature: str) -> str:
    if feature in RESERVOIR_FEATURES:
        return "reservoir_operation_proxy"
    if feature in CROSSBORDER_FEATURES:
        return "local_crossborder_inflow_proxy"
...
def fit_map_ridge(
    ...
    for i, feat in enumerate(FIXED_FEATURES, start=1):
        if feat in RESERVOIR_FEATURES:
            penalty[i] = 1.0 / max(reservoir_sigma, EPS)
        elif feat in CROSSBORDER_FEATURES:
...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    grid_values = product(
        ...
        [720.0],  # storage_scale
        [0.75, 0.85],  # res_rho
        [0.35, 0.65],  # release_alpha
        [0.25],  # external_fraction
        [0.80, 0.90],  # crossborder_decay
        [2, 4],  # crossborder_max_order
        [3.0],  # fixed_sigma
        [0.6, 1.5],  # reservoir_sigma
        [1.0, 3.0],  # crossborder_sigma
        [1.0],  # station_sigma
        [0.15],  # slope_sigma
        [0.25],  # regime_slope_sigma
        [0.0],  # anomaly_weight
        [1.0],  # flow_contrast_weight
    )
    for (
        rho,
        wm,
        ...
        featured = prepare_design(
            add_hydrologic_features(
                ...
                crossborder_max_order=int(crossborder_max_order),
            )
        )
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_17 local cross-border plus reservoir season-regime Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_12 SAS young-water / old-storage season x flow-regime Bayesian hierarchy, then adds two targeted data-mechanism modules in the same fitted equation: the local-truncated reach-112 cross-border inflow proxy from 20260607_15 and the topology-defined reservoir operation proxy from 20260607_16.",
        "",
        ...
        "",
        "Local cross-border features: " + ", ".join(CROSSBORDER_FEATURES),
        "",
        "Reservoir operation features: " + ", ".join(RESERVOIR_FEATURES),
        "",
        "The fitted equation is:",
        "",
        "`logQ = fixed monthly hydrologic/SAS terms + local cross-border terms + reservoir operation terms + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The flow-state gates are computed from modeled antecedent wetness and SAS young-water fraction, and the season gates are calendar wet/dry season indicators. The cross-border and reservoir terms are generated from topology and hydrologic drivers only. No 2019-2022 observed Q is used to form gates, choose local modules, or correct predictions. This is one parameter-level Bayesian/MAP model, not prediction post-processing or output stitching.",
        "",
    ...
    (RUN_DIR / "README_20260607_17.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    beta = fit_map_ridge(
        ...
        reservoir_sigma=float(best["reservoir_sigma"]),
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_metric_summary.csv", index=False, encoding="utf-8-sig")
        ...
        param_rows.append(
            {
                ...
                "prior_sigma": float(
                    best["reservoir_sigma"]
                    if feat in RESERVOIR_FEATURES
                    else best["crossborder_sigma"]
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_reservoir_crossborder_season_regime_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
