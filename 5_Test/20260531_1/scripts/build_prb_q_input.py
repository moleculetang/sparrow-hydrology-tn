...
RUN_DIR = ROOT / "5_Test" / "20260531_1"
TOPO_RESULTS = ROOT / "0_reach_topology" / "results"
DISCHARGE_DIR = ROOT / "0_hydro_sediment_data" / "discharge"
...
def discover_discharge_csvs() -> pd.DataFrame:
    stage2 = DISCHARGE_DIR / "stage2_result"
    ...
    if files.empty:
        raise RuntimeError("No discharge CSV files found in stage2_result.")
...
def write_reports(indata: pd.DataFrame, reach: pd.DataFrame, topo: pd.DataFrame, discharge_q: pd.DataFrame, station_match: pd.DataFrame) -> None:
    ...
    required_mapping = pd.DataFrame(
        [
            ...
            ("Q_obsv_cfs", "quarter mean discharge, m3/s converted to cfs", "0_hydro_sediment_data/discharge/stage2_result"),
    ...
    summary = [
        "# 20260531_1 PRB Q input build summary",
        "",
        "## Source boundary",
        "- Topology and catchments came from `E:/SPARROW/0_reach_topology/results`.",
        "- Discharge observations came from `E:/SPARROW/0_hydro_sediment_data/discharge/stage2_result`.",
        "- New files were written only under `E:/SPARROW/5_Test/20260531_1`.",
        "",
        "## Counts",
        f"- reaches: {len(reach)}",
...
def write_readme() -> None:
    text = """# 20260531_1 PRB Q input build

This folder builds a first runnable Pearl River Basin Q input table for the native SPARROW Python runtime.

Source data are read only from:

- `E:/SPARROW/0_reach_topology/results`
- `E:/SPARROW/0_hydro_sediment_data/discharge/stage2_result`
...
conda --no-plugins run -n sparrow python E:/SPARROW/run_sparrow.py --config E:/SPARROW/5_Test/20260531_1/prb_q_config.py
```
"""
...
def main() -> int:
    ...
    log = [
        "# 20260531_1 run log",
        "",
        "Built PRB Q input table from existing topology results and discharge stage2_result.",
        f"Rows: {len(indata)}",
