...
DECISION_POLICY = json.loads(
    (RUN / "inputs" / "source_metadata" / "station_screening_decision_policy_v2.json").read_text(encoding="utf-8")
)
...
PROTECTED_STATIONS = {
    ...
    for row in DECISION_POLICY.get("domain_constraints", {}).get("protected_from_exclusion", [])
...
def main() -> None:
    ...
    workers = max(1, int(os.environ.get("SPARROW_AUDIT_WORKERS", "2")))

    def evaluate(row: object, quiet: bool) -> tuple[dict[str, object], pd.DataFrame | None]:
        site = str(row.q_site)
        try:
            result, fold_detail = audit_station(site, observed, base_predictions, quiet=quiet)
    ...
    def persist(result: dict[str, object], fold_detail: pd.DataFrame | None, progress: str) -> None:
        nonlocal prior
        site = str(result["q_site"])
        ...
        if fold_detail is not None:
            ...
            detail = pd.concat(
                [detail[~detail["q_site"].astype(str).eq(site)] if not detail.empty else detail, fold_detail],
                ignore_index=True,
            )
            detail.to_csv(existing_detail_path, index=False, encoding="utf-8-sig")
        if result["status"] == "completed":
            print(
                f"[{progress}] {site}: good={int(result['cumulative_good_gain']):+d}, "
                f"dNSE={float(result['mean_delta_median_NSElog']):+.5f}, "
                f"dKGE={float(result['mean_delta_median_KGE']):+.5f}, "
                f"escalate={bool(result['escalate_full_ablation'])}",
                flush=True,
            )
        else:
            print(f"[{progress}] ERROR {site}: {result['error']}", flush=True)

    indexed_rows = list(enumerate(remaining.itertuples(index=False), start=1))
    pending_rows = []
    for index, row in indexed_rows:
        if str(row.q_site) in completed:
            print(f"[{index}/{total}] resume-skip {row.q_site}", flush=True)
        else:
            pending_rows.append((index, row))
    if workers == 1:
        for index, row in pending_rows:
            result, fold_detail = evaluate(row, quiet=True)
            persist(result, fold_detail, f"{index}/{total}")
    else:
        print(f"parallel_station_workers={workers}; pending_stations={len(pending_rows)}", flush=True)
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(evaluate, row, False): (index, row) for index, row in pending_rows}
            finished = 0
            for future in as_completed(futures):
                index, _row = futures[future]
                result, fold_detail = future.result()
                finished += 1
                persist(result, fold_detail, f"{finished}/{len(pending_rows)}; source-index={index}/{total}")
    ...
    protected_signal = (
        results["q_site"].astype(str).isin(PROTECTED_STATIONS)
        & results["escalate_full_ablation"].astype(str).str.lower().isin({"true", "1", "yes"})
    )
    results["protected_from_exclusion"] = results["q_site"].astype(str).isin(PROTECTED_STATIONS)
    results["protected_signal_suppressed"] = protected_signal
    ...
    candidates = results[
        results["escalate_full_ablation"].astype(str).str.lower().isin({"true", "1", "yes"})
        & ~results["protected_from_exclusion"]
    ].copy()
    ...
    coverage = evidence[["q_site", "reach_id", "reach_class", "reservoir_deferred", "persistent_model_failure", "severe_data_flag_count"]].merge(
        results[
            [
                ...
                "escalate_full_ablation",
                "protected_from_exclusion",
                "protected_signal_suppressed",
            ]
        ],
        on="q_site",
        how="left",
    ...
    summary = {
        ...
        "protected_from_exclusion_stations": sorted(PROTECTED_STATIONS),
        "protected_signals_suppressed": int(protected_signal.sum()),
...
- protected-from-exclusion stations: {', '.join(summary['protected_from_exclusion_stations']) or 'none'}
- protected deletion signals recorded but suppressed: {summary['protected_signals_suppressed']}
