...
def choose_hyperparameters(df: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    rows = []
    for group_sigma in [0.10, 0.30, 0.80, 1.50]:
        beta = fit_map_ridge(
            tr,
            stations,
            mean,
...
def write_readme(best: dict[str, float], summary: pd.DataFrame, good: pd.DataFrame) -> None:
    lines = [
        "# 20260607_28 spatial-group response Bayesian model",
        "",
        "## Mechanism",
        "",
        "This generation keeps the 20260607_19 production-routing Bayesian mainline, then adds shared spatial-group hydrologic response slopes for area, flow-size, reservoir-influence, and season-area groups.",
        "",
        ...
        "",
        "`logQ = fixed monthly hydrologic/SAS terms + nonlinear production-routing terms + spatial group response slopes + station random intercept + station random slopes + season x flow-regime gated station random slopes + Gaussian priors`",
        "",
        "The spatial groups are computed only from reach/basin attributes and season, never from validation-period performance. This is parameter-level Bayesian/MAP fitting, not prediction post-processing.",
        "",
    ...
    (RUN_DIR / "README_20260607_28.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    grid.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_hyperparameter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pred_obs.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_prediction_vs_observed_2006_2022.csv", index=False, encoding="utf-8-sig")
    metrics = station_metrics(pred_obs)
    metrics.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_metrics_by_station.csv", index=False, encoding="utf-8-sig")
    good = metrics[metrics["good_validation"]].copy()
    not_good = metrics[~metrics["good_validation"]].copy()
    good.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    not_good.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_not_good_validation_stations.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_metric_summary.csv", index=False, encoding="utf-8-sig")
    ...
    fixed_params.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_fixed_parameters.csv", index=False, encoding="utf-8-sig")
    spatial_group_params.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_spatial_group_parameters.csv", index=False, encoding="utf-8-sig")
    random_slope_params.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_station_slope_parameters.csv", index=False, encoding="utf-8-sig")
    regime_slope_params.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_regime_slope_parameters.csv", index=False, encoding="utf-8-sig")
    station_effect.to_csv(REPORT_DIR / "monthly_bayes_spatial_group_response_station_random_effects.csv", index=False, encoding="utf-8-sig")
    pd.concat(
        [
        ...
        REPORT_DIR / "monthly_bayes_spatial_group_response_parameter_group_strength.csv", index=False, encoding="utf-8-sig"
    )
